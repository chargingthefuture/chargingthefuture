...
function buildIssueBody({ skillLabel, sector, occupation, rationale, source, sourceSubmissionId }) {
  ...
  const lines = [
    ...
    '- The append-only change-ops list is the only repo path that writes the taxonomy. It lives in',
    '  `ctf/scripts/lib/taxonomyChangeOps.mjs`, is validated by CI on every PR, and is applied',
    '  idempotently by `node ctf/scripts/seedSkillsTaxonomy.mjs` via the owner-dispatched',
    ...
    '   `TAXONOMY_CHANGE_OPS` in `ctf/scripts/lib/taxonomyChangeOps.mjs`, in a PR. Never edit or',
    '   reorder existing ops — append only.',
    '3. CI validates the ops list on the PR. After merge, the owner runs the',
    '   `Skills Taxonomy — Apply Change Ops (production)` workflow to apply it to the live DB;',
