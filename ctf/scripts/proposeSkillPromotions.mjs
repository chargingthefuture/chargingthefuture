...
function buildIssueBody({ skillLabel, sector, occupation, rationale, sourceSubmissionId }) {
  ...
  const lines = [
    ...
    '- The idempotent upsert logic for the legacy dataset is in',
    '  `ctf/scripts/lib/syncSkillsTaxonomyFromLegacy.mjs`.',
    '- Curated, owner-approved promotions are seeded idempotently by',
    '  `ctf/scripts/seedSkillsTaxonomyPromotions.mjs` (list lives in',
    '  `ctf/scripts/lib/seedSkillsTaxonomyPromotions.mjs`); it runs right after the legacy sync',
    '  so reseeds keep promoted skills.',
