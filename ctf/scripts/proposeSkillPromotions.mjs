...
function buildIssueBody({ skillLabel, sector, occupation, rationale, source, sourceSubmissionId }) {
  ...
  const lines = [
    ...
    '- Curated, owner-approved promotions are the only seed path that writes the taxonomy. They are',
    '  applied idempotently by `node ctf/scripts/seedSkillsTaxonomy.mjs` (list lives in',
    '  `ctf/scripts/lib/seedSkillsTaxonomyPromotions.mjs`); each looks its sector up by name in the',
    '  live DB (never creating sectors) and upserts the occupation/skill, so reseeds keep promoted',
    '  skills.',
    ...
    '   add it under the chosen sector first (an occupation must always have a sector).',
    '2. Add the skill (or skills) under that occupation.',
    '3. The preferred durable way is to append an entry to the promotions list in',
    '   `ctf/scripts/lib/seedSkillsTaxonomyPromotions.mjs` so the promotion survives reseeds, then',
    '   run that seed against the database.',
    '4. Only regenerate `schema.demo.sql` if you changed `schema.sql`; a promotion is usually data',
    '   only, with no schema change.',
