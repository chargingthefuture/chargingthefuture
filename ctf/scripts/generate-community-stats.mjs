 ...
 * that throws is skipped with a note rather than failing the entire draft, so a
...
const STAT_PROVIDERS = [
  ...
  },
  {
    ...
    async collect(client) {
      ...
      // fraction of the skills has anyone behind it. The percentage is integer and can read
  ...
  },
  {
    ...
    async collect(client) {
      ...
      // community-wide aggregate — a count of wallets, the total members are holding, and how
  ...
  },
  {
    ...
    async collect(client) {
      ...
      // Entire-community aggregates only — every underlying query is a SUM/COUNT, never a row. Both
