...
// fulfillment. The web route is POST /api/socket-relay/fulfillments/:id/chat and
// needs a real fulfillment id, so this first reads GET /api/socket-relay/my-fulfillments
...
export async function fetchSocketRelayStreamCredentials(): Promise<SocketRelayStreamCredentials> {
  const mine = await authedFetchJson<{ ok: boolean; items?: MyFulfillment[] }>(
    '/api/socket-relay/my-fulfillments',
  );
  ...
  const res = await authedFetch(`/api/socket-relay/fulfillments/${active.id}/chat`, {
    method: 'POST',
