...
function sourceLabel(p: DirectoryListItem, accent: string): { label: string; color: string } {
  if (p.source === 'community-generated') return { label: 'Community', color: COMMUNITY };
  if (p.source === 'admin') return { label: 'Admin-claimed', color: accent };
        ...
        <View style={styles.editHeader}>
          ...
          <View style={styles.editHeaderTitleWrap}>
            ...
            <Text style={styles.editHeaderSub}>
              {isUnclaimed ? 'Unclaimed' : 'Claimed'} · {sourceLabel(p, accent).label}
            </Text>
          </View>
    ...
    <View style={styles.root}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            const b = sourceLabel(item, accent);
                ...
                <View style={styles.cardTopRow}>
                  ...
                  <View style={styles.cardInfo}>
                    ...
                    <View style={styles.badgeRow}>
                      ...
                      <Text style={[styles.statusText, { color: item.claimedByUserId ? '#22C55E' : tokens.textSecondary }]}>
                        {item.claimedByUserId ? 'Claimed' : 'Unclaimed'}
                      </Text>
                    </View>
                  </View>
                </View>
                <Text style={[styles.cardHandle, { color: item.unclaimedHandle ? COMMUNITY : tokens.textSecondary }]}>{handle}</Text>
...
function makeStyles(t: ThemeTokens, accent: string) {
  return StyleSheet.create({
    ...
    tabActive: { backgroundColor: `${accent}18`, borderColor: `${accent}40` },
    tabText: { fontSize: 12, color: t.textSecondary },
    ...
    cardAvatarText: { fontSize: 13, fontWeight: '700', color: accent },
    cardInfo: { flex: 1, minWidth: 0 },
    cardName: { fontSize: 13, fontWeight: '700', color: t.textPrimary, marginBottom: 2 },
    cardRole: { fontSize: 11, color: t.textSecondary, marginBottom: 4 },
    ...
    editProfileBtnText: { fontSize: 12, fontWeight: '600', color: accent },
    deleteBtn: {
    ...
    deleteBtnText: { fontSize: 12, fontWeight: '600', color: t.danger },
    ...
    editHeaderTitle: { fontSize: 15, fontWeight: '700', color: t.textPrimary },
    editHeaderSub: { fontSize: 11, color: t.textSecondary },
    editHeaderClose: {
      width: 30,
      height: 30,
      borderRadius: 8,
    ...
    editHeaderCloseText: { fontSize: 14, color: t.textSecondary },
    ...
    attachHint: { fontSize: 12, color: t.textSecondary, lineHeight: 18, marginBottom: 8 },
    attachBtn: {
      marginTop: 8,
      backgroundColor: accent,
      borderRadius: 9,
    ...
    skillChipText: { fontSize: 12, color: accent },

    noticeText: { fontSize: 12, color: accent, marginTop: 4 },
    ...
    discardBtnText: { color: t.textSecondary, fontSize: 14 },
