...
export async function fetchSocketRelayStreamCredentials(): Promise<SocketRelayStreamCredentials> {
  ...
  const res = await fetch(`${baseUrl}/api/socketrelay/fulfillments/active/chat`, { method: 'POST' });
  const data = await res.json();
  if (!data.ok) throw new Error(data.message || 'Unable to load SocketRelay chat credentials');
  return {
