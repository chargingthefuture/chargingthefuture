...
// The web route is POST /api/trust-transport/trips/[tripId]/chat; it returns
...
export async function fetchTrustTransportStreamCredentials(tripId: string): Promise<TrustTransportStreamCredentials> {
  const data = await authedFetchJson<TripChatResponse>(`/api/trust-transport/trips/${tripId}/chat`, {
    method: 'POST',
    headers: {
