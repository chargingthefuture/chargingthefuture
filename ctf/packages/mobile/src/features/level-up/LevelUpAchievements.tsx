...
// value comes from GET /api/level-up/achievements. The mockup splits badges into
...
const BadgeTile: React.FC<{
  ...
  accent: string;
  t: ThemeTokens;
}> = ({ achievement, s, accent, t }) => {
  const color = TRACK_COLORS[achievement.track] ?? accent;
  return (
    <View style={[s.tile, achievement.earned ? s.tileEarned : s.tileLocked]}>
      <View style={[s.iconBox, { backgroundColor: achievement.earned ? `${accent}18` : t.border }]}>
        <Text style={[s.iconText, { color: achievement.earned ? accent : t.textMuted }]}>
          {achievement.earned ? '★' : '🔒'}
        </Text>
      </View>
      <Text style={[s.tileName, { color: achievement.earned ? TEXT : SUBTLE }]} numberOfLines={2}>{achievement.name}</Text>
      {achievement.track ? (
        <View style={[s.trackBadge, { backgroundColor: `${color}15` }]}>
          <Text style={[s.trackBadgeText, { color }]}>{achievement.track}</Text>
        </View>
      ) : null}
      {achievement.creditReward > 0 ? (
        <Text style={[s.reward, { color: achievement.earned ? accent : SUBTLE }]}>+{achievement.creditReward} SC</Text>
      ) : null}
    </View>
  );
}
  ...
  if (achievements.length === 0) {
    return (
      <View style={s.center}>
        ...
        <Text style={s.emptyBody}>Earn badges by completing cohort milestones. Badges are awarded — never bought.</Text>
      </View>
    );
  }
    ...
    <ScrollView contentContainerStyle={s.list}>
      <View style={s.statsRow}>
        {[
          ...
          { label: 'SC Gained', value: String(scFromBadges), color: accent },
      ...
      {earned.length > 0 ? (
        <View style={{ marginBottom: 20 }}>
          <Text style={s.sectionLabel}>Earned — {earned.length} badges</Text>
          <View style={s.grid}>
            {earned.map((a) => <BadgeTile key={a.id} achievement={a} s={s} accent={accent} t={tokens} />)}
          </View>
        </View>
      ) : null}
      ...
      {locked.length > 0 ? (
        <View>
          <Text style={s.sectionLabelDim}>Locked — {locked.length} badges</Text>
          <View style={[s.grid, { opacity: 0.7 }]}>
            {locked.map((a) => <BadgeTile key={a.id} achievement={a} s={s} accent={accent} t={tokens} />)}
          </View>
        </View>
      ) : null}
    </ScrollView>
...
function makeStyles(t: ThemeTokens, accent: string) {
  return StyleSheet.create({
    ...
    tile: { width: '31%', backgroundColor: t.surface, borderRadius: 10, padding: 12, borderWidth: 1, alignItems: 'center' },
    tileEarned: { borderColor: `${accent}30` },
    tileLocked: { borderColor: t.border },
    ...
    trackBadge: { borderRadius: t.radius, paddingHorizontal: 6, paddingVertical: 1, marginBottom: 4 },
