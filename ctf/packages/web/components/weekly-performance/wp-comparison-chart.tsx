...
export function WeeklyPerformanceComparisonChart({ comparison }: { comparison: WpComparison | null }) {
  ...
  return (
    <div style={{ padding: "20px 24px", borderRadius: 16, background: t.SURFACE, border: `1px solid ${t.BORDER_SOLID}`, marginBottom: 24 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <div style={{ fontSize: 14, fontWeight: 600, color: t.TITLE }}>This week vs last week</div>
        <div style={{ display: "flex", gap: 12, fontSize: 11 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 5 }}><div style={{ width: 10, height: 10, borderRadius: 2, background: BRAND }} /><span style={{ color: t.MUTED }}>This week</span></div>
          <div style={{ display: "flex", alignItems: "center", gap: 5 }}><div style={{ width: 10, height: 10, borderRadius: 2, background: "rgba(255,255,255,0.15)" }} /><span style={{ color: t.MUTED }}>Last week</span></div>
        </div>
      </div>
      <div style={{ display: "flex", alignItems: "flex-end", gap: 16, height: 140 }}>
        {rows.map((row) => (
          <div key={row.metricKey} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6, minWidth: 0 }}>
            ...
            <span style={{ fontSize: 10, color: t.MUTED, textAlign: "center", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: "100%" }}>{humanizeMetricKey(row.metricKey)}</span>
          </div>
        ))}
      </div>
    </div>
