...
export function WeeklyPerformanceEmptyMain({ week, isCurrent = false }: { week: WpWeek | null; isCurrent?: boolean }) {
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", minHeight: 0, padding: "40px 48px" }}>
      <div style={{ padding: "28px 32px", borderRadius: 16, background: t.SURFACE, border: `1px solid ${t.BORDER_SOLID}`, marginBottom: 32 }}>
        <div style={{ textAlign: "center", padding: "32px 0", display: "flex", flexDirection: "column", alignItems: "center", gap: 14 }}>
          <div style={{ width: 64, height: 64, borderRadius: 18, background: `${t.ACCENT}10`, border: `1px dashed ${t.ACCENT}25`, display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          {week && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 20px", borderRadius: 20, background: "rgba(255,255,255,0.03)", border: `1px solid ${t.BORDER_SOLID}` }}>
              ...
              <span style={{ fontSize: 12, color: t.MUTED }}>Week: {formatWeekRange(week)}</span>
            </div>
          )}
        </div>
        <div style={{ display: "flex", alignItems: "flex-end", gap: 8, height: 80, opacity: 0.15 }}>
          {[50, 65, 45, 75, 80, 55, 30].map((h, i) => (
            <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
              <div style={{ width: "80%", height: `${h}%`, borderRadius: "3px 3px 0 0", background: t.ACCENT }} />
            </div>
          ))}
        </div>
      </div>
