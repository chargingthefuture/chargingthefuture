...
export function SkillsTaxonomyEmptyState({ isAdmin }: { isAdmin: boolean }) {
  ...
  return (
    <div style={{ width: "100%", minHeight: "100vh", background: t.BG, fontFamily: "'Inter',system-ui", color: t.TITLE, display: "flex", flexDirection: "column" }}>
      <div style={{ height: 56, borderBottom: `1px solid ${t.BORDER}`, display: "flex", alignItems: "center", padding: "0 28px", gap: 12, background: t.HEADER, flexShrink: 0 }}>
        <Layers size={18} color={t.ACCENT} />
        <div>
          <div style={{ fontSize: 15, fontWeight: 600 }}>Skills Taxonomy</div>
          <div style={{ fontSize: 12, color: t.MUTED }}>3-level hierarchy — sectors, job titles, skills</div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "48px 64px" }}>
        <div style={{ maxWidth: 520, textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", gap: 24 }}>
          <div style={{ width: 88, height: 88, borderRadius: 24, background: `${t.ACCENT}12`, border: `1px solid ${t.ACCENT}25`, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Layers size={42} style={{ color: t.ACCENT, opacity: 0.6 }} />
          </div>
          <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, width: "100%", maxWidth: 360 }}>
            {LEVELS.map(({ icon: Icon, label, desc }) => (
              <div key={label} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", borderRadius: 12, background: t.SURFACE, border: `1px solid ${t.BORDER_SOLID}`, textAlign: "left" }}>
