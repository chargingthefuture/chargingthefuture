...
import { getSkillUpTokens, type NavKey } from "./su-shared";
    ...
    <aside style={{ width: 220, background: t.SURFACE, borderRight: `1px solid ${t.BORDER_SOLID}`, display: "flex", flexDirection: "column", flexShrink: 0 }}>
      <div style={{ padding: "20px 16px 16px", borderBottom: `1px solid ${t.BORDER_SOLID}` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 15, color: t.TEXT_BODY }}>SkillUp</span>
        </div>
        ...
        {isAdmin && (
          <>
            ...
            {TRAINER_TOOLS.map(({ icon: Icon, label }) => (
              <a key={label} href="/admin/skill-up" style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 10px", borderRadius: 8, marginBottom: 2, cursor: "pointer", color: t.TEXT_SUBTLE, fontSize: 13, textDecoration: "none" }}>
                <Icon size={15} />
                {label}
              </a>
            ))}
