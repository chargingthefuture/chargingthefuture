...
// Palette from the UnlockPublic / MobileUnlockPublic design mockups, resolved through the
// theme-aware Unlock tokens (default theme keeps the shipped hex values).
  ...
  return (
    <div style={{ width: '100%', minHeight: '100dvh', background: t.BG, fontFamily: FONT_FAMILY, color: t.TITLE, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ height: 52, borderBottom: `1px solid ${t.BORDER_SOLID}`, display: 'flex', alignItems: 'center', padding: '0 28px', gap: 10 }}>
        <PublicShellBackLink />
        <UnlockIcon size={18} color={t.ACCENT} />
      ...
      <div style={{ padding: '48px 64px 40px', display: 'flex', gap: 48, alignItems: 'flex-start' }}>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 16 }}>
          <span style={{ padding: '4px 14px', borderRadius: 20, background: `${t.ACCENT}12`, border: `1px solid ${t.ACCENT}25`, fontSize: 12, color: t.ACCENT, fontWeight: 600, display: 'inline-block', width: 'fit-content' }}>
            Verified access only
          </span>
          <h1 style={{ margin: 0, fontSize: 34, fontWeight: 800, lineHeight: 1.15 }}>
            Create your account to begin<br />
            <span style={{ color: t.ACCENT }}>the verification process</span>
          </h1>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <a href={verifyUrl ?? signInUrl} style={{ padding: '14px 32px', borderRadius: 10, background: t.ACCENT, border: 'none', color: '#fff', fontSize: 15, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, textDecoration: 'none' }}>
              {verifyUrl ? 'Finish verifying' : <><UserPlus size={16} /> Get started — it&apos;s free</>}
            </a>
          </div>
        </div>
        <div style={{ width: 280, flexShrink: 0 }}>
          <div style={{ padding: '20px', borderRadius: 16, background: t.SURFACE_CARD, border: `1px solid ${t.BORDER_SOLID}` }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: t.ACCENT, marginBottom: 14 }}>Why Quora verification?</div>
            {REASONS.map(({ icon, t: title, d }) => (
              <div key={title} style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
                <span style={{ fontSize: 18, flexShrink: 0 }}>{icon}</span>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: t.TITLE, marginBottom: 2 }}>{title}</div>
      ...
      <div style={{ padding: '0 64px 48px' }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: t.TITLE, marginBottom: 16 }}>How verification works</div>
        <div style={{ display: 'flex', gap: 12 }}>
          {STEPS.map(({ n, icon, title, desc }) => (
            <div key={n} style={{ flex: 1, padding: '20px', borderRadius: 16, background: t.SURFACE_CARD, border: `1px solid ${t.BORDER_SOLID}` }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                <div style={{ width: 26, height: 26, borderRadius: '50%', background: `${t.ACCENT}12`, border: `1px solid ${t.ACCENT}25`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: t.ACCENT }}>{n}</div>
                <span style={{ fontSize: 18 }}>{icon}</span>
              </div>
              <div style={{ fontSize: 14, fontWeight: 600, color: t.TITLE, marginBottom: 6 }}>{title}</div>
              <div style={{ fontSize: 12, color: t.MUTED, lineHeight: 1.5 }}>{desc}</div>
            </div>
          ))}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <a href={verifyUrl ?? signInUrl} style={{ padding: '14px 24px', borderRadius: 12, background: t.ACCENT, border: 'none', color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, textDecoration: 'none' }}>
  ...
  return (
    <div style={{ width: '100%', minHeight: '100dvh', background: t.BG, fontFamily: FONT_FAMILY, color: t.TITLE, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '12px 16px', background: `${t.ACCENT}10`, borderBottom: `1px solid ${t.ACCENT}25`, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <PublicShellBackLink />
            <UnlockIcon size={16} color={t.ACCENT} />
            <span style={{ fontSize: 16, fontWeight: 700 }}>Unlock Access</span>
          </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '24px 16px' }}>
        <span style={{ padding: '3px 12px', borderRadius: 20, background: `${t.ACCENT}12`, border: `1px solid ${t.ACCENT}25`, fontSize: 11, color: t.ACCENT, fontWeight: 600, display: 'inline-block', marginBottom: 14 }}>
          Verified access only
        </span>
        ...
        <p style={{ margin: '0 0 22px', fontSize: 13, color: t.SUBTLE, lineHeight: 1.7 }}>
        ...
        <a href={verifyUrl ?? signInUrl} style={{ width: '100%', padding: '14px', borderRadius: 12, background: t.ACCENT, border: 'none', color: '#fff', fontSize: 15, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxSizing: 'border-box', marginBottom: 20, textDecoration: 'none' }}>
          {verifyUrl ? 'Finish verifying' : <><UserPlus size={15} /> Get started — it&apos;s free</>}
        </a>

        {/* How it works */}
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: t.TITLE, marginBottom: 12 }}>How verification works</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {STEPS.map(({ n, icon, title, desc }) => (
              <div key={n} style={{ display: 'flex', gap: 12, padding: '14px', borderRadius: 12, background: t.SURFACE_CARD, border: `1px solid ${t.BORDER_SOLID}`, alignItems: 'center' }}>
                <div style={{ width: 28, height: 28, borderRadius: '50%', background: `${t.ACCENT}12`, border: `1px solid ${t.ACCENT}25`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: t.ACCENT, flexShrink: 0 }}>{n}</div>
                <span style={{ fontSize: 18, flexShrink: 0 }}>{icon}</span>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: t.TITLE }}>{title}</div>
        ...
        <div style={{ padding: '14px', borderRadius: 14, background: t.SURFACE_CARD, border: `1px solid ${t.BORDER_SOLID}`, marginBottom: 20 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: t.ACCENT, marginBottom: 12 }}>Why Quora verification?</div>
          {REASONS.slice(0, 3).map(({ icon, t: title, d }) => (
