  ...
  return (
    <ScrollArea style={{ flex: 1, minHeight: 0 }}>
      <div style={{ padding: "24px" }}>
        <div style={{ fontSize: 22, fontWeight: 800, color: t.TITLE, marginBottom: 4 }}>Earn ServiceCredits</div>
        <div style={{ fontSize: 14, color: t.MUTED, marginBottom: 20 }}>A few rewards come from the platform. The rest you earn from other members.</div>

        <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.06em", color: t.MUTED, textTransform: "uppercase", marginBottom: 12 }}>From the platform</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 28 }}>
          {PLATFORM_EARN_METHODS.map((m) => {
            const inner = (
              <>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: t.TITLE, marginBottom: 4 }}>{m.title}</div>
                  <div style={{ fontSize: 12.5, color: t.SUBTLE, lineHeight: 1.5, marginBottom: 6 }}>{m.detail}</div>
                  <Badge style={{ background: `${m.color}15`, color: m.color, border: `1px solid ${m.color}30`, fontSize: 11 }}>{m.note}</Badge>
                </div>
                <div style={{ fontSize: 18, fontWeight: 800, color: t.ACCENT, flexShrink: 0 }}>{m.credits}</div>
              </>
            );
        ...
        <div style={{ fontSize: 20, fontWeight: 800, color: t.TITLE, marginBottom: 6 }}>Everything else is peer-to-peer</div>
        <div style={{ fontSize: 13, color: t.SUBTLE, lineHeight: 1.6, marginBottom: 14, maxWidth: 620 }}>
