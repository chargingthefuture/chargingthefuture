...
// One row of mutual_time_admin_audit_trail, as the admin route returns it.
type AuditEvent = {
  id: string;
  actor_id: string;
  command: string;
...
const COMMAND_LABELS: Record<string, string> = {
  "mutual-time.event.create": "Opened an event",
  "mutual-time.event.close": "Closed an event",
  "mutual-time.vote.save": "Saved a member's picks",
};
...
const REASON_LABELS: Record<string, string> = {
  not_found: "the event was not there",
  not_open: "the event was already closed",
...
function describeEvent(event: AuditEvent): { failed: boolean; explanation: string | null } {
...
function EventRow({ event, t }: { event: AuditEvent; t: Tokens }) {
  const { failed, explanation } = describeEvent(event);
  return (
...
function EventList({ events, loading, err, t }: { events: AuditEvent[] | null; loading: boolean; err: string | null; t: Tokens }) {
...
// Collapsible panel listing every admin action on Mutual Time — opening an event, closing one —
// including the ones that were refused and why. Loads lazily on first expand.
//
// This is the check on the admin, including the owner. Until 2026-08-28 these actions were written
// to the server's log and nowhere else, so closing an event people had put their time into left no
// record anyone could read back.
export function MutualTimeAuditPanel({ t }: { t: Tokens }) {
  ...
  const loadList = useCallback(async () => {
    ...
    setErr(null);
    try {
      const res = await fetch("/api/mutual-time/admin/audit-events?limit=200");
      if (!res.ok) {
      ...
      {expanded && (
        <div style={{ padding: "0 14px 14px" }}>
          <div style={{ fontSize: 11, color: t.SUBTLE, lineHeight: 1.5, marginBottom: 10 }}>
            Every admin action on Mutual Time, newest first — including the ones that were refused
