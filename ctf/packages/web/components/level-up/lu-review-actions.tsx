...
// Action controls for the LevelUp admin review queues (2026-08-05). Until now the dispute and
...
// cohort. These controls call the live, already-hardened routes. Kept out of lu-admin-shell.tsx
// (862 lines) per rule 116.
import { useState } from 'react';
import type { AdminDispute, AdminValidation } from './lu-admin-shared';
import type { LevelUpTokens } from './lu-shared';

type Outcome = { ok: boolean; message: string };

async function postLevelUpAction(url: string, body: Record<string, unknown>, fallback: string): Promise<Outcome> {
  try {
...
const buttonStyle = (t: LevelUpTokens, busy: boolean): React.CSSProperties => ({
...
export function DisputeResolveControl({ dispute, t, onDone }: {
  ...
  async function resolve() {
    ...
    setBusy(true);
    setError(null);
    const outcome = await postLevelUpAction(
      `/api/level-up/disputes/${dispute.id}/resolve`,
...
export function ValidationActions({ validation, t, onDone }: {
  ...
  async function run(action: 'validate' | 'release') {
    ...
    setError(null);
    const outcome = await postLevelUpAction(
      `/api/level-up/milestones/${validation.milestoneId}/${action}`,
...
export function ClaimTrainerControl({ cohortId, t, onDone }: {
  ...
  async function claim() {
    ...
    setError(null);
    const outcome = await postLevelUpAction(
      `/api/level-up/cohorts/${cohortId}/claim-trainer`,
