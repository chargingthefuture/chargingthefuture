...
// Build the multipart form body the contributions endpoint expects. Kept out of the submit handler
// so that handler stays a thin orchestration: build → send → apply the result.
function buildContributionPayload(input: {
  mode: Mode;
  file: File | null;
  filledPosts: LinkedPostDraft[];
  agreed: Set<string>;
  thirdPartyNote: string;
  askForQuoraUrl: boolean;
  quoraProfileUrl: string;
}): FormData {
  const { mode, file, filledPosts, agreed, thirdPartyNote, askForQuoraUrl, quoraProfileUrl } = input;
...
// All contribution state, the network calls, and the derived flags. Grouped into one hook so the
// component tree below is plain rendering. Hooks run in a fixed order (all state, then the history
// loader, then the effect) and are never conditional.
function useKnowledgeContribution(askForQuoraUrl: boolean) {
  ...
  function changeQuoraUrl(value: string) {
    setQuoraProfileUrl(value);
    setError(null);
  }
  ...
  function addPost() {
    setPosts((current) => [...current, { url: '', text: '' }]);
  }
  ...
  function updatePost(index: number, field: keyof LinkedPostDraft, value: string) {
    setPosts((current) => current.map((item, i) => (i === index ? { ...item, [field]: value } : item)));
    setError(null);
  }
  ...
  // Applies the server's answer: an error message on failure, or the receipt plus a full form reset
  // on success. Split out of submit so submit itself stays small.
  function handleSubmitResponse(ok: boolean, data: ContributionResponse | null) {
    if (!ok || !data?.contribution) {
...
export function ComicKnowledgeShell({ askForQuoraUrl = false }: { askForQuoraUrl?: boolean }) {
  ...
  const c = useKnowledgeContribution(askForQuoraUrl);
    ...
    <main style={{ minHeight: '100dvh', background: t.BG, color: t.TITLE, fontFamily: "'Inter',system-ui,sans-serif" }}>
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '24px 16px 48px' }}>
        ...
        <ConsentSection
          t={t}
          agreed={c.agreed}
          onToggleClause={c.toggleClause}
          allAgreed={c.allAgreed}
          askForQuoraUrl={askForQuoraUrl}
          quoraProfileUrl={c.quoraProfileUrl}
          onQuoraChange={c.changeQuoraUrl}
          thirdPartyNote={c.thirdPartyNote}
          onThirdPartyChange={c.setThirdPartyNote}
        />
        <SubmitSection
          t={t}
          mode={c.mode}
          ...
          posts={c.posts}
          onAddPost={c.addPost}
          onRemovePost={c.removePost}
          onUpdatePost={c.updatePost}
          onFileChange={c.changeFile}
          canSend={c.canSend}
...
function ModeSelector({ t, mode, onSelectMode }: { t: ComicTokens; mode: Mode; onSelectMode: (mode: Mode) => void }) {
  return (
...
function WhatHappensSection({ t }: { t: ComicTokens }) {
  return (
    <section style={cardStyle(t)}>
...
function WhatIsUsedSection({ t }: { t: ComicTokens }) {
  return (
    <section style={cardStyle(t)}>
...
function ConsentSection({
  ...
  askForQuoraUrl: boolean;
  quoraProfileUrl: string;
  onQuoraChange: (value: string) => void;
  thirdPartyNote: string;
  onThirdPartyChange: (value: string) => void;
}) {
  return (
    <section style={{ ...cardStyle(t), borderColor: `${t.ACCENT}55` }}>
      ...
      {askForQuoraUrl ? (
        <QuoraProfileField t={t} value={quoraProfileUrl} allAgreed={allAgreed} onChange={onQuoraChange} />
      ) : null}
...
function SubmitSection({
  ...
  mode: Mode;
  allAgreed: boolean;
  posts: LinkedPostDraft[];
  onAddPost: () => void;
  onRemovePost: (index: number) => void;
  onUpdatePost: (index: number, field: keyof LinkedPostDraft, value: string) => void;
  ...
  return (
    <section style={cardStyle(t)}>
      ...
      {/* Everything in here is disabled until all six consent lines are ticked, so the order is
          ...
          consent is given. */}
      {mode === 'export' ? (
        <ExportFileInput t={t} allAgreed={allAgreed} onFileChange={onFileChange} />
      ) : (
        <PostList
          t={t}
          allAgreed={allAgreed}
      ...
      <SubmitButton t={t} canSend={canSend} submitting={submitting} onSubmit={onSubmit} />
    </section>
  );
}

function ExportFileInput({
  t,
  allAgreed,
  onFileChange,
}: {
  ...
  return (
    <>
      ...
      {posts.length < MAX_LINKED_POSTS ? (
        <AddPostButton t={t} allAgreed={allAgreed} onAdd={onAddPost} />
      ) : null}
...
function PostEditor({
  ...
  t: ComicTokens;
  post: LinkedPostDraft;
  index: number;
  totalPosts: number;
  allAgreed: boolean;
  onRemove: (index: number) => void;
  onUpdate: (index: number, field: keyof LinkedPostDraft, value: string) => void;
}) {
  return (
    <div
      style={{
...
function AddPostButton({ t, allAgreed, onAdd }: { t: ComicTokens; allAgreed: boolean; onAdd: () => void }) {
  return (
    <button
      type="button"
      onClick={onAdd}
...
function ReceiptSection({
  ...
  t: ComicTokens;
  receipt: ContributionSummary;
  unlockLink: string | null;
}) {
  return (
...
function HistorySection({
  ...
  return (
    <section style={cardStyle(t)}>
      ...
      {history.map((item) => (
        <HistoryRow key={item.id} t={t} item={item} onWithdraw={onWithdraw} />
      ))}
    </section>
  );
}
...
function FooterNotes({ t }: { t: ComicTokens }) {
  return (
    <>
