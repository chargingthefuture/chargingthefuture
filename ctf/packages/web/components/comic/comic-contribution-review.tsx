...
export function ComicContributionReview() {
  ...
  async function review(contributionId: string, action: 'accept' | 'decline') {
    ...
    try {
      const res = await fetch(`/api/comic/admin/contributions/${contributionId}/review`, {
        ...
        body: JSON.stringify(
          action === 'accept'
            ? { action, excludedEntryIds: [...(excluded[contributionId] ?? [])] }
            : { action, reason: declineReason[contributionId] ?? '' },
        ),
      });
      const data = (await res.json().catch(() => null)) as
        | { message?: string; promoted?: number; alreadyPresent?: number; grant?: { status: string; amount?: number } }
        | null;
      if (!res.ok) {
      ...
      if (action === 'decline') {
        setNotice('Declined. The contributor sees your reason on their own page.');
      } else {
        // Say plainly what happened to the credits — especially the not-unlocked case, which is a
        // decision rather than a failure and would otherwise look like a silent no-op.
        const grantLine =
          data?.grant?.status === 'granted'
            ? `${data.grant.amount} ServiceCredits granted.`
            : data?.grant?.status === 'skipped_not_unlocked'
              ? 'No credits: this member is not verified yet. The writing is in the library; the grant can be made once they finish Unlock.'
              : data?.grant?.status === 'already_granted'
                ? 'Credits were already granted for this contribution.'
                : 'The credits grant did not go through — retry it by hand.';
        setNotice(
          `Accepted. ${data?.promoted ?? 0} added to the library` +
            (data?.alreadyPresent ? `, ${data.alreadyPresent} already there` : '') +
            `. ${grantLine}`,
        );
      }
      await load();
      ...
      <div style={{ maxWidth: 880, margin: '0 auto', padding: '24px 16px 48px' }}>
        ...
        {loading ? (
          ...
          contributions.map((contribution) => {
            const excludedSet = excluded[contribution.id] ?? new Set<string>();
            const keeping = contribution.entries.length - excludedSet.size;
            return (
              <section key={contribution.id} style={cardStyle(t)}>
                ...
                {contribution.entries.map((entry) => {
                  const isExcluded = excludedSet.has(entry.id);
                  return (
                    <div
                      key={entry.id}
                      style={{
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                        ...
                        <button
                          type="button"
                          onClick={() => toggleExcluded(contribution.id, entry.id)}
                          style={{
                            marginLeft: 'auto',
                            padding: '4px 10px',
                            borderRadius: 8,
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 14 }}>
                  <button
                    type="button"
                    onClick={() => void review(contribution.id, 'accept')}
                    disabled={busyId === contribution.id || keeping === 0}
                    style={{
                      display: 'inline-flex',
                      alignItems: 'center',
                      gap: 6,
                      ...
                      fontSize: 14,
                      fontWeight: 700,
                      cursor: busyId === contribution.id || keeping === 0 ? 'not-allowed' : 'pointer',
                      opacity: keeping === 0 ? 0.5 : 1,
                    }}
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => void review(contribution.id, 'decline')}
                ...
                <input
                  ...
                  value={declineReason[contribution.id] ?? ''}
                  onChange={(event) =>
                    setDeclineReason((current) => ({ ...current, [contribution.id]: event.target.value }))
                  }
...
const bannerStyle = (t: ReturnType<typeof getComicTokens>): React.CSSProperties => ({
  marginTop: 14,
  padding: '10px 14px',
  borderRadius: 10,
  background: t.SURFACE,
...
const noteStyle = (t: ReturnType<typeof getComicTokens>): React.CSSProperties => ({
  marginTop: 8,
  marginBottom: 0,
  padding: '8px 12px',
  borderRadius: 8,
...
const labelStyle = (t: ReturnType<typeof getComicTokens>): React.CSSProperties => ({
  display: 'block',
  marginTop: 12,
  fontSize: 12,
  fontWeight: 600,
