...
const COMMAND_LABELS: Record<string, string> = {
  "comic.contribution.review": "Decided on a contribution",
...
// recorded field is one row here.
const METADATA_PHRASES: ReadonlyArray<(meta: Record<string, unknown>) => string | null> = [
  (meta) => (meta.active === true ? "switched on" : meta.active === false ? "switched off" : null),
...
function describeMetadata(event: AuditEvent): string | null {
  const parts = METADATA_PHRASES.map((phrase) => phrase(event.metadata)).filter((p): p is string => p !== null);
  return parts.length > 0 ? parts.join(" · ") : null;
}

function EventRow({ event, tokens: t }: { event: AuditEvent; tokens: ComicTokens }) {
  ...
  return (
    <div style={{ padding: 10, borderRadius: 9, background: t.SURFACE, border: `1px solid ${t.BORDER_SOLID}` }}>
      ...
      {detail && <div style={{ fontSize: 11, color: t.SUBTLE, marginBottom: 2 }}>{detail}</div>}
...
function EventList({ events, loading, err, tokens: t }: { events: AuditEvent[] | null; loading: boolean; err: string | null; tokens: ComicTokens }) {
...
// Collapsible panel listing every admin decision on the AI Assistant — accepting or declining a
// member's contribution, regenerating or resolving a draft answer, switching a knowledge entry on or
// off — including the ones that were refused and why. Loads lazily on first expand.
//
// This is the check on the reviewer, including the owner. Until 2026-08-28 these decisions were
// written to the server's log and nowhere else, so a member whose contribution was declined left no
// record anyone could read back.
export function ComicAuditPanel({ tokens: t }: { tokens: ComicTokens }) {
  ...
  const loadList = useCallback(async () => {
    ...
    setErr(null);
    try {
      const res = await fetch("/api/comic/admin/audit-events?limit=200");
      if (!res.ok) {
      ...
      {expanded && (
        <div style={{ padding: "0 14px 14px" }}>
          <div style={{ fontSize: 11, color: t.SUBTLE, lineHeight: 1.5, marginBottom: 10 }}>
            Every admin decision on the AI Assistant, newest first — including the ones that were
            refused and why. Most recent 200.
          </div>
