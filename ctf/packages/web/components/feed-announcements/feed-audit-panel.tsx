...
import type { FeedAnnouncementsTokens } from "./feed-announcements-shared";

// One row of feed_admin_audit_trail, as the admin route returns it. Feed and Announcements share one
// trail because they share the audit helper; plugin_id says which surface the action was taken on.
type AuditEvent = {
  id: string;
  actor_id: string;
  plugin_id: string;
...
const COMMAND_LABELS: Record<string, string> = {
  "feed.announcement.draft.create": "Wrote an announcement draft",
  "feed.announcement.draft.update": "Edited an announcement draft",
  "feed.announcement.publish": "Published an announcement",
  "feed.announcement.archive": "Archived an announcement",
  "feed.admin.config.update": "Changed the feed settings",
  "feed.community.moderation.hide": "Hid a Commons post",
  "feed.community.moderation.restore": "Restored a Commons post",
  "feed.question.category.relabel": "Re-labeled a question's category",
  "feed.membership.event.emit": "Recorded a membership change",
};
...
const REASON_LABELS: Record<string, string> = {
  ...
  conflict: "it was already in that state",
};

function EventRow({ event, tokens: t }: { event: AuditEvent; tokens: FeedAnnouncementsTokens }) {
  ...
  return (
    <div style={{ padding: 10, borderRadius: 9, background: t.SURFACE, border: `1px solid ${t.BORDER_SOLID}` }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 3 }}>
        ...
        <span style={{ fontSize: 10, fontWeight: 700, color: t.ACCENT, border: `1px solid ${t.BORDER_STRONG}`, borderRadius: 4, padding: "1px 6px" }}>
          {event.plugin_id}
        </span>
        {failed && (
...
function EventList({ events, loading, err, tokens: t }: { events: AuditEvent[] | null; loading: boolean; err: string | null; tokens: FeedAnnouncementsTokens }) {
...
// Collapsible panel listing every admin action taken on the Feed and on Announcements — writing,
// editing, publishing and archiving an announcement, changing the feed settings, hiding or restoring
// a post, moderating a question — including the ones that were refused and why. Loads lazily on
// first expand.
//
// This is the check on the admin, including the owner. Until 2026-08-28 both surfaces wrote these
// events to the server's log and nowhere else, so nothing an admin did could be read back.
export function FeedAuditPanel({ tokens: t }: { tokens: FeedAnnouncementsTokens }) {
  ...
  const loadList = useCallback(async () => {
    ...
    setErr(null);
    try {
      const res = await fetch("/api/feed/admin/audit-events?limit=200");
      if (!res.ok) {
      ...
      {expanded && (
        <div style={{ padding: "0 14px 14px" }}>
          <div style={{ fontSize: 11, color: t.SUBTLE, lineHeight: 1.5, marginBottom: 10 }}>
            Every admin action on the Feed and on Announcements, newest first — including the ones
            that were refused and why. Most recent 200.
          </div>
