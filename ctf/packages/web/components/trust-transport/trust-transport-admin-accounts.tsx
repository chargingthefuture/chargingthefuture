...
export function TrustTransportAdminAccounts() {
  ...
  async function act(kind: 'restrict' | 'restore') {
    ...
    const res = kind === 'restrict'
      ? await adminMutate(`/api/trust-transport/admin/accounts/${encodeURIComponent(id)}/restrict`, { reason: reason.trim() || null })
      : await adminMutate(`/api/trust-transport/admin/accounts/${encodeURIComponent(id)}/restore`, {});
    if (res.ok) {
