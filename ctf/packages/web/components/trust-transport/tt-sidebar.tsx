...
import { COLOR, type RideType, type TripRequest } from "./tt-shared";
      ...
      <div style={{ padding: "20px 16px 12px" }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {rideTypes.map((rt) => {
            ...
            return (
              <button key={rt.id} type="button" onClick={() => onRideType(rt.id)} style={{ flex: 1, padding: "8px 6px", borderRadius: 10, background: active ? `${rt.color}20` : "rgba(255,255,255,0.04)", border: `1px solid ${active ? rt.color + "50" : "rgba(255,255,255,0.06)"}`, cursor: "pointer", textAlign: "center" }}>
                <Icon size={16} style={{ color: active ? rt.color : "#6B7280", margin: "0 auto 2px" }} />
                <div style={{ fontSize: 10, color: active ? rt.color : "#6B7280", fontWeight: 600 }}>{rt.name}</div>
              </button>
            );
          })}
        </div>
      ...
      <ScrollArea style={{ flex: 1 }}>
        <div style={{ padding: "12px 8px 16px" }}>
          ...
          <div style={{ padding: "6px 10px", fontSize: 12, color: "#6B7280" }}>
            My Requests: <span style={{ color: COLOR, fontWeight: 600 }}>{requests.length}</span>
          </div>
        </div>
      </ScrollArea>
    </aside>
