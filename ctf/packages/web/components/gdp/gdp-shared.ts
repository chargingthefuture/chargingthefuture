...
export const PROJECTED_VALUE_DISCLAIMER =
  "This is what the posts already on the board would add if every one of them closed successfully — open rides and deliveries, quotes waiting on an answer, requests nobody has done yet, recurring activities waiting to be confirmed. Most posts never close, so treat it as interest, not achievement. It is not part of the Community Value Index, and like the index it isn't money, a price, or a redemption value.";
