...
import { formatRentParts, getLighthouseTokens, listingAcceptsCredits, type CurrencyMap, type LighthouseTokens, type Property } from "./shared";

/** "City, State" with blank parts dropped; empty string when neither is set. */
function locationLabel(p: Property): string {
  return [p.city, p.state].filter((s) => s && String(s).trim().length > 0).join(", ");
}

function LocationRow({ label, t }: { label: string; t: LighthouseTokens }) {
  if (!label) return null;
  return (
    <div style={{ fontSize: 12, color: t.MUTED, marginBottom: 8, display: "flex", alignItems: "center", gap: 4 }}><MapPin size={11} /> {label}</div>
  );
}

function BedBathRow({ p, t }: { p: Property; t: LighthouseTokens }) {
  const hasBedrooms = Number.isFinite(p.bedrooms);
  const hasBathrooms = Number.isFinite(p.bathrooms);
...
function RentDisplay({ p, currencies, t }: { p: Property; currencies: CurrencyMap; t: LighthouseTokens }) {
...
function PropertyCard({
  ...
  const heartColor = isSaved ? "#EC4899" : t.FAINT;
  const heartFill = isSaved ? "#EC4899" : "none";
  return (
    <div style={{ borderRadius: 16, background: "rgba(255,255,255,0.02)", border: `1px solid ${t.ACCENT}20`, overflow: "hidden", cursor: "pointer" }}>
      ...
      <div style={{ padding: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
          ...
          <button onClick={() => onToggleSave(p.id)} style={{ background: "none", border: "none", cursor: "pointer", flexShrink: 0 }} aria-label="Save listing">
            <Heart size={16} style={{ color: heartColor }} fill={heartFill} />
          </button>
        </div>
        <LocationRow label={locationLabel(p)} t={t} />
        <BedBathRow p={p} t={t} />
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 8 }}>
          <div style={{ minWidth: 0, flex: 1 }}>
            <RentDisplay p={p} currencies={currencies} t={t} />
    ...
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 14 }}>
        {properties.length === 0 ? (
          ...
          properties.map((p) => (
            <PropertyCard key={p.id} p={p} currencies={currencies} saved={saved} onToggleSave={onToggleSave} onSelect={onSelect} t={t} />
          ))
        )}
      </div>
    </div>
