...
async function maybeAutoGenerateDirectoryProfile(
  ...
  const submissionResult = await client.query<{
    ...
    `
      SELECT id, full_name, bio, quora_profile_url, claimed_professions, skills
      FROM skills_hunt_submissions
      WHERE id = $1::uuid
        AND status = 'accepted'
      LIMIT 1
  ...
  // No generic "Skills Hunt contributor" headline — a nominated profile is shown as a
  ...
  // owner claims it (per the Skills Hunt inventory: community-<hex> namespace).
  ...
  const insertedDirectoryProfile = await client.query<{ id: string }>(
    `
      INSERT INTO directory_profiles
        (claimed_by_user_id, first_name, headline, bio, profile_url, sector_id, job_title_id, is_active, source, invited_by_username, unclaimed_handle)
      VALUES
        (NULL, $1, $2, $3, $4, NULL, NULL, true, 'community-generated', $5, $6)
      RETURNING id
    `,
    [submission.full_name, headline, submission.bio, submission.quora_profile_url, invitedByUsername, unclaimedHandle],
  );
  ...
  // directory_profile_skills joined to skills_taxonomy_skills). Each submitted label is
  // matched to a canonical taxonomy skill by name or alias (case-insensitive); the order
  // submitted is preserved. Free-text "proposed" skills that are not yet in the taxonomy
  // are intentionally left unlinked — an admin promotes those into the taxonomy first.
  const submittedSkills = asStringArray(submission.skills);
