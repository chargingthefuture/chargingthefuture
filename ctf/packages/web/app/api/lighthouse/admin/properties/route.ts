...
export async function GET() {
  ...
  try {
    ...
    reportError(error, { area: 'lighthouse', op: 'admin_properties' });
    return NextResponse.json(
      { ok: false, code: LIGHTHOUSE_ERROR_CODE.persistenceUnavailable, message: `Admin property listing unavailable: ${failureReason(error)}` },
      { status: 503 },
    );
  }
}
