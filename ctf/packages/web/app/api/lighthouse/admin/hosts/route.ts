...
export async function GET() {
  ...
  try {
    ...
    reportError(error, { area: 'lighthouse', op: 'admin_hosts' });
    return NextResponse.json(
      { ok: false, code: LIGHTHOUSE_ERROR_CODE.persistenceUnavailable, message: `Host listing unavailable: ${failureReason(error)}` },
      { status: 503 },
    );
  }
}
