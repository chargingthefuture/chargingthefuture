...
import { z } from 'zod';
import { createCohort, insertSkillUpAudit, listCohorts } from 'lib/skill-up/repository';
...
export async function POST(request: Request) {
  ...
  if (!gate.auth.isAdmin && gate.auth.role !== 'trainer') {
    return NextResponse.json({ ok: false, code: 'skill_up_forbidden', message: 'Trainer or admin role required to create cohorts.' }, { status: 403 });
  }
  ...
  if (!parsed.success) {
    return NextResponse.json({ ok: false, code: 'skill_up_invalid_payload', message: 'Invalid create cohort payload.', issues: parsed.error.issues }, { status: 400 });
  }
  ...
  try {
    ...
    await insertSkillUpAudit({
      actorId: gate.auth.userId,
      command: 'skill-up.cohort.create',
      policyStatus: 'allow',
      reason: 'ok',
      targetType: 'cohort',
    ...
    reportError(error, { area: 'skill-up', op: 'cohorts' });
    return skillUpErrorResponse(error, 'Create cohort unavailable.');
  }
}
