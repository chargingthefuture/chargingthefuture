...
import { z } from 'zod';
import { adminAdjustCredits, insertSkillUpAudit } from 'lib/skill-up/repository';
...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ ok: false, code: 'skill_up_invalid_payload', message: 'Invalid admin credit adjustment payload.', issues: parsed.error.issues }, { status: 400 });
  }
  ...
  try {
    ...
    await insertSkillUpAudit({
      actorId: gate.auth.userId,
      command: 'skill-up.admin.adjust_credits',
      policyStatus: 'allow',
      reason: 'ok',
    ...
    reportError(error, { area: 'skill-up', op: 'admin_adjust_credits' });
    return skillUpErrorResponse(error, 'Admin credit adjustment unavailable.');
  }
}
