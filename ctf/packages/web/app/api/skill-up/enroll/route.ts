...
import { z } from 'zod';
import { enrollInCohort, insertSkillUpAudit } from 'lib/skill-up/repository';
...
export async function POST(request: Request) {
  ...
  if (!gate.auth.isAdmin && gate.auth.role === 'trainer') {
    return NextResponse.json({ ok: false, code: 'skill_up_forbidden', message: 'Trainers cannot enroll in cohorts.' }, { status: 403 });
  }
  ...
  if (!parsed.success) {
    return NextResponse.json({ ok: false, code: 'skill_up_invalid_payload', message: 'Invalid enrollment payload.', issues: parsed.error.issues }, { status: 400 });
  }
  ...
  try {
    ...
    await insertSkillUpAudit({
      actorId: gate.auth.userId,
      command: 'skill-up.enrollment.create',
      policyStatus: 'allow',
      reason: 'ok',
    ...
    reportError(error, { area: 'skill-up', op: 'enroll' });
    return skillUpErrorResponse(error, 'Enrollment unavailable.');
  }
}
