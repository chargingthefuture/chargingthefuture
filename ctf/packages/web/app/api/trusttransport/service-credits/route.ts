...
export async function POST(request: Request) {
  ...
  try {
    ...
    reportError(error, { area: 'trusttransport', op: 'service_credits' });
    return NextResponse.json({ ok: false, code: TRUSTTRANSPORT_ERROR_CODE.persistenceUnavailable, message: 'Unable to send ServiceCredits.' }, { status: 503 });
  }
}
