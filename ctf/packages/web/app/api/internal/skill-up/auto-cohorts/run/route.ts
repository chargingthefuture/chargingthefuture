...
// admin "Refresh proposals" action on the SkillUp admin screen forces a re-read on demand.
...
export async function POST(request: Request) {
  if (!isAuthorized(request)) {
    return NextResponse.json(
      { ok: false, code: 'skill_up_scheduler_forbidden', message: 'Invalid cron secret.' },
      { status: 403 },
    );
  }
  ...
  try {
    ...
    reportError(error, { area: 'skill-up', op: 'internal_auto_cohorts_run' });
    return NextResponse.json(
      { ok: false, code: 'skill_up_scheduler_unavailable', message: `Proposal run unavailable: ${failureReason(error)}` },
      { status: 503 },
    );
  }
}
