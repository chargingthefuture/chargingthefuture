...
import { z } from 'zod';
import { insertLevelUpAudit, transferCreditsForLevelUp } from 'lib/level-up/repository';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await insertLevelUpAudit({
      actorId: gate.auth.userId,
      command: 'level-up.transfer.create',
      policyStatus: 'allow',
      reason: 'ok',
      targetType: 'transfer',
    ...
    reportError(error, { area: 'level-up', op: 'transfers' });
