...
import { claimAutoCohortTrainer } from 'lib/level-up/auto-cohort';
...
export async function POST(request: Request, { params }: RouteProps) {
  ...
  if (!gate.auth.isAdmin && gate.auth.role !== 'trainer') {
    return NextResponse.json(
      { ok: false, code: 'level_up_forbidden', message: 'Trainer or admin role required to claim a cohort.' },
      { status: 403 },
    );
  }
  ...
  try {
    ...
    if (outcome === 'not_found') {
      return NextResponse.json({ ok: false, code: 'level_up_not_found', message: 'No auto-created cohort with that id.' }, { status: 404 });
    }
    if (outcome === 'already_claimed') {
      return NextResponse.json({ ok: false, code: 'level_up_invalid_state', message: 'This cohort already has a trainer.' }, { status: 409 });
    }
    return NextResponse.json({ ok: true, cohortId }, { status: 200 });
  } catch (error) {
    reportError(error, { area: 'level-up', op: 'claim_trainer' });
    return levelUpErrorResponse(error, 'Claim cohort unavailable.');
  }
}
