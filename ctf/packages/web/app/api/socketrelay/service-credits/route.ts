...
export async function POST(request: Request) {
  ...
  try {
    ...
    reportError(error, { area: 'socketrelay', op: 'service_credits' });
    return NextResponse.json({ ok: false, code: SOCKETRELAY_ERROR_CODE.persistenceUnavailable, message: 'Unable to send ServiceCredits.' }, { status: 503 });
  }
}
