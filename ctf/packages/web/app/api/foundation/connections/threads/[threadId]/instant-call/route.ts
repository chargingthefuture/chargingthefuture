...
// Place an instant 1:1 call ring (Foundation "Connect now", issue #808 task 3). The caller is a
...
// Billing (task 4) and push (task 5) are not done here.
export async function POST(request: Request, context: { params: Promise<{ threadId: string }> }) {
  ...
  try {
    const call = await ringInstantCall({ threadId, callerUserId: gate.auth.userId });
    ...
    await insertFoundationAudit({
      ...
      targetType: 'thread',
      targetId: threadId,
      metadata: { callId: call.id, calleeUserId: call.calleeUserId },
    });
