...
4. The note is the whole of what the member writes — there is no category picker in the box. The stored `issue_type` and `suggestion_category` columns are filled with the fixed value "general" on every submission, and `release_surface` with "web"; they carry no member-chosen meaning today.
