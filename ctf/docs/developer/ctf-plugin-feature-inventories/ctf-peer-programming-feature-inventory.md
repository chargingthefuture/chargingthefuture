...
- `GET /api/peer-programming/room` â€” Resolve the caller's current cohort, topic guidance, and tier.
- `POST /api/peer-programming/messages` â€” Create a new top-level message in the caller's cohort.
- `POST /api/peer-programming/messages/[messageId]/replies` â€” Reply to a message thread.
- `POST /api/peer-programming/feedback` â€” Submit structured feedback for the iteration loop.

### Admin Routes

- `GET /api/peer-programming/admin/topics` â€” List weekly topic guidance revisions.
- `PUT /api/peer-programming/admin/topics` â€” Upsert weekly topic guidance for a week key.
- `POST /api/peer-programming/admin/assignments/run` â€” Run the weekly cohort assignment process.
...
1. `peer_programming_weekly_topics` â€” Weekly topic guidance (id, week_start_date, title, guidance, revision_note, status, created_by_user_id, published_by_user_id, published_at).
2. `peer_programming_cohorts` â€” Weekly cohorts (id, week_start_date, cohort_label, fallback_open, topic_id, assigned_by_user_id).
3. `peer_programming_cohort_members` â€” Cohort membership (id, cohort_id, user_id).
4. `peer_programming_messages` â€” Cohort messages with threaded replies (id, cohort_id, author_user_id, parent_message_id, body, tier).
5. `peer_programming_feedback` â€” Structured feedback (id, cohort_id, user_id, issue_type, suggestion_category, release_surface, note).
6. `peer_programming_assignment_notifications` â€” Notification ledger (id, cohort_id, user_id, idempotency_key, payload, delivered_at).
...
- [ ] Contract and integration design documentation. [MANUAL TESTING DEFERRED FOR MVP â€” see Rule 118.]
  - Acceptance criteria:
