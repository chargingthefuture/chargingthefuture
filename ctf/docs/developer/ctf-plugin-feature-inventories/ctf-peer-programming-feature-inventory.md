...
1. The "Session Feedback" box on the Cohorts tab appears only after the member's own cohort has ended — while the cohort is still running there is nothing to look back on, so no box is shown.
2. A member listening in on someone else's cohort never sees the box, even if that cohort has ended; it is only for reviewing your own cohort.
3. When the member's cohort has ended, the box reads "Your cohort has ended. Tell us how it went." and takes a free-text note.
4. The note is all of what the member writes — there is no category picker in the box. The stored `issue_type` and `suggestion_category` columns are filled with the fixed value "general" on every submission, and `release_surface` with "web"; they carry no member-chosen meaning today.
5. Feedback records are retained for iteration analytics and audit.
...
4. Admins can **end (close) a cohort** — the "End cohort" action on each active, non-standing cohort in the admin Cohorts list. An ended cohort's Direct Line becomes read-only (members keep reading; no one can post), and it drops out of the member "running cohorts" list while staying visible to admins with an "Ended" badge. The standing Cohort 1 cannot be ended.

### Member Feedback Inbox

1. Admins read member feedback (author, time, note) newest first in the admin page's "Member feedback" panel. The row shows no category labels: the member form posts the fixed value "general" for both `issue_type` and `suggestion_category`, so printing them put "general general" on every row and told the admin nothing.
...
- `POST /api/peer-programming/messages` — Create a new top-level message. The caller must be a member of the target cohort; non-members are denied and the tier is set server-side (never trusted from the request body). After the message is stored it emits a best-effort notifications-center notification (`peer-programming.cohort.message`, category `community`) to every **other** cohort member via `notifySafe`, deep-linked to the room (`/apps/peer-programming?cohortId=<id>`) and deduped per (member, message) — so a cohort message surfaces in the 🔔 notifications center and (for members who opted the Community category into device push) pings their device. The sender is never notified; a notification failure never blocks the post.
...
- `POST /api/peer-programming/goals/tasks/[taskId]` — Body `{ action, result? }`. A member of the goal's cohort other than its owner may `take` an open task (or one held past 24 hours unfinished), and the holder may `release` it or `finish` it with a `result` (up to 1000 characters). The goal owner may `keep` or `send_back` a finished result, or `remove` a task nobody has finished. Each action is one conditional update, so two members pressing at once cannot both succeed; the loser gets 409 `peer_programming_task_unavailable` with what happened. `finish` notifies the goal owner. Audited as `peer-programming.goal.task.<action>`.
...
- `POST /api/peer-programming/admin/cohorts/end` — Admin-only, CSRF-guarded: end (close) a cohort. Body `{ cohortId }`. Sets the cohort's `status = 'ended'` and stamps `ended_at` / `ended_by_user_id`, freezing its Direct Line — the message and reply routes then reject posting to it (409 `peer_programming_cohort_ended`). The single standing Cohort 1 cannot be ended (403 `policy_denied`); an unknown id is 404. Idempotent (ending an already-ended cohort returns it unchanged). Audited as `peer-programming.cohort.end`. Backs the admin "End cohort" button. Command `admin.cohort.end` (see contracts).
...
The sign-in record is the entire definition of "active" here, as it is on the Weekly
...
2. `peer_programming_cohorts` — Weekly cohorts (id, week_start_date, cohort_label, fallback_open, topic_id, assigned_by_user_id, is_standing, status, ended_at, ended_by_user_id). `is_standing` (BOOLEAN NOT NULL DEFAULT FALSE) marks the single standing, always-open Cohort 1 used in low-population mode (see "Single standing, always-open Cohort 1 mode" below); a partial-unique index `uq_peer_programming_cohorts_standing ON (is_standing) WHERE is_standing` guarantees at most one standing row. The standing row persists across weeks and is found by `is_standing = TRUE`, not by the current week. Ordinary weekly cohorts keep `is_standing = FALSE`. **Lifecycle:** `status TEXT NOT NULL DEFAULT 'active' CHECK (status IN ('active','ended'))` plus `ended_at TIMESTAMPTZ` and `ended_by_user_id TEXT` — an `ended` cohort is closed and read-only (the message/reply routes reject posting to it; see the admin "End cohort" action). The standing Cohort 1 can never be ended. Additive columns; every existing row backfills to `active`, and nothing is ever `ended` until an admin ends a cohort. (This is the first, non-destructive slice of the cohort-end policy in issue #1361 — the auto-end-at-week-end job and the message-retention cleanup are still deferred.)
...
4. `peer_programming_messages` — Cohort messages with threaded replies (id, cohort_id, author_user_id, parent_message_id, body, tier). Index `idx_peer_programming_messages_cohort_created ON (cohort_id, created_at)` backs the room read (`listMessages`: `WHERE cohort_id = $1 ORDER BY created_at ASC LIMIT 300`); without it that read is a sequential scan + sort over the entire table, which under single standing Cohort 1 mode (all members' messages in one ever-growing cohort) eventually exceeds the DB statement timeout and fails the room load.
...
10. `peer_programming_goal_tasks` — Tasks on a goal (id, goal_id → `peer_programming_goals(id) ON DELETE CASCADE`, description, taken_by_user_id, taken_at, result, finished_at, kept_at, created_at, updated_at). A task is open when `taken_by_user_id` is null or `taken_at` is more than 24 hours old with no `finished_at`; taken while held; finished once `finished_at` is set; kept once `kept_at` is set. Sending back clears the helper, the result and `finished_at`. Indexes `idx_peer_programming_goal_tasks_goal ON (goal_id, created_at)` and a partial `idx_peer_programming_goal_tasks_finished ON (finished_at) WHERE finished_at IS NOT NULL` for the 24-hour count. On account deletion a helper's id is pseudonymized (`pseudo` on `taken_by_user_id`) and the result stays with the goal; the board then shows "A former member".
...
7. **The goal board is written server-side by role, never by client claim.** Goal changes require the caller to be the goal's owner; taking, releasing and finishing a task require membership of the goal's cohort and exclude the owner; keeping, sending back and removing require the owner. Each is a single conditional update, and ids that are not UUIDs are answered 404 before any query. An ended cohort's board rejects every write with 409.
...
- 2026-08-18: **Intent and Outcome rewritten in plain words — it ships to the public user guide
  ...
  "tiered participation", "structured feedback") replaced by what they actually describe. The entire
...
- 2026-07-23: **Cohort message notifications + dated message stamps.** Two fixes to a missed-message report. (1) A new cohort Direct Line message now notifies every other cohort member via the shared notifications center: `POST /api/peer-programming/messages` emits `peer-programming.cohort.message` (category `community`) through `notifySafe` after the write — best-effort, deduped per (member, message), never the sender, deep-linked to the room. PeerProgramming previously had **no** notifications-center producer, so cohort messages went unseen until someone opened the room; the "Community" device-push toggle already advertised PeerProgramming but nothing fed it. (2) The Direct Line message stamp (`pp-chat-tab.tsx`) now shows the date **and** time (`Mon D, h:mm AM/PM`) instead of time alone, so messages from different days can be told apart. No schema or contract change (the message route already existed); the notification reuses the existing notifications backbone.
...
- 2026-07-21: **Cohort-end policy — first non-destructive slice (issue #1361).** Cohorts can now be ended (closed) and their Direct Line frozen. This ships the safe, verifiable parts of the owner-approved cohort-end spec now; the auto-end-at-week-end job and the ~90-day message-retention cleanup remain deferred until weekly auto-generated cohorts are enabled. Changes: (1) **Schema** — added `status TEXT NOT NULL DEFAULT 'active' CHECK (status IN ('active','ended'))`, `ended_at TIMESTAMPTZ`, and `ended_by_user_id TEXT` to `peer_programming_cohorts` (additive; every existing row backfills to `active`; regenerated `schema.demo.sql`). (2) **Repository** — `endCohort({cohortId, actorId})` marks a cohort ended (rejects the standing cohort; idempotent), `isCohortEnded(cohortId)` for the write-gate, and `listActiveCohorts` now excludes ended cohorts from the running/listen-in list; the cohort model carries `status`/`endedAtIso`. (3) **Write-gate** — `POST /messages` and `/replies` reject an ended cohort with 409 `peer_programming_cohort_ended` (audited `cohort_ended`), even for a member, closing the "post into a closed cohort via a leftover link" hole. (4) **Admin action** — new `POST /api/peer-programming/admin/cohorts/end` (admin + CSRF, audited `peer-programming.cohort.end`) and an "End cohort" button (with confirm) on each active, non-standing cohort in the admin Cohorts list; ended cohorts show an "Ended" badge. (5) **Member UI** — the room response returns `ended`, and the Direct Line shows "This cohort has ended — the conversation is read-only" with no composer. (6) **Contracts** — added the `admin.cohort.end` command, access-policy, and audit-event entries. Web-only (PeerProgramming is web-only per rule 105 / PR #1742). Deferred, per the spec: auto-end scheduler, retention/delete job, and a member "Past cohorts" view.
