...
- `GET /api/peer-programming/admin/cohorts` — Admin-only: every cohort formed for the current week with live member counts and the fallback-open flag. Backs the admin "Active cohorts" list so an admin can reach and manage any cohort after it forms (the member-scoped `/room` returns only the admin's own cohort).
