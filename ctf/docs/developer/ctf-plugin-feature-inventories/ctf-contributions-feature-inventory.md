...
- Submit a contribution claim of one of three kinds:
  - **Gift card** (`amazon`, `apple`, or `dennys`): the member states the amount — an integer number of
...
- Confirm or reject each claim, exactly once:
  ...
  - Credits = confirmed amount x `credits_per_usd`, clamped by the per-user-per-cycle cap, then
    rounded to an integer; a positive grant goes through the canonical service-credits mint with
    idempotency key `contribution-<submissionId>`. A grant clamped or rounded to 0 still confirms
    with `credits_granted = 0`. The rounding is what keeps credits entire: `credits_per_usd` is an
...
- Table: `contributions_submissions` — contribution claims.
  ...
  - `claimed_amount_usd NUMERIC` (gift_card only; validated as an integer, 1 to 500 — the column
...
- 2026-08-09: **Gift-card claims are integer dollars, $1 to $500, and credit grants are rounded**
  ...
  - `GIFT_CARD_MIN_USD` moves from 0 to 1 and `isValidGiftCardAmount` now requires an integer, so
  ...
  - `computeCycleCappedGrant` rounds the grant after the cap clamp (rounded, not truncated, so the
    ...
    cap). This — not the integer-dollar rule — is what actually guarantees integer credits: the
  ...
  as they are. New test case CONT-3b covers the integer-dollar rule and the rounding.
