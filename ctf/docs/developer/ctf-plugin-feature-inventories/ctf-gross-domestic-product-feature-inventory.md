...
Implemented user routes (live dashboard):

- `GET /api/gdp/report/current` — live dashboard report (see §4.2 item 6 for the payload).
- `GET /api/gdp/countries` — real per-country member distribution for the dashboard's "Top Countries" panel. Returns `{ ok, countries: [{ country, members }], totalMembers }` from `listMemberCountsByCountry` (`lib/gdp/repository.ts`), which counts claimed, active `directory_profiles` rows that have a `country` set, most members first. This is "location tied to people" — a people-count per country read from the shared member profile, never an invented per-country money figure. No small-count suppression (owner decision, 2026-07-11): every country with a member is shown. Behind the same `requireGdpReadAccess` gate as the report.
