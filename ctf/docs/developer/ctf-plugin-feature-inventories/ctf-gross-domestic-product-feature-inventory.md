...
- `GET /api/gdp/report/current` — authenticated read. Computes the dashboard **live** on each
  ...
  snapshot step.
- `GET /api/gdp/countries` — real per-country member distribution for the dashboard's "Top Countries" panel. Returns `{ ok, countries: [{ country, members }], totalMembers }` from `listMemberCountsByCountry` (`lib/gdp/repository.ts`), which counts active `directory_profiles` rows that have a `country` set (claimed or not — the same member population as the dashboard's total member count), most members first. This is "location tied to people" — a people-count per country read from the shared member profile, never an invented per-country money figure. No small-count suppression (owner decision, 2026-07-11): every country with a member is shown. Behind the same `requireGdpReadAccess` gate as the report.
