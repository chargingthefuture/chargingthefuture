...
5. Recognition pipeline (issue #121 follow-up): actual recognized volume is rolled up by an extensible source registry in `ctf/packages/web/lib/gdp/recognition.ts` (`recognizeGdpVolume`) and the `scripts/recognizeGdp.mjs` job (`pnpm gdp:recognize`), which sum each registered plugin's eligible settled spend by currency ALONGSIDE the projection target — they do not replace it. **Two figures are produced and shown side by side.** (i) `gdp_recognized_volume_usd` (`is_estimate=TRUE`): fiat/crypto volume normalized to USD via `currency_usd_rates`. (ii) `gdp_recognized_volume_sc` (`is_estimate=FALSE`): ServiceCredits-denominated volume reported in SC units and **never converted to USD** — ServiceCredits is a non-redeemable utility token with no fiat peg, so it carries no USD rate (the `SC` row is intentionally absent from `currency_usd_rates`, and the normalizer skips `SC`). Both render together on the GDP map overlay (web `gdp-world-map.tsx`, Android `Gdp.tsx`) so neither figure alone understates community self-sufficiency. Registered sources (owner-approved, one at a time): (a) **TrustTransport** completed-task earnings (`trusttransport_earnings_ledger`, `credit`/`release` entries) → USD; (b) **LevelUp** trainer payouts for validated mentorship work — governed mint grants with reason `levelup_trainer_split` in `service_credits_governance_events`, ServiceCredits → the SC figure. LevelUp recognition is the trainer-split slice only; learner-side amounts (escrow returns, completion bonuses, stipends, microgrants) are excluded as incentives/returns, not spend, and the SC ledger is not read directly because its mint-grant entries are tagged `accounting_scope = service_credits_non_gdp`. Other priced plugins (LightHouse rent, SocketRelay favors, Foundation quotes) are **not yet** registered because they do not currently record an on-platform settled-spend amount. Once they adopt the shared multi-currency selector and record a settled chosen-currency amount (issue #420), each registers as a source: fiat/crypto volume into `gdp_recognized_volume_usd`, ServiceCredits volume into `gdp_recognized_volume_sc`. A fiat/crypto currency with no active rate is surfaced and excluded, never silently treated as zero.
...
- 2026-06-11: Registered **LevelUp trainer payouts** as a recognition source and split ServiceCredits out of the USD figure (owner-approved). (1) Source: `lib/gdp/recognition.ts` adds `levelUpTrainerPayoutSource` to `RECOGNITION_SOURCES` and `scripts/recognizeGdp.mjs` adds the matching query — sum of governed mint grants with reason `levelup_trainer_split` from `service_credits_governance_events`, denominated in ServiceCredits. Trainer-split slice only; learner-side amounts (escrow returns, completion bonuses, stipends, microgrants) are excluded as incentives/returns. Reads governance events, not the SC ledger (whose mint-grant entries are tagged `accounting_scope = service_credits_non_gdp`). (2) ServiceCredits is now **never converted to USD**: removed the `SC` row from `seedCurrencyUsdRates.mjs` and made the recognizer skip `SC`. `recognizeGdpVolume` (renamed from `recognizeGdpVolumeUsd`) returns a USD estimate (fiat/crypto only) plus a separate `serviceCreditsVolume`; the job writes a new `gdp_recognized_volume_sc` metric (`is_estimate=FALSE`, registered in `canonical_metrics.yaml`) alongside `gdp_recognized_volume_usd`. (3) UI: the GDP map overlay (web `gdp-world-map.tsx`/`gdp-map.tsx`, Android `Gdp.tsx`) now shows the ServiceCredits activity beside the USD figure, in SC units with an honest caption that it is not converted to dollars — so the two together reflect the full community economy. Seeded a demo `gdp_recognized_volume_sc` value in `seedGdp.mjs`. LightHouse rent, SocketRelay favors, and Foundation quotes are not yet registered — they do not currently record an on-platform settled amount; they become eligible once they adopt the shared multi-currency selector and settle a chosen-currency amount on-platform (issue #420). No schema change; reuses `gdp_metric_snapshots`.
