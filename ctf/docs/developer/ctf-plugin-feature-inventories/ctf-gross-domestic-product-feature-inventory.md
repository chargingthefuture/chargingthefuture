...
- 2026-08-29: **The SocketRelay lines in both value figures now say "requests", not "favors"**
  ...
  the SocketRelay chat's ongoing-arrangement prompt to "a request like this one". The same pass drops
  the word "still" from every line on that card (owner directive): the source labels are now
  "TrustTransport open requests" and "LightHouse homes available", the per-app count line reads
  "N open posts", and the explanatory sentence opens with "open rides and deliveries". Wording only: no
