...
5. Recognition pipeline (issue #121 follow-up): actual recognized volume is rolled up by an extensible source registry in `ctf/packages/web/lib/gdp/recognition.ts` (and the `scripts/recognizeGdp.mjs` job, `pnpm gdp:recognize`), which sums each registered plugin's eligible settled spend by currency, converts via `currency_usd_rates`, and writes the `gdp_recognized_volume_usd` metric (`is_estimate=TRUE`) ALONGSIDE the projection target — it does not replace it. The first registered source is TrustTransport completed-task earnings (`credit`/`release` ledger entries); LightHouse, LevelUp, and other eligible-spend sources register as the owner approves them. A currency with no active rate is surfaced and excluded, never silently treated as zero.
