...
3. **Audit log panel** at the bottom of `/admin/feed-announcements`, shared with the Feed. Every admin action on either surface, newest first, with a chip per row saying which one it came from — including the actions that were refused, and why. Reads `GET /api/feed/admin/audit-events?limit=200`. The rows live in `feed_admin_audit_trail`, documented in the Feed inventory §4.2, because Announcements and the Feed share `lib/feed/audit.ts` and therefore share one trail.
...
4. Allow/deny audit events for command execution and admin actions. **Every admin mutation writes a durable row to `feed_admin_audit_trail`** via `recordFeedAdminAudit`, which also emits the observability line — on success, on a refusal, and on a persistence failure. Before 2026-08-28 `logFeedAudit` wrote the event to the server's log alone, so nothing an admin did could be read back. The member-facing commands (reads, reactions, replies, dismissals) still emit `logFeedAudit` alone: a row per interaction would be volume, not accountability.
...
- 2026-08-28: **Every admin action on Announcements is recorded in a table an admin can read.** Owner directive: every admin action is recorded, on every surface, from the day the surface ships. `lib/feed/audit.ts`, which Announcements shares with the Feed, built the entire contract-shaped event and ended in `console.info` — a line in the server's log, which nothing can query, no screen can show, and which ages out of the host's retention window. One fix covers both surfaces because they share the helper: new table `feed_admin_audit_trail` with a `plugin_id` column saying which surface an action came from (documented in the Feed inventory §4.2), and `recordFeedAdminAudit` writing the row as well as the line. The five Announcements admin mutation routes — draft create and update, publish, archive, and the membership-event emitter — now record every outcome, refusals included. The new **Audit log** panel at the bottom of `/admin/feed-announcements` reads them back. `POST /api/announcements/admin/targeting/validate` is deliberately **not** audited: it validates a targeting expression with a pure function, reads no table and writes none, so there is no admin action there to record. Full detail in the Feed inventory change log for the same date.
