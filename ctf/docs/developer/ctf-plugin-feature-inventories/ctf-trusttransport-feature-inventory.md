...
- `POST /api/trusttransport/trips/:tripId/chat` — Mint Stream credentials for the trip thread: chat channel (`channelId`/`streamChannelId`), participant token, and the video `callId` for the trip's 1:1 video room.
...
- 2026-06-12: Made the trip chat route issue a video call id so the Android trip "stream" tab's video room works. `POST /api/trusttransport/trips/:tripId/chat` now returns `callId` (`trusttransport-trip-:tripId`, the Stream Video room for the trip's two participants) alongside the existing chat channel credentials; it also returns `streamChannelId` so the web chat tab connects to the real channel instead of falling back to the raw trip id. On Android, `fetchTrustTransportStreamCredentials` now uses the platform base URL (a relative `/api/...` URL never resolves in React Native) and maps the canonical `stream*` fields, and `TrustTransportStreamTab` joins the video call correctly (`call('default', callId)` + `join`, mirroring the Chyme video pattern) instead of passing an unjoined call with no id. No schema change — chat and video share the same per-trip id across Stream's separate chat/video namespaces.
