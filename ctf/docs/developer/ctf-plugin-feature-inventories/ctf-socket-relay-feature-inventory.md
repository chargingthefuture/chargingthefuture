...
4. Admin request removal is transactional: because these tables have no FK cascade, the delete clears the request plus its fulfillments, participants, and request-events itself (no orphans), retains `socket_relay_messages` as moderation evidence (rule 100), and writes the `socket-relay.admin.request.delete` audit row **in the same transaction** — the removal never commits without its audit record (if the audit insert fails, the entire delete rolls back). See the deletion contract.
...
- 2026-08-29: **The Direct Line's ongoing-arrangement prompt now says "a request like this one".**
  ...
  for a post than the board does. The stored sector value on the recurring arrangement is still
  `favor` — that is the Recurring Activity plugin's own taxonomy, not SocketRelay's word for a post,
  and it is unchanged. Copy only; the same change renames the two SocketRelay lines in the community
  value figures (see the Gross Domestic Product inventory).
