...
- 2026-05-31: Brought the three contract files into the current per-entry shape so they pass the schema-drift gate's contract validator. The command, access-policy, and audit contracts predated that gate and still used the older single `id:` style; the gate only validates contract files that change in a pull request, so this latent mismatch surfaced when a sibling plugin's contracts were re-validated. Each entry now carries `pluginId: clicklog` and `version: 1.0.0`; the command file's `id:` became `command:`; the access-policy file keeps `requiredRoles`; and the audit file keeps its existing `eventId`. No behaviour, schema, route, or API change — documentation/contract shape only.
