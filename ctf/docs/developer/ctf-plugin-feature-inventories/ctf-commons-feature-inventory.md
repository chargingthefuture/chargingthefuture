# Commons Feature Inventory (CTF Rewrite)
...
>
> 1. Wire the Hub home channel to the Feed backend — replace stubbed `/api/commons/messages`
...
- The Commons owns the unified home/landing experience: app shell, the single blended
  `community` channel, live hero stats, and the plugin grid.
- Data layer: the Commons channel is backed by the Feed model (`feed_items` + `lib/feed/inference.ts`)
  as the single source of truth — see the Consolidation Decision above. The Commons does not own a
...
9. The phone-width top bar switches sections with two icon buttons — a speech-bubbles icon for the Commons and a grid icon for Apps — the same 38px square as the admin, help and settings buttons beside them, so the entire bar is one row of equal boxes. Each button names itself for screen readers and on hover ("Commons", "Apps"). Once a member is there, the page says which one it is: the Apps page heads "All Apps", and the Commons channel row starts with the word "Commons" ahead of the `#general` chip.
10. **A member not yet approved sees the Commons stripped back to the verification banner and the chat (owner decision, 2026-09-26).** For a signed-in member who is not an admin and has not been approved through Unlock (the home page passes a non-null `verification`): the Apps section is gone (its top-bar tab and icon-rail button, and with it the app grid, sort and search), the plugin links under @comic answers and under announcements are hidden, the locked `#weavers` contributor chip is hidden, and the suggestion chips under the chat are only "I can’t find my Quora profile URL" (asks @comic) and "I'm new. What do I do first?". Every one of those led to something closed to them until approval and together they buried the one thing they need to do. It all returns on the first page load after approval. An approved member, an admin, and a signed-out visitor see no change. Implemented with a React context (`components/community-shell/commons-unlock-focus.ts`) set by `CommunityShell`. This reverses the earlier "nothing is hidden here" choice, for this group only.

### Commons Chat (the blended `community` channel)
...
3. One blended stream interleaving admin-only announcements, AI Q&A answers, and peer-to-peer community posts. History loaded via `GET /api/commons/messages` (backed by `listFeedTimeline` over `feed_items`), polled while the shell is mounted.
4. Sending from the input creates a peer-to-peer community post via `POST /api/commons/messages` (backed by `createFeedCommunityPost`, CSRF-guarded); dedup on display by `(from, sender, text, time)` tuple.
...
6. One-tap suggestion chips render persistently above the composer (whether or not the chat already has messages). Each chip's behavior is explicit (`lib/concierge/commons-suggestions.ts`), so a tap always does the right thing and never merely pre-fills the composer (#471): a **navigate** chip ("Show housing options" → LightHouse, "Open the provider directory" → Foundation, "Browse the skills directory" → Directory, "Check my Service Credits" → ServiceCredits) opens that plugin directly (`/apps/<slug>`) — it is an action, not a question; an **ask** chip ("What is the GDP tracker showing this week?") routes the question to the `@comic` AI assistant via `askComic`, which prepends the `@comic` mention (the server only routes a mentioned body) and shows the "Reviewing for safety" pending card immediately, then the human-approved answer when it is ready; an **answer** chip ("I'm new. What do I do first?", first in the row) carries a fixed reply written in the chip itself and shows it in the chat at once through `answerChip` — the member's question as their own message, then the reply with one button per place it names (Unlock, Fireside, the Knowledge Library, the Directory, and the blog post "New here? Three things to do first") — with no AI draft and no review queue, because a member who has just signed in should not wait on a person to be told where to start (owner decision, 2026-09-25). The reply and the post say the same three things: send the Quora profile address on Unlock, write while waiting (Fireside or the Knowledge Library), then put skills on the Directory once approved. Buttons under a bubble come from `ChatMessage.actions` (`{ label, href }[]`), which only the client's own local replies set — the concierge's single "Open X →" button uses the same field — so a stored message can never carry one. The local keyword concierge (`lib/concierge/resolver`, `sendConciergeAsk`) remains available for free-text asks but no longer backs the visible chip row.
7. Announcements render as official items on their own card, signed with the operator's name (Farah, `OFFICIAL_SENDER_LABEL` in `lib/commons/constants.ts`) and marked official by the shield badge — the name says who wrote it, the badge says it is official. Community posts render with their author. Every avatar glyph, official or peer, is the first letter of that sender's name; there is no fixed house glyph.
...
11. Unread divider: on entry the chat reads the member's `feed_commons_last_seen` marker (`GET /api/commons/last-seen`) and draws a single "New messages" divider before the first stream entry newer than it (none if everything is already seen). After the member has viewed the chat it marks seen once (`POST /api/commons/last-seen`, debounced/best-effort). Placement uses the per-entry `epoch` already computed for the unified stream.
12. Emoji reactions: under each peer bubble a compact reaction row shows any emoji with at least one reaction as a pill (emoji + count, highlighted when the member reacted), plus a small "add reaction" affordance that reveals a fixed quick set (👍 ❤️ 😂 🎉 🙏 😢 👋 💯) to pick from. Tapping a pill or a picker emoji toggles the member's reaction (`POST /api/commons/messages/:postId/reactions`), flips it optimistically, and reconciles via the existing 10s poll. Reactions are stored in our own database (`feed_community_post_reactions`), not Stream — the first feature of the Stream-adoption initiative ("approach b").
13. Live Stream layer (the real-time foundation, kept beneath the custom Commons design — task 1 of `STREAM_FEATURE_ADOPTION.md`): on entry the shell calls `POST /api/commons/join`, which now mints real Stream credentials for the shared `ctf-feed-community` channel (`getFeedStreamCredentials(userId, displayName, 'community')`). The client opens a `stream-chat` connection to that channel and subscribes to events. A `message.new` event (or a recovered connection) triggers the existing `refreshHistory()` so new posts appear immediately instead of waiting for the poll; the 10s poll stays as a backstop but slows to 30s while the live connection is healthy. The connection is disconnected on unmount. Every part of this is best-effort: when Stream is not configured (`POST /api/commons/join` returns `{ ok: true, configured: false }`) or any live step fails, the client silently stays on the 10s poll and the chat is fully functional — the live layer never breaks Commons.
14. Pictures from admins (owner decision, 2026-09-25): an admin sees a "Share a picture (admins)" button above the message box. It opens a panel with a file picker, a preview, a required "What it shows" description (read aloud to members using a screen reader), and the message to go with it. Share posts both as one message. Every member then sees the picture in that message, above its text; tapping it opens the full-size copy in an in-app lightbox (dismissed by the backdrop, the X, or Escape — not a `target="_blank"` link, since the installed app has no browser chrome to navigate back with). Signed-out visitors see it too, in the public Commons, while public viewing is on — a screenshot only members could see would not replace one Quora erased with the account. Members cannot attach pictures. The admin author can fix a typo in the caption afterwards with the same "Edit" control every peer post shows (bug fix, 2026-09-26) — sending the edit rewrites the text in place rather than deleting and reposting, so the picture, its reactions, and its replies survive. The owner uses this to explain the app with screenshots, now that Quora erases the accounts those were shared from.
...
15. Delete your own post: each of the member's own peer bubbles carries a small "Delete" affordance (a trash action beside "Reply"; web confirms with a dialog, Android with a destructive `Alert`). Confirming calls `DELETE /api/commons/messages/:postId`, optimistically drops the post from the stream, and restores it on failure. There is deliberately **no edit** — to change a post a member deletes it and posts again, so a corrected message is a fresh row with its own moderation and no inherited reactions/replies (the anti-fraud choice over in-place edit; owner decision 2026-07-16). Author-only; the server rejects deleting anyone else's post.

### Sidebar — Channels (deferred)

- The MVP is a single blended `community` channel; there is no `hub_channels` table and no multi-channel sidebar. Splitting into multiple Commons channels is a possible future option pending feedback (see Gaps). Stubbed `GET /api/commons/channels` returns a single `community` channel.

### Sidebar — Direct Messages (deferred)

- Direct messages are out of scope for the consolidated MVP — peer-to-peer interaction happens only in the public blended channel (intentional, for soft moderation and marketing visibility). There is no `GET /api/commons/dms` route and no `hub_dm_threads` table.

### Sidebar — Bots (deferred)

- There is no `hub_bots` / `hub_bot_routes` system-bot entity in the MVP. The assistant capability is the Feed AI Q&A — now built as the `comic` subsystem (`@comic` mention; user-facing label "AI Assistant"), specified in `ctf-comic-feature-inventory.md` (the source of truth for the AI assistant). `GET /api/commons/bots` used to answer here with a hardcoded empty list that nothing read; it was removed on 2026-08-03 rather than left standing as a capability that did not exist.

### Commons Apps (Apps Section)
...
- `POST /api/commons/messages` — create a peer-to-peer community post, backed by `createFeedCommunityPost` (CSRF-guarded; rate-limit + moderation honored). Body accepts an optional `replyToPostId` (the quoted post's id) plus a display-only `quotedMessage` echoed back on the created message so the sender's optimistic copy renders the quote. An unknown/malformed `replyToPostId` is rejected with 400.
- `POST /api/commons/images` — admins only (owner decision, 2026-09-25): post a message with a picture. Commons access gate, then `isAdmin` (else 403 "Only admins can share pictures in the Commons."), then `x-ctf-csrf: '1'`. Multipart form: `image` (at most 3 MB; the type is read from the file's first bytes and must be PNG, JPEG or WebP), `alt` (required, at most 500 characters, what the picture shows), `text` (the message, required, admin length cap), `width` / `height` (1–4096, from the browser that scaled it). The web client scales the picture to at most 1600 px on its long side and re-encodes it to WebP (JPEG where the browser cannot write WebP) before sending, which also drops the location and camera data a phone writes into a photo. Calls `createFeedCommunityPost` with `input.image`, which inserts the post and its `feed_community_post_images` row in one transaction (post rate limit and moderation still apply). Returns 201 `{ ok, message }` in the same shape as `POST /api/commons/messages`, with `message.image` set. There is no other way to attach a picture: `POST /api/commons/messages` never passes one.
- `GET /api/commons/images/:postId` — serve the picture on a Commons post. `postId` must be a UUID (else 400). A member who passes the Commons access gate gets any picture whose post's `moderation_status` is `accepted`. Anyone else, signed out included, gets it only while public viewing is on (`isPublicCommunityReadOn`: `feed_render_config.is_public` and the community channel enabled — the rule `GET /api/feed/public/community` uses), under the same per-IP public read limit (key `commons-image`), and only for a post that public list would show (timeline item active, published, not expired, addressed to member/admin/all). With public viewing off, a signed-out request gets the gate's 401. A picture that does not qualify is 404. Headers: the stored `Content-Type`, `X-Content-Type-Options: nosniff`, `Content-Disposition: inline`, `Cache-Control: private, max-age=86400` (no shared cache, so a deleted post's picture is not served from somebody else's copy).
- `DELETE /api/commons/messages/:postId` — delete the requesting member's own community (peer) post (hub access gate + `x-ctf-csrf: '1'`). The `postId` path segment must be a well-formed UUID, else 400 before the repository is called. Author-only: backed by `deleteCommunityPost`, which verifies the caller owns the post (else 403), then hard-deletes it — cascading its replies + reactions and removing the projected `feed_items` row (with its targets, read state, and dismissals). Returns `{ ok, postId }`; 404 if the post is gone. A text-only post has no edit endpoint by design: a member corrects it by deleting and reposting, so a corrected message is a fresh row with its own moderation and no inherited reactions/replies (closes the bait-and-switch edit vector). The one exception is a picture post — see `PATCH` below.
- `PATCH /api/commons/messages/:postId` — rewrite the text of the requesting member's own Commons **picture** post in place (bug fix, 2026-09-26: the Edit control disappeared for a post with a picture, since delete-and-repost would lose it). Hub access gate + `x-ctf-csrf: '1'`; `postId` must be a well-formed UUID, else 400. Body `{ text }`, validated against the same length/URL caps `POST /api/commons/messages` uses. Backed by `editCommunityPost`: author-only (403 otherwise), the post must currently carry a row in `feed_community_post_images` (else 403 — a text-only post has no PATCH path, since only an admin can attach a picture and the bait-and-switch concern the DELETE-and-repost model exists for does not apply to admin content), the post must not be moderator-hidden (else 403 — a moderator's takedown cannot be edited back into view), and the new text passes the same content-moderation check a fresh post would (else 422). On success, updates `feed_community_posts.body` and `edited_at`, syncs the projected `feed_items` row, and returns `{ ok, postId, body, editedAtIso }`. The picture, its reactions, and its replies are untouched.
- `GET /api/commons/last-seen` — read the member's last-seen marker for the Commons home channel (`{ ok, lastSeenAtIso }`); `lastSeenAtIso` is null when never recorded. Backed by `getCommonsLastSeen` over `feed_commons_last_seen`. Best-effort.
- `POST /api/commons/last-seen` — move the member's last-seen marker to now (CSRF-guarded; optional `seenAtIso` clamped to server NOW() and never moved backwards). Backed by `updateCommonsLastSeen`. Best-effort.
- `GET /api/commons/channels` — the channel list the Commons shell reads. Returns `#general` for everyone,
...
- `POST /api/commons/join` — mints the live Stream credentials for the Commons home shell. Backed by `getFeedStreamCredentials(userId, displayName, 'community')` (the shared `ctf-feed-community` channel). Returns `{ ok: true, configured: true, streamApiKey, streamChannelId, streamUserId, streamToken }` when Stream is configured, or `{ ok: true, configured: false }` when it is not (no API key/secret) so the client stays on polling. Commons access gate enforced. Previously returned hardcoded stub credentials (`'todo-stream-token'`); the stub is removed.
...
- `GET /apps` — Commons home, Apps section (`app/apps/page.tsx`).

## Data Model and Storage Contracts

The Commons owns no message/channel/bot tables. Its channel is backed by the Feed schema (canonical in `ctf/schema.sql`):

- `feed_items` — the blended timeline projection (`item_type` ∈ announcement | question | community).
- `feed_community_posts` / `feed_community_replies` — peer-to-peer posts. `feed_community_posts.reply_to_post_id` (nullable self-reference, `ON DELETE SET NULL`) records a Signal-style quoted reply between two peer posts. `feed_community_posts.edited_at` (nullable `TIMESTAMPTZ`) is stamped when `PATCH /api/commons/messages/:postId` rewrites a picture post's text in place; null for every text-only post, which is never edited in place.
- `feed_commons_last_seen` — per-member "last seen" marker for the Commons home channel (`user_id` PK, `last_seen_at`). Drives the single "New messages" divider in the Commons chat; read on entry, updated after viewing. Best-effort; a failure never breaks the chat.
...
- `feed_community_post_images` — the picture an admin attached to a community post (owner decision, 2026-09-25). `post_id` PK → `feed_community_posts.id` `ON DELETE CASCADE` (one picture per post, deleted with it — by its author, by moderation deletion, or with the account), `content_type` (CHECK png / jpeg / webp), `bytes` BYTEA, `byte_size`, `width`, `height`, `alt_text`, `created_at`. No user column: the author is the post's. Read as metadata only into `FeedCommunityDetail.image` → `HubMessage.image` (`{ url, alt, width, height }`) by `listFeedTimeline`; the bytes are served only by `GET /api/commons/images/:postId`. Helpers in `lib/feed/community-images.ts`.
...
Platform-neutral tables consumed read-only by the Commons shell:
...
2. Mutating Commons routes (`POST /api/commons/messages`) require a valid session and pass the Feed CSRF check (`x-ctf-csrf` + same-origin); reject with `401`/`403` otherwise.
...
9. Pictures (`feed_community_post_images`) are admin-only to post: `POST /api/commons/images` refuses a non-admin with 403 before reading the upload, and `POST /api/commons/messages` never attaches one. The file type is read from its first bytes, not from its name or the type the browser claimed, and served with `nosniff`. A picture is public along with its post: served to signed-in members, and to signed-out visitors while public viewing is on (rate-limited per IP, and only for a post the public list shows). It is served only while its post is up, and deleted with it. The browser re-encodes the picture before upload, dropping location and camera data; a direct call to the route by an admin that skips the browser would store the file as sent.
10. `PATCH /api/commons/messages/:postId` (`editCommunityPost`, contracts `feed.community.post.edit`) is author-only and requires the target post to currently carry a `feed_community_post_images` row — a text-only post has no in-place edit, closing the bait-and-switch vector `feed.community.post.delete`'s design note describes (post something innocuous, gather reactions, then rewrite it). A picture post is exempt because only an admin can create one. The new text is re-checked against the same content-moderation rule a fresh post passes, and a post a moderator has hidden cannot be edited back into view.
...
- Android: **surface removed 2026-07-20 (rule 105, PR #1742)** — this feature is now web-only, served by the installable web app (PWA). (Note: the native Commons/Chyme surface that remains on Android is covered by the Chyme inventory, not here.) Historical detail follows. The mobile home (`ctf/packages/mobile/src/features/hub/`) read the blended
  channel from `GET /api/commons/messages` (the same feed-backed timeline the web Hub uses, flattened to
  the `HubMessage` shape) and sends a peer-to-peer community post via `POST /api/commons/messages` with
  ...
  `POST /api/commons/join` for instant new-post updates and a typing indicator, and falls back to the 15s
...
- Public unauthenticated read of the blended channel is **not** part of this Android pass: like web,
  `GET /api/commons/messages` still requires a signed-in session. As of 2026-06-09 the Commons routes use the
...
There is no `seedHub.mjs`; the Commons channel's data layer is seeded by the Feed seed `ctf/scripts/seedFeedAnnouncements.mjs` (announcements, feed items, render config including `is_public`, community/question fixtures). The dropped `hub_*` tables are not seeded.

## Gaps and Known Technical Debt

1. Public unauthenticated read of the blended channel: `feed_render_config.is_public` is set and read into config, but `GET /api/commons/messages` / `listFeedTimeline` still require an authenticated session. A public read path (and the policy for which item types are exposed publicly) is the tracked follow-up.
2. Mobile Hub parity: delivered. The mobile home reads/writes the feed-backed channel via `GET/POST /api/commons/messages` (`ctf/packages/mobile/src/features/hub/`). The parity contract reconciliation was done on the existing `feed-announcements` entry (its `mobileFeatureDirs` now includes `hub`) rather than a standalone `hub` contract entry, because the web/Android parity gate requires every contract slug to exist in the plugin registry and the Hub is the home route (`/`), not a navigable app tile with its own registry slug.
3. Separate channels, direct messages, and system bots were dropped from the MVP (single blended channel). Revisit splitting into multiple Commons channels after feedback.
4. ~~`GET /api/commons/channels|dms|bots` are stubs (single-channel / empty); they can be removed or formalized when/if multi-channel returns.~~ **Closed (2026-08-03).** The description was out of date on all three counts: `/api/commons/dms` never existed; `/api/commons/channels` is not a stub (it is read by the Commons shell and does the real eligibility filtering for the gated contributor channel); and `/api/commons/bots` was a hardcoded empty list nothing called, so it was removed along with its `HubBotInfo` / `HubBotsResponse` types. Nothing to formalize until multi-channel actually returns.
...
- 2026-09-26: **The Commons is stripped back to the verification banner and the chat for a member not yet approved.** Owner decision: members stuck at Unlock were facing about 28 app tiles, a sort and a search, plugin links under answers and announcements, and a locked contributor chip, every one of which led to a wall. `CommunityShell` now derives `unlockFocus` (signed in, not an admin, `verification` non-null), forces the chat section, drops the Apps tab and icon-rail button and the locked chip, and provides `CommonsUnlockFocusContext` so `ComicAnswerCard`, `AnnouncementLinkedPlugins` and the suggestion chips (`commonsSuggestionChips({ unlockFocus })`) adapt. The banner's help note is now the shared Unlock help box (see the unlock inventory). No change for approved members, admins, or signed-out visitors.

- 2026-09-26: **Bug fix: the Edit control disappeared on a Commons post with a picture.** The Edit
  button was hidden for any post carrying a `feed_community_post_images` row, because the product's
  edit model is delete-and-repost, which would have dropped the picture. Added
  `editCommunityPost` (`lib/feed/repository.ts`) and `PATCH /api/commons/messages/:postId`: an
  in-place text rewrite, restricted to a post that carries a picture (so it stays admin-only, closing
  the same bait-and-switch concern the delete-and-repost model exists for) and blocked on a
  moderator-hidden post, mirroring the already-shipped `editAnnouncementReply` pattern. New nullable
  `feed_community_posts.edited_at` column, stamped on edit. Web: the "Edit" control now shows for a
  picture post too (`shell-chat-panel.tsx`); sending while editing PATCHes the post in place instead
  of posting a new message (`use-home-chat.ts`), shown by a "Editing your post" composer banner
  (reuses the existing reply-banner styling). A text-only post is unchanged — still delete + repost.
  New audit command `feed.community.post.edit` and matching access-policy contract entry.
- 2026-09-26: **Bug fix: expanding a Commons picture stranded the member.** `chat-message-image.tsx`
  opened the full-size picture with `<a target="_blank">`. The app is installed as a standalone PWA
  (`display: 'standalone'` in `manifest.ts`), which has no browser chrome, so that new tab left a
  bare native image viewer with no back button, no URL bar, and no way back into the app short of a
  force-close. Replaced with an in-app lightbox (`chat-image-lightbox.tsx`), dismissed by the
  backdrop, an X button, or Escape — the same pattern already used by `comic-consent-modal.tsx` and
  the shared `dialog-focus.ts` focus trap. No route, no `router.back()`, nothing for rule 134 to
  govern: it never leaves the app.
- 2026-09-25: **Pictures in the Commons, admins only.** The owner explains the product with
  screenshots, and Quora erases the accounts those were shared from, so the Commons is where they are
  shown now. New table `feed_community_post_images` (one picture per post, `ON DELETE CASCADE` from
  the post, bytes stored as BYTEA — there is no image store in the project and the volume is a handful
  of screenshots). New routes `POST /api/commons/images` (admin gate, CSRF, multipart, type sniffed
  from the bytes, 3 MB cap, alt text required) and `GET /api/commons/images/:postId` (Commons gate,
  accepted posts only, private cache). `createFeedCommunityPost` takes an optional `image` and writes
  it in the same transaction as the post; `loadCommunityDetails` reads the picture metadata into
  `FeedCommunityDetail.image`, carried to `HubMessage.image` and `ChatMessage.image`. Web: the
  admin-only "Share a picture (admins)" panel (`commons-image-share.tsx`, scales to 1600 px and
  re-encodes in the browser) and the picture above the message text (`chat-message-image.tsx`).
  Signed-out visitors see the pictures too while public viewing is on: `GET /api/feed/public/community`
  now carries `PublicCommunityPost.image`, and the image route serves them under the same rule
  (`isPublicCommunityReadOn`, extracted from `listPublicCommunityPosts`). Android: out of scope
  (web-only per rule 105).

- 2026-09-25: **"I'm new. What do I do first?" answer chip.** A third chip kind, `answer`, in
  `lib/concierge/commons-suggestions.ts`: a fixed reply written in the chip, shown in the chat on the
  spot through a new `answerChip` in `use-home-chat.ts`, with one button per place it names. The
  new chip leads the row and gives a new member the three things to do first, matching the blog post
  of that name. No AI draft and no review queue, so nobody waits to be told where to start (owner
  decision). `ChatMessage.actionLabel`/`actionSlug` are replaced by `actions: { label, href }[]`,
  which the concierge reply now uses too; a new `.chatActionRow` wraps several buttons at phone
  width. Guide sentence on the chips updated. Web-only; no route, schema, or contract change.

- 2026-08-09: **This file renamed `ctf-survivor-hub-chat-feature-inventory.md` →
  `ctf-hub-feature-inventory.md` (owner decision).** The last place the retired name was still
  load-bearing. Rule 120's convention is `ctf-{plugin-slug}-feature-inventory.md`, and this
  plugin's slug is `hub` (stated in Scope and Boundary above), so the old name was off-convention
  as well as off-brand. Cross-references in `ctf-feed-feature-inventory.md` and
  `ctf-comic-feature-inventory.md`, plus the self-reference in this file's own Delivery Status
  section, were repointed in the same change so every path still resolves.
  Not renamed, deliberately: the `design/.../survivor-hub/` mockup paths in
  `PRODUCTION_READINESS_PLAN.md` and `design-audit-2026-05-29.md`, and the deleted
  `survivor-hub-chat` mobile fixture names in the 2026-06-01 quota note. Those name real
  directories in the deprecated `design/` submodule and files that no longer exist; editing them
  would make correct references wrong.
- 2026-08-09 (latest): **"Hub" renamed to "Commons" throughout — routes, folders, types, and this
  file (owner decision).** The UI has called this surface the Commons for a long time (it is the
  canonical term in `BRAND_VOICE_LEXICON.md`); only the code still said Hub.
  - Routes moved: `/api/hub/*` → `/api/commons/*`, all seven of them
    (`messages`, `messages/[postId]`, `messages/[postId]/reactions`, `channels`, `join`,
    `last-seen`, `first-visit-notice`). **This is a breaking URL change.** It is safe here because
    nothing outside this repo calls them: the native app dropped its Commons surface under rule 105
    and now ships only Chyme, so every caller is a web component in this repo and moves with the
    routes. A browser holding the old page mid-rollout will 404 on its next poll and recover on
    reload.
  - Folders moved: `app/api/hub/` → `app/api/commons/`, `lib/hub/` → `lib/commons/`, and
    `lib/concierge/hub-suggestions.ts` → `commons-suggestions.ts`.
  - Types and functions renamed: `HubMessage` → `CommonsMessage`, `requireHubAccess` →
    `requireCommonsAccess`, and the rest of the `Hub*`/`hub*` set that moved with those folders.
  - This file renamed `ctf-survivor-hub-chat-feature-inventory.md` →
    `ctf-commons-feature-inventory.md`, matching rule 120's `ctf-{slug}-feature-inventory.md`
    convention. Cross-references in the feed and comic inventories were repointed.
  - **Stored data migrated too** (owner directive, same day — this began as a follow-up and was
    folded in):
    - Table `feed_hub_last_seen` → **`feed_commons_last_seen`**. The rename in `schema.sql` is
      guarded on both sides and runs *before* the `CREATE TABLE IF NOT EXISTS` for the new name —
      order that matters, because creating the empty new table first would strand every member's
      marker in the old one and reset the unread divider for everybody. The primary key is renamed
      with it. Readers updated in `lib/feed/repository.ts` and the deletion registry.
    - Column value `comic_conversations.channel = 'hub'` → **`'commons'`**, with `ComicChannel`,
      `COMIC_CHANNELS`, the API coercion, the client's ask body, and the comic seed all moved to
      `'commons'`. `normalizeChannel` folds a legacy `'hub'` row to `'commons'` on read, so rows
      written before the migration render identically.
    - The CHECK still **accepts** `'hub'`, on purpose and temporarily. `update-neon-db.yml` applies
      `schema.sql` on push to main while the web app redeploys separately, so for a few minutes the
      previous release serves against an already-migrated database; a strict
...
- 2026-08-09: **"Survivor Hub" retired as a name, and the "SH" avatar glyph is gone
  ...
  `OFFICIAL_SENDER_LABEL` in `lib/commons/constants.ts` (used by `app/api/commons/messages/route.ts`, the
...
- 2026-08-03: **Removed the `GET /api/commons/bots` stub and corrected the channel/DM descriptions.** The
  ...
  inventory lines that no longer matched the code: `GET /api/commons/dms` is described as a stub but no
  such route exists, and `GET /api/commons/channels` is described as a stub when it is the real,
...
- 2026-06-23: **Android parity — Commons live Stream layer + typing indicator (#730).** The React
  ...
  matches the web Commons live layer. On entry it calls `POST /api/commons/join` through `authedFetch`
  ...
  `POST /api/commons/join` returns `configured: false`, or the join call / connect / watch fails,
  ...
  `/api/commons/join` route. This opens a live Stream connection per mobile Hub viewer; see the quota note
...
- Canonical spec: `ctf/docs/developer/ctf-plugin-feature-inventories/ctf-hub-feature-inventory.md`.
- The Commons has no cross-plugin runtime dependency. See [112-platform-architecture-rules.mdc](../../../../.claude/rules/112-platform-architecture-rules.mdc).
...
- [x] Repoint `GET /api/commons/messages` at `listFeedTimeline` (blended `feed_items` read).
- [x] Repoint `POST /api/commons/messages` at `createFeedCommunityPost` (CSRF-guarded peer post).
...
- [x] Wire the mobile home to the same feed-backed channel (reads `GET /api/commons/messages`; sends a
      community post via `POST /api/commons/messages` with `x-ctf-csrf: 1`). See
...
- [ ] Remove or formalize the `GET /api/commons/channels|dms|bots` stubs once multi-channel direction is decided.

---

### Pre-Merge Gates

- [ ] `GET /api/commons/messages` returns the blended Feed timeline; `POST` creates a community post (CSRF-guarded).
...
- 2026-07-18: Added an announcements filter chip (📣) next to the "@ Mentions" chip in the Commons stream, on web (`shell-chat-panel.tsx`) and Android (`HubHome.tsx`). "Announcements" is too long for a chip, so it shows the megaphone emoji alone. When on, the stream reads `GET /api/commons/messages?channel=announcements`, which returns only official announcements — including ones that scrolled off the recent page — so a member with limited message history can still surface them. The two filters are mutually exclusive; turning one on clears the other, and the AI (`@comic`) cards are hidden while any filter is active.
