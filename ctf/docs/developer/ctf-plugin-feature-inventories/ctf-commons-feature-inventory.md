...
14. Pictures from admins (owner decision, 2026-09-25): an admin sees a "Share a picture (admins)" button above the message box. It opens a panel with a file picker, a preview, a required "What it shows" description (read aloud to members using a screen reader), and the message to go with it. Share posts both as one message. Every member then sees the picture in that message, above its text; tapping it opens the full-size copy in a new tab. Members cannot attach pictures. The owner uses this to explain the app with screenshots, now that Quora erases the accounts those were shared from.
...
- `GET /api/commons/images/:postId` — serve the picture on a Commons post. Commons access gate (the same sign-in as the messages); `postId` must be a UUID (else 400). Served only while the post's `moderation_status` is `accepted` (else 404). Headers: the stored `Content-Type`, `X-Content-Type-Options: nosniff`, `Content-Disposition: inline`, `Cache-Control: private, max-age=86400`. Pictures are not part of the signed-out public view.
...
9. Pictures (`feed_community_post_images`) are admin-only to post: `POST /api/commons/images` refuses a non-admin with 403 before reading the upload, and `POST /api/commons/messages` never attaches one. The file type is read from its first bytes, not from its name or the type the browser claimed, and served with `nosniff`. A picture is served only to signed-in members and only while its post is up; it is deleted with its post. The browser re-encodes the picture before upload, dropping location and camera data; a direct call to the route by an admin that skips the browser would store the file as sent.
...
- 2026-09-25: **Pictures in the Commons, admins only.** The owner explains the product with
  ...
  Signed-out public view unchanged: it carries no pictures. Android: out of scope (web-only per rule
  105).
