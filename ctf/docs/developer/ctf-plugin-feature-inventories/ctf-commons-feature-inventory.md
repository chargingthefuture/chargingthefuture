...
- `GET /api/commons/last-seen` — read the member's last-seen marker for the Commons home channel (`{ ok, lastSeenAtIso }`); `lastSeenAtIso` is null when never recorded. Backed by `getCommonsLastSeen` over `feed_hub_last_seen`. Best-effort.
...
- `feed_hub_last_seen` — per-member "last seen" marker for the Commons home channel (`user_id` PK, `last_seen_at`). Drives the single "New messages" divider in the Commons chat; read on entry, updated after viewing. Best-effort; a failure never breaks the chat.
...
- 2026-08-09 (latest): **"Hub" renamed to "Commons" throughout — routes, folders, types, and this
  ...
  - **Deliberately not renamed:** the string `'hub'` where it is *stored data* —
    `comic_conversations.channel` has `CHECK (channel IN ('hub', 'feed'))`, so `ComicChannel`,
    `COMIC_CHANNELS` and the comic seed keep it. Changing those means a data migration, not a
    rename. Also unchanged: the `feed_hub_last_seen` table name, the `from: 'hub'` client-side
    message discriminator, the `area: 'hub'` error-reporting tag (renaming it would split error
    grouping history), and the `design/.../survivor-hub/` mockup paths, which name real
