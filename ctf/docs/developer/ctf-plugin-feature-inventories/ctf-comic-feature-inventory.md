...
- 2026-06-13: Added a live AI-services status panel to the `/admin/comic` review console. New admin-only read endpoint `GET /api/comic/admin/ai-status` pings Rasa (`POST /model/parse`) and Ollama (`GET /api/tags`) with a 5s timeout and returns `{ configured, reachable, latencyMs }` for each (`pingRasa` in `lib/comic/rasa.ts`, `pingOllama` in `lib/chatbot/ollama.ts`). The console's queue header shows two badges (green reachable + latency / red unreachable / grey not configured) so the owner can tell at a glance whether the AI backends are working — no separate monitoring page. Read-only; never blocks the queue. No schema or contract change.
