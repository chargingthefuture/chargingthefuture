...
1. Reach the assistant with `@comic <question>`; reply renders inline.
   - **Unlock questions from a member not yet approved (2026-09-26).** A question that names Quora
     plus a word for the link ("profile", "URL", "link", "address"), or says "profile URL", gets a
     **scripted draft** built from `lib/unlock/quora-url-help-steps.ts`, picked by which way the
     member is stuck: can't get into the account, a link that was refused, on the Quora app, or the
     plain browser steps (which also carry the one-line app version). Any other question naming
...
6. `comic_knowledge_entries` — **retrieval knowledge base for draft grounding (#504 retrieval
   ...
   text, `content_hash` text, `source_ref` text null — stable per-item identity for repo-sourced
   rows (added 2026-07-28), `active` boolean default true — the curation off-switch, `authored_at`
   null, `created_at`, `updated_at` timestamptz null — set when an edit updates a row in place).
   Full-text search runs over question+title+content via the GIN expression index
   `idx_comic_knowledge_entries_search`; also indexed on `active`. **Two idempotency strategies,
   split by two partial unique indexes (2026-07-28):** `uq_comic_knowledge_entries_source_ref`
   (`WHERE source_ref IS NOT NULL`) keys repo-sourced rows on `source_ref`;
   `uq_comic_knowledge_entries_content_hash` (`WHERE source_ref IS NULL`) keeps the legacy
   content-hash dedupe for rows imported from the raw HTML export. (The old global
   `UNIQUE(content_hash)` is dropped in the guarded DDL so legacy DBs converge.) Populated by
   `ctf/scripts/importComicKnowledge.mjs` from the JSONL produced by three parsers:
   `parseQuoraExportToComicDataset.mjs` (raw Quora HTML export, no `source_ref` → content-hash
   dedupe), `parseQuoraMarkdownToComicDataset.mjs` (the owner's **edited** Quora Markdown repo —
   `chargingthefuture/quora`, one `.md` per post — emits a stable `source_ref` so a re-import of an
   edited file **updates the existing row in place** instead of leaving two versions active), and
   `parseWikiToComicDataset.mjs`. The import is add/update only — it never sets `active = FALSE`;
   retiring an entry stays a manual curation step. At draft time `retrieveComicGrounding()` fetches
   the top 4 active entries ranked by `ts_rank`/`websearch_to_tsquery` and injects them into the
   Ollama model instructions; retrieval is best-effort (failure or no match → the draft runs
   ungrounded, as before). The draft turn records which entries grounded it in
   `comic_turns.grounding_entry_ids` (jsonb array, default `[]`) so grounded vs ungrounded drafts
   can be compared on correction rate — the #504 "measure" step.
...
- 2026-07-28: **Edited Quora Markdown repo becomes the source of truth for grounding, with in-place edit reconcile (#504; schema change).** Closes the loop between the owner's copy-editing pass and the bot. Before this, the import pipeline only read Quora's raw HTML export; the owner's per-post corrections in the `chargingthefuture/quora` repo (one Markdown file per answer/post/comment) had no path to `comic_knowledge_entries`, and re-running the HTML parser would have overwritten the edits with the original text. (1) New parser `ctf/scripts/parseQuoraMarkdownToComicDataset.mjs` reads that Markdown repo (header block + `---` + edited body), maps each section folder to a record type (answers/answer-drafts → answer, spaces-posts → post, the three comment folders → comment), runs the same `redact()` safety pass as the HTML parser, and emits the existing JSONL shape **plus a stable `source_ref`** = `quora:<account>/<section>/<numeric-index>` (constant across body edits and slug renames). Answer drafts **are** included here (owner decision) — unlike the HTML parser, which skips them — because a draft kept in the curated repo is one the owner chose to keep. (2) Schema: added `source_ref text null` and `updated_at timestamptz null` to `comic_knowledge_entries`; replaced the global `UNIQUE(content_hash)` with two partial unique indexes — `uq_comic_knowledge_entries_source_ref` (`WHERE source_ref IS NOT NULL`) and `uq_comic_knowledge_entries_content_hash` (`WHERE source_ref IS NULL`) — so repo-sourced rows dedupe on identity and legacy HTML-export rows keep content-hash dedupe; the old auto-named constraint is dropped in guarded DDL so legacy DBs converge. `schema.demo.sql` mirrored. (3) `importComicKnowledge.mjs` upserts on `source_ref` when present: first import inserts, an unchanged re-import is a no-op, an **edited** file updates the same row in place (new content + `updated_at`) so no out-of-date second copy lingers — the reconcile gap. The legacy no-`source_ref` path is unchanged (insert-or-nothing on content hash). Import remains add/update only — it never deactivates a row; retiring an entry stays a manual `active = FALSE` curation step (owner decision: no auto-deactivation of items removed from the repo). Verified end-to-end against an embedded Postgres: insert → unchanged skip → in-place update, one row per `source_ref`, no orphan, legacy dedupe intact. No route/contract change (loader script + schema only); grounding retrieval and human review are unchanged.
