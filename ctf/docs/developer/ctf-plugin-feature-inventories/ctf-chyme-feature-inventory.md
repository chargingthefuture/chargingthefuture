...
6. The Android audio room does not render other members' **persistent** raised hands. It now persists its own hand (`POST /api/chyme/hand`, 2026-06-26) so web sees it, but mobile shows other participants' hands only from the transient Stream reaction — it does not poll the room state for the server-persisted `handRaised` set the way the web room does (`chyme-live-shell`'s 15s room poll → `raisedHandUserIds`). A mobile member therefore won't see another member's hand stay up after the reaction auto-clears. Follow-up: add a light room poll on mobile and thread a `raisedHandUserIds` set into `ChymeAudioRoom` to match web.
