...
- `POST /api/chyme/hand` — persists the caller's raise/lower hand on their presence row (`{ raised: boolean }`); returns `{ ok, room }` with refreshed participants. The raised hand stays visible to everyone until lowered, the member leaves, or their presence goes stale. Audit command `chyme.hand`.
- `POST /api/chyme/leave` — drops the member's presence row on exit (which also clears any raised hand).
...
6. `POST /api/chyme/service-credits` (a state-changing, money-moving route) has no same-origin CSRF header check — it is gated by `requireChymeAccess` only, where the sibling plugin service-credits routes (lighthouse / foundation / skills-hunt) all also call `ensureMutationCsrf`. Adding the CSRF guard for parity is a follow-up (code change, out of scope for this documentation pass).
