...
6. The other chyme mutation routes (`POST /api/chyme/hand`, `/messages`, `/leave`, `/heartbeat`) still do not call `ensureMutationCsrf` — they are gated by `requireChymeAccess` only. They move no money (presence/chat state), so the risk is lower than the now-fixed service-credits route, but tightening them for full parity is a small non-money follow-up.
