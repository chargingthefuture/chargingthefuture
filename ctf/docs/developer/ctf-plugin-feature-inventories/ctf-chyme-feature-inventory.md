...
3. Live audio is implemented with the Stream Video SDK (`@stream-io/video-react-sdk`) in `components/chyme/chyme-audio-room.tsx`: joining the `default` call type audio-only, real microphone mute/unmute, live speaking and mute indicators per participant, hearing other participants (`ParticipantsAudio`), raise-hand broadcast (Stream reaction), and leave. Requirement: the Stream app used by `STREAM_API_KEY`/`STREAM_API_SECRET` (and the demo `*_STAGING` pair) must have the Video product enabled, since the same user token is used for both Chat and Video. Speaker-vs-listener moderation (request-to-speak grant flow) is not yet built — every joiner may speak.
