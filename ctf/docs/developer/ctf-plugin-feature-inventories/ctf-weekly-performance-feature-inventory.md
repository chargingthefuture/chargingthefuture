...
Live numbers (2026-06-29): a week with no stored snapshot in `weekly_performance_metrics` no longer renders empty. `getWeekMetrics` falls back to `computeLiveWeekMetrics` (`lib/weekly-performance/live-metrics.ts`), which counts the week window directly from upstream tables — distinct active members (`login_events`), questions asked (`feed_questions`), answers posted (`feed_answers`), accepted community posts (`feed_community_posts`), and enrollments started (`level_up_enrollments`). All are non-financial activity counts scoped to `[week_start, week_start + 7 days)` on each row's `created_at`; each is table-existence-guarded so a missing table contributes 0 rather than failing the read. A stored snapshot still wins, so a closed/published week keeps its recorded values.
