...
Live numbers (2026-06-29): weekly numbers are **always** computed live, matching the V2 dashboard, which aggregated on read for any selected week. There is no "close the week" step and no stored snapshot to wait for. `getWeekMetrics` returns `computeLiveWeekMetrics` (`lib/weekly-performance/live-metrics.ts`) for every week, counting the week window directly from upstream tables — distinct active members (`login_events`), questions asked (`feed_questions`), answers posted (`feed_answers`), accepted community posts (`feed_community_posts`), and enrollments started (`level_up_enrollments`). All are non-financial activity counts scoped to `[week_start, week_start + 7 days)` on each row's `created_at`; each is table-existence-guarded so a missing table contributes 0 rather than failing the read. The current week is labelled "Live" and keeps moving (silent 60s polling + focus refetch on web); every earlier week is a settled historical window with no "closed" state. The `weekly_performance_metrics` table and the `weekly-performance.metrics.get` route are unchanged; the read is simply always live (the table is no longer consulted for the dashboard).
