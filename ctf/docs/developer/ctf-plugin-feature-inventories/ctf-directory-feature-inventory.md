...
1. Admin controls are embedded in the same Directory UI surface with role-gated visibility — including **attaching an unclaimed profile to a user account (assign)**, which is implemented **inline on the Directory surface (admin-only controls on the profile detail), not as a separate admin page**.
...
> Design note (owner decision, 2026-06-03): Directory admin controls are inline and role-gated on the same Directory surface — there is deliberately no standalone Directory admin page. Do not design a separate admin page for Directory; the owner is consolidating admin actions onto each plugin's own surface to avoid one admin page per app.
