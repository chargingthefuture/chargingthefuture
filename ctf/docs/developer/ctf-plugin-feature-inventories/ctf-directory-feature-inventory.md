...
3. **Directory is no longer public-facing** (2026-05-18). The `isPublic` toggle was removed; every authenticated member sees every active, non-deleted profile. There is no anonymous projection route. Legacy public URLs are not redirected â€” backwards compatibility is intentionally not preserved.
...
1. `directory_profiles` â€” Directory profile records with claimed/unclaimed state.
2. `directory_announcements` â€” Directory announcements with activation/deactivation state.
3. Skills hierarchy (shared taxonomy) â€” Selector-backed taxonomy data.
4. Profile policy contracts â€” Claimed/unclaimed state and assignment constraints.
5. Public projection contracts â€” Privacy-filtered output shape for unauthenticated callers.

### New columns on `directory_profiles` (Skills Hunt + Clerk username co-change, 2026-05-11)

1. `source TEXT NOT NULL DEFAULT 'admin' CHECK (source IN ('admin', 'self', 'community-generated'))` â€” drives the "Community generated profile" badge in the UI.
2. `invited_by_username TEXT NULL` â€” denormalized from `skills_hunt_directory_profiles.invited_by_username` so the (auth-gated) profile page renders attribution without a join.
3. `unclaimed_handle TEXT NULL` â€” auto-generated vanity handle for unclaimed profiles. Format: `community-<6char-hex>` (no leading `@` in storage; `@` is presentation only). Cleared/ignored once `claimed_by_user_id` is set. Uniqueness is enforced by a case-insensitive partial unique index (`directory_profiles_unclaimed_handle_key` on `lower(unclaimed_handle)` WHERE `unclaimed_handle IS NOT NULL`), not an inline column constraint â€” so `Community-7F3A2B` and `community-7f3a2b` cannot coexist, and the migration block can drop/recreate the index without tripping over a constraint-backed index on the fresh-schema path.
4. `deleted_at TIMESTAMPTZ NULL` â€” soft-delete for GDPR and moderation removals; `is_active` remains in place but `deleted_at` takes precedence for visibility filters.
...
- [ ] Anti-scraping controls design. [TESTING DEFERRED FOR MVP â€” see Rule 118.]
  - Acceptance criteria:
...
- [ ] Parity coverage tracking. [AUTOMATED PARITY COVERAGE TESTING DEFERRED FOR MVP â€” see Rule 118.]
  - Acceptance criteria:
...
- [ ] Implementation evidence tracking. [EVIDENCE CAPTURE DEFERRED FOR MVP â€” see Rule 118.]
  - Acceptance criteria:
