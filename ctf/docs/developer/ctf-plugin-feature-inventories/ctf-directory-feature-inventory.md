...
- 2026-06-02: v2 to v3 data carry-over. The live v2 Directory data (66 profiles) is brought into the v3 schema as **unclaimed** profiles during the cutover. `schema.sql` ADDs the v3 columns onto the existing v2 `directory_profiles` and backfills each unclaimed row a `community-<hex>` handle, but the v2 table stores a profile's data under different column names, so the v3 columns (`display_name`, `bio`, `profile_url`) landed empty. Added post-schema migration `db/migrations/post/0001_directory_v2_to_v3_backfill.sql` that copies `display_name <- first_name`, `bio <- description`, `profile_url <- quora_url`, only where the v3 column is still blank and only when the v2 source column exists (no-op on a fresh v3 database). Profiles stay unclaimed (`claimed_by_user_id` left NULL). Skills / sectors / job_titles (v2 arrays) are deferred to a follow-up migration since they need the normalized skills tables. `directory_profiles.id` is left as `varchar` (does not block; app tolerates it). No code/route/schema-shape change.
