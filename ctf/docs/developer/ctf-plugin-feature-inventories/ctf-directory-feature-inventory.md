...
3. No admin path yet to promote a member-added free-text skill (`directory_profile_proposed_skills`, status `pending`) into the canonical taxonomy. The data model supports it (a `status` column ready to move to `promoted`), and the existing skills-taxonomy admin already promotes SkillsHunt proposals; a small admin action/route to surface and promote the directory-self-added labels is a follow-up. Until then, self-added skills remain visible as "pending review" chips indefinitely.
