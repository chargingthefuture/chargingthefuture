...
**Routing to the central promotion queue.** Self-added labels are not a Directory-local promotion: they feed the one cross-app intake every skill addition goes through. The scheduled job `proposeSkillPromotions.mjs` (workflow `skills-proposal-issues.yml`, every 6h) scans `directory_profile_proposed_skills` (status `pending`) alongside accepted SkillsHunt nominations, skips any label already in the taxonomy or already tracked, and files one `skill-proposal` GitHub issue per new label into `skills_hunt_proposed_skill_promotions` (with `source = 'directory'`). That GitHub issue **is** the review queue — there is deliberately no in-app promotion screen. When the owner approves and `seedSkillsTaxonomyPromotions.mjs` promotes the skill into the taxonomy, it flips the matching `directory_profile_proposed_skills` row to `promoted` (by normalized label), which clears the member's "pending review" chip.
...
3. Member-added free-text skills route to the central `skill-proposal` GitHub-issue queue (shared with SkillsHunt) for owner review and promotion — there is intentionally no in-app promotion screen (owner decision, 2026-06-25). Minor remaining wrinkle: once a label is promoted, its row flips to `promoted` and the "pending review" chip clears, but the member is not auto-given the now-official taxonomy skill — they must re-select it from the (now-populated) Specializations list. Auto-attaching the promoted taxonomy skill to the profile is a possible later enhancement.
