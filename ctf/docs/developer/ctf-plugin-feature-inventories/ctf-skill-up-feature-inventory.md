...
10. **Sessions run in English by default** (2026-09-13). Every cohort card says so before anyone enrolls: a trainer may run part or all of a cohort in another language when everyone enrolled agrees to it, and English stays the default either way. Somebody who does not read or speak English comfortably is told to ask the trainer first, because no translation is provided and nobody is responsible for arranging one. This is a stated rule on the card, not an enforced field — nothing in the data model records a cohort's language.
11. **The cohort card opens collapsed** (2026-09-13). Visible at a glance: title, status, trainer name, milestone count, seats, returnable deposit, Enroll. A **Details** toggle opens the tags, the trainer and learner figures, and the language note.

12. **Claiming a cohort as its trainer, from the member screen** (2026-09-13). A cohort with nobody teaching it says so on its browse card and offers a Claim as trainer control. The server decides: your claimed Directory profile has to carry a skill under the occupation the cohort trains. Every refusal names the thing to change — claim your Directory profile, or add a skill for this occupation — rather than saying no.
...
- 2026-09-13: **A member had no way to claim a cohort as its trainer.** Owner report. Hand approval of trainers was replaced on 2026-08-29 by a check in code, and `POST /api/skill-up/cohorts/[cohortId]/claim-trainer` has taken plain read access ever since — any approved member may call it, and it decides on whether their claimed Directory profile carries a skill under the cohort's occupation. Its three refusals were already written for the person rather than the operator. But the only control that called it was `ClaimTrainerControl` in the admin cohort list, so a member looking at a cohort reading Trainer TBD had nowhere to say yes, and the public invitation to come and teach pointed at a button that existed for admins only. The browse card now carries the control whenever the cohort still has no trainer, and shows the server's refusal as written. Two details worth keeping: the card reads the server's own `needsTrainer` rather than inferring it from a missing trainer name, because those are different questions — a cohort is claimable while it is still owned by the scheduler that opened it, and the name can be absent for other reasons; and the control sits on the card rather than behind the Details toggle, since a cohort with nobody teaching it is making an ask and the ask should not be something you have to open to find. A successful claim refreshes the cohort list through the same path the Progress tab already uses after leaving a cohort.

- 2026-09-13: **The cohort card states the session language, opens collapsed, and no longer carries an empty band above its title.** Owner directive, three parts. **Language:** every card now says sessions are held in English by default, that a trainer may run part or all of a cohort in another language when everyone enrolled agrees, that English stays the default either way, and that somebody not comfortable in English should ask the trainer before enrolling — no translation is provided and nobody is responsible for arranging one. It sits on the card rather than in a policy page because the moment it matters is the moment before enrolling, and a learner who cannot follow the sessions has a deposit down by the time they find out. It is a stated rule, not an enforced one: no column records a cohort's language and nothing checks it. **Layout:** the track and status chips sat on a row of their own above the title, so every cohort whose track repeats its title — which is all three of the open ones, opened from the proposal queue and titled with their occupation — rendered that row empty on the left and pushed the title down behind a band of whitespace. Title and chips now share one row, which closes the gap and lines the status chip up with the title it describes. **Density:** the card had grown to carry the tags, both economics columns and the seats/deposit/enroll row at once. The figures and tags moved behind a `Details` toggle (a real button with `aria-expanded`, keyboard-operable), leaving the scanning view as title, status, trainer, milestone count, seats, deposit and Enroll. Covered by test-script cases LU-1 and the new LU-1b.

- 2026-09-13: **Leaving a cohort before it started made it impossible to enroll in it again.** Owner bug report. `skill_up_enrollments` carried an unconditional `UNIQUE (cohort_id, user_id)`, and leaving does not delete the row — it sets `status = 'dropped'` and keeps it as the record of what happened. So that row held the slot forever and the database refused every later attempt. The application had always disagreed with the index: `assertEnrollmentSeatAvailable` counts only `'enrolled'` and `'active'`, so the seat check passed and the `INSERT` then failed on the constraint. This is the same fault in the same shape as the SkillsHunt duplicate guard fixed on 2026-08-27 — a blanket unique index where the code's rule is a partial one. Replaced with a partial index carrying the code's own predicate (`WHERE status <> 'dropped'`), in `schema.sql` and in `post/0014`. Verified against a scratch Postgres: on the shipped index a re-enroll after leaving is refused, on the partial one it succeeds, and a second **live** enrollment is still refused either way. Completing a cohort does not make it re-enrollable — only dropping does. **Second, smaller fault in the same flow:** a refused leave left the confirmation buttons on screen with the error beside them, so the same press could be repeated against a row the server had already rejected, which is how one refusal became a red banner on every card. A failed leave now closes the confirmation and re-reads the enrollments from the server, so the screen returns to what is actually true.

- 2026-09-13: **A class that has started can no longer be left, and a closing cohort now returns what it still holds.** Owner decision, two halves that only work together. **The gate:** `leaveCohort` refuses with `already_started` once the enrollment carries an assigned trainer **and** the cohort's start date has arrived. Both halves are required — a date that has passed with nobody teaching is not a class, and a trainer attached to a cohort that starts next week is not one either. The cohort `status` column is deliberately not consulted: nothing in the codebase sets a cohort to `active`, so a gate resting on it would never fire. A missing or unreadable start date reads as not started, which leaves the exit open — erring the other way would trap somebody because of a bad row. **The settlement:** `closeExpiredAutoCohorts` used to flip an expired cohort to `completed` in a single `UPDATE`, and its own comment said that was safe only because auto cohorts carried no deposit. That stopped being true when every cohort moved to the flat 50, and it matters more now that a learner cannot take themselves out — this is the only path by which their remaining deposit comes back. It now refunds every held escrow for the cohort first, through the new `refundHeldEscrowForCohort`, and flips the status only after that succeeds; a cohort that fails to settle is left open and reported, so the next run tries again rather than closing over stranded credits. **Nobody loses credits either way** (owner decision 2026-09-13): the gate removes the exit, not the deposit. **One rule, one place:** the member enrollment list returns a server-decided `canLeave`, and the Progress tab shows the Leave control or the explanation from that flag rather than working it out again — the client re-deriving a server rule is what produced the SkillsHunt round window, the nomination form and the trainer rate faults inside one week. Covered by `lib/skill-up/class-started.test.ts`.
...
- 2026-09-12: **Every cohort card read "Trainer earns 0 SC per learner", whatever its rate was set to.** Owner bug report. The card works out what one learner finishing is worth by multiplying the cohort's `trainer_credits_per_milestone` by its milestone count — and **nothing on the server ever sent a milestone count**. `cohortEconomics` reads `cohort.milestoneCount ?? cohort.milestoneCountForEarnings ?? 0`, both of those fields were optional on the client type, and neither `mapCohort` nor the cohort list query produced either one. So the multiplication was always by zero and the rate on the row never reached the screen. This is not the stored data: verified against a scratch Postgres that the column carries 10 through its own `NOT NULL DEFAULT 10` backfill, that the shipped query returns no count, and that the fixed query returns 3 for a cohort with the standard three-milestone skeleton — so the card moves from 0 to 30 SC per learner with no row rewritten. The list query now counts milestones in a `LEFT JOIN`, the same shape as the active-enrollment count beside it, and `mapCohort` exposes `milestoneCount`. `getCohortDetail` gains the same two columns in its `SELECT` — it had been omitting `trainer_credits_per_milestone` entirely — and takes its count from the milestones it already loads, so the two read paths cannot disagree. The learner side was never wrong: the deposit and the get-it-back line come from `required_credits`, which post/0011 set. Covered by `lib/skill-up/cohort-economics.test.ts`, which separates the two ways the figure can be zero — a rate genuinely set to zero, and a cohort with no milestones — from the missing-field zero this fixes.
