...
2. Fulfillment detail and ‚Äúmy fulfillments‚Äù views.
...
1. `socketrelay_user_extension` ‚Äî Per-user profile extension fields.
2. `socketrelay_requests` ‚Äî Request lifecycle rows (status, ownership, repost lineage).
3. `socketrelay_request_events` ‚Äî Event log for request state transitions.
4. `socketrelay_fulfillments` ‚Äî Fulfillment claims and outcomes per request.
5. `socketrelay_fulfillment_participants` ‚Äî Participant access records for fulfillment chats.
6. `socketrelay_messages` ‚Äî Participant-only chat messages on a fulfillment.
7. `socketrelay_admin_audit_trail` ‚Äî Audit log for admin mutations.
...
### Äî Web MVP Delivery (Release Gate)
...
### Äî Android Deferrals Tracking (Not Strict Parity Gate)
...
### Äî Risk Mitigation and Hardening
...
### Äî Validation, Seeds, and Release Gates [MVP: VALIDATION DEFERRED ‚Äî see Rule 118.]
...
- [ ] Lifecycle integration design documentation.
  - Acceptance criteria:
    - Request ‚Üí fulfillment ‚Üí close and chat access constraints are documented.
...
- [ ] Implementation tracking. [EVIDENCE CAPTURE DEFERRED FOR MVP ‚Äî see Rule 118.]
  - Acceptance criteria:
