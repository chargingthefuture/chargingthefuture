...
5. Android ownership detection leans on `GET /api/socketrelay/my-requests` (a card is "mine" if its id appears in that list) because the React Native client never learns its own user id. Works, but costs an extra request per feed load; if the mobile app later gains session identity, switch to comparing `ownerUserId` directly.
