...
- **When the Anthropic API is unavailable (no credit, no key, rate limited, vendor down):** no proposal is lost. The failing skill has no issue and no held claim, so it stays a candidate and the next scheduled run picks it up from the same submissions — the accepted nomination and the Directory entry are never consumed or marked. A failure whose status means every candidate would fail the same way (401/402/403/429/5xx, or a 400 whose body names the credit balance) stops the run at the first candidate instead of burning the rest, and the run **exits non-zero** so the scheduled workflow goes red and names the reason rather than passing quietly with nothing filed. A run in which every attempted candidate failed also exits non-zero. Practical effect: while the API is paused the workflow keeps going red every 6 hours with the credit-balance message; when funds are added, the next run files the backlog with no manual step.
