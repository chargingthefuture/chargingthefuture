...
4. Dead peer-transfer plumbing remains after the #1105 route removal: the `skills_hunt_service_credits_transactions` table and the unused repository helpers `createSkillsHuntServiceCreditsTransaction` / `getSkillsHuntServiceCreditsTransactionsForUser` (plus their `SkillsHuntServiceCreditsTransaction*` types) are never written by the live reward path (treasury mint via the canonical ServiceCredits ledger). Dropping the table is a separate schema migration (owner-review); the helpers/types can be removed in a code-only follow-up.
