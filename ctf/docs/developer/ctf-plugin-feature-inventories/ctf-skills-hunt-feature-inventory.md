...
- **When the Anthropic API is unavailable (no credit, no key, rate limited, vendor down):** no proposal is lost. The failing skill has no issue and no held claim, so it stays a candidate and the next scheduled run picks it up from the same submissions — the accepted nomination and the Directory entry are never consumed or marked. A failure whose status means every candidate would fail the same way stops the run at the first candidate instead of burning the rest, and the run **exits non-zero** so the scheduled workflow goes red rather than passing quietly with nothing filed. A run in which every attempted candidate failed also exits non-zero. **The red run then has to earn itself.** Red alone is a false alarm — an unpaid account and a broken pipeline are the same red X in the Actions list, and the months an account can sit unpaid are exactly when that difference matters. So the run names which of four states it is (`no_credit` — 402, or 400 naming the credit balance/billing/quota; `key_rejected` — 401/403; `rate_limited` — 429; `vendor_down` — 5xx), says outright that nothing is broken and nothing is lost, quotes the vendor's own sentence, and says what to do — "add funds when it suits, nothing in this repo needs changing" for the funding case, "nothing" for a throttle or an outage. That reading goes to stdout AND, under GitHub Actions, to the run's error annotation and job summary, so the run page answers the question without anyone opening the log. A missing `ANTHROPIC_API_KEY` gets the same plain reading instead of a bare "not set". Practical effect: while the API is unfunded the workflow keeps going red every 6 hours, but each red run reads as "skill proposals paused — the Anthropic account is out of credit (not a code failure)"; when funds are added, the next run files the backlog with no manual step.
