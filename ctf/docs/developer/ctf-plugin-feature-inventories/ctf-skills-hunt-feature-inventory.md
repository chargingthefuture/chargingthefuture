...
3. Four goal types: total accepted nominations, nominations with a skill in a named sector, rare-skill finds, and distinct sectors covered. Sector-goal progress counts a nomination when any of its skills belongs to the sector in the Skills Taxonomy (with the older claimed-profession text match kept for back-compat).
4. Some missions open automatically from Workforce talent gaps (see §2.4): when Workforce shows a sector is short of people, an active round gets a "Scout the [sector] sector" mission pointing scouting effort at it. To the member these look and behave like any other mission.
...
1. Create, list, and archive missions per round from the admin Missions tab (soft-archive only; hard delete is not exposed so progress history survives). All mission mutations write `skills_hunt_audit_log`-style audit events. **A new mission is created active — there is no status picker (owner directive 2026-08-27, "no drafts are needed").** The create form asks only for what a mission is: title, goal type, goal target, bonus points, an optional color and description. The round already carries its own draft/active lifecycle, so a second gate inside it gated nothing, and because there is still no mission edit control (the `PUT` route exists but no screen calls it — see Gaps) a mission created as draft could only ever be archived, never shown to members.
...
3. Admin panel on the Missions tab: kill switch, minimum sector gap, max auto missions per round, default goal target and bonus points (stored in `skills_hunt_auto_mission_config`), plus a "Run now" button that runs the same generator the weekly cron calls.
...
14. `skills-hunt.mission.list`
15. `skills-hunt.mission.create`
16. `skills-hunt.mission.update`
17. `skills-hunt.mission.archive`
18. `skills-hunt.mission.auto_generate`
19. `skills-hunt.mission.auto_config_update`
...
12. `skills_hunt_missions` — themed sub-goals within a round (`goal_type` ∈ count_total_accepted | count_skills_in_sector | count_rare_skill_finds | count_distinct_sectors; `goal_metadata` carries `sectorName` for the sector goal; statuses active/locked/archived, database-enforced by `skills_hunt_missions_status_check`; soft-archive only. **Missions have no draft state (owner directive 2026-08-27)** — they are created active, and Archive is the only lifecycle action on the admin surface. The round already carries its own draft/active lifecycle, and with no mission edit control a draft mission could only be archived, never shown to members. `locked` is retained for existing rows and still has no locking-condition logic behind it). Auto-open provenance columns (2026-08-27): `auto_created`, `source_sector`, `source_gap_at_creation`, with the partial unique index `uq_skills_hunt_auto_mission_active` guaranteeing at most one non-archived auto mission per (round, sector).
13. `skills_hunt_mission_progress` — per-user per-mission progress, recomputed from accepted submissions on every review outcome; `completed_at` set once on crossing the goal target. `bonus_credited_at` exists but is not yet written (see Gaps).
14. `skills_hunt_auto_mission_config` — singleton of admin-editable auto-mission knobs: `enabled` (kill switch), `min_gap_threshold`, `max_per_round`, `default_goal_target`, `default_bonus_points`. Coded defaults apply until the row is written.
...
3. Mission `bonus_points` are display-only: `skills_hunt_mission_progress.bonus_credited_at` exists but no code writes it, and no ServiceCredits ledger entry is made on mission completion (continuity §2.9 specifies one; unbuilt). Auto-opened missions default their bonus to 0 so they do not widen this. The `locked` mission status is display-only too — no locking-condition logic exists.
