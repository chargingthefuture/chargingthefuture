...
3. A mission's bonus points are **points, not credits** (owner directive 2026-08-27): points are a leaderboard ranking figure and have no connection to ServiceCredits. Note the gap this leaves — `bonus_points` is currently decorative in both directions: the leaderboard score is `SUM(points_awarded)` over accepted submissions only, so a mission's bonus does not reach a member's rank, and `bonus_credited_at` is still never written. Recorded in Gaps. This supersedes continuity §2.9's line about a service-credits ledger entry on mission completion.
...
1. Create, list, and archive missions per round from the admin Missions tab (soft-archive only; hard delete is not exposed so progress history survives). All mission mutations write `skills_hunt_audit_log`-style audit events.
...
12. `skills_hunt_missions` — themed sub-goals within a round (`goal_type` ∈ count_total_accepted | count_skills_in_sector | count_rare_skill_finds | count_distinct_sectors; `goal_metadata` carries `sectorName` for the sector goal; statuses draft/active/locked/archived; soft-archive only). Auto-open provenance columns (2026-08-27): `auto_created`, `source_sector`, `source_gap_at_creation`, with the partial unique index `uq_skills_hunt_auto_mission_active` guaranteeing at most one non-archived auto mission per (round, sector).
13. `skills_hunt_mission_progress` — per-user per-mission progress, recomputed from accepted submissions on every review outcome; `completed_at` set once on crossing the goal target. `bonus_credited_at` is declared but never written (see Gaps).
...
3. Mission `bonus_points` are decorative. Points are a leaderboard ranking figure and have no connection to ServiceCredits (owner directive 2026-08-27), but a mission's bonus does not reach the leaderboard either: the score is `SUM(points_awarded)` over accepted submissions, which a mission completion never touches. `skills_hunt_mission_progress.bonus_credited_at` is declared and never written. Continuity §2.9's line about a service-credits ledger entry on completion is superseded and must not be implemented. The `locked` mission status is display-only too — no locking-condition logic exists.
