...
5. Auto cohorts now carry a trainer split and a milestone skeleton, but a trainer payout only settles when a milestone is released against an enrollment whose `assigned_trainer_id` is set. The enroll path does not yet copy a claimed auto cohort's trainer (its `created_by_user_id` after `claim-trainer`) onto new enrollments, so trainer payouts will not fire for auto cohorts until that wiring is added. Tracked as a follow-up; does not affect the learner deposit/return or completion-bonus paths.
