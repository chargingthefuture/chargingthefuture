...
| Web Community Planning | ✅ Delivered | `workforce-community-planning.tsx`, reached from the sidebar **Community Planning** view (desktop) / **Community** tab (mobile-responsive) in `workforce-shell.tsx`. Renders the ten planning teams from [issue #1465](https://github.com/chargingthefuture/chargingthefuture/issues/1465), each expandable to its matched-member roster (reusing `workforce-member-list.tsx`), with the team's demand gap and any unmapped sector flagged. Bound to `GET /reports/community-planning`. Read-only; recomputes live. |
