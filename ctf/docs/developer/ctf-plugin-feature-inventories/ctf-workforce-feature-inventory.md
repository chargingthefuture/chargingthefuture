...
1. A tab at the end of the Workforce tab row (after Community), signed-in only. A member sees their own figures and nobody else's, and nobody sees theirs.
2. A card built from the member's own claimed Directory listing: initials, first and last name, occupation, and the skills on the listing (including skills the member added that are not yet in the shared catalog). No claimed listing yet means an unclaimed card that asks the member to claim their Directory listing, rather than a guessed name.
3. The reverse of the overview's even split. The overview divides a thriving economy's output evenly across everybody; this starts from one percent of the population estimate — 50,000 people out of 5,000,000 — and works back to what reaching that many people would mean for the person serving them.
4. Weighted per trade, from data the model already holds. The demand model says how many practitioners of the member's occupation a population this size needs (`WorkforceOccupation.target`); inverting it gives how many people one practitioner serves when the trade is staffed normally, and how many times that natural load reaching 1% would be. A trade the model needs many of serves fewer people each, so two members with different occupations listed see different figures. Nothing is authored for this — it is derived from the same demand model the Overview tab shows. Limit, and it is upstream data rather than this tab: Skills Taxonomy gained a per-occupation weight on 2026-09-14 (Gaps item 2, resolved) but ships with none set, and an unweighted occupation counts as the neutral 1 — so a sector still splits evenly today. This separates trades in different sectors; two occupations inside one sector share a figure until somebody weights them, and the tab sharpens on its own as weights are added.
5. The member's own two numbers, entered on the screen: what they charge for one job, and how many times one person needs them in a year. The second is the weight no table in the app carries — a trade called once every few years and one called monthly cannot share a rate, and the member knows theirs. Expressed as a rate rather than a count so less than once a year stays expressible (0.5 is once every two years). Held in the browser only: nothing is saved, sent, or stored.
6. A rate ladder at round numbers as a reference: $5, $20, $50, $100 and $250 per person per year, each across those 50,000 people and as a multiple of the even-split average earnings. $5 a person comes to $250,000, which is 3.5× the average.
7. A fixed list of five routes for reaching that many people, each naming the part of the app that carries it and linking to it: Foundation (do the work in person, or travel to it), Foundation again (the same question taken by call instead), PeerProgramming (work a problem live with a group watching), SkillUp (take apprentices, teach the trade), Knowledge Library (write it down once). The list is fixed rather than generated per trade, so it can only ever name surfaces that exist.
8. The screen says plainly that the figures are speculative and are not a forecast of what anybody will earn — they are arithmetic on a stated share of a population estimate, and talent, work and luck decide the rest.
...
8. `workforce.report.occupations.fetch` (per-occupation training gaps — the SkillUp recruiting/training signal)
9. `workforce.onePercent.fetch` (the caller's own card plus the 1% arithmetic, rate ladder, and route list — self-scoped, read-only, nothing stored)
...
- `GET /api/workforce/reports/occupations` — per-occupation training gaps (supports `?limit=`); the breakdown that later signals SkillUp which cohorts to stand up
- `GET /api/workforce/reports/community-planning` — read-only roster overlay for the survivor-built community planning document ([issue #1465](https://github.com/chargingthefuture/chargingthefuture/issues/1465)), scoped to a community that runs without outside services. Returns the planning teams — the ten from that document plus three added so every live taxonomy sector is drawn from — each a named union of Workforce sectors, with the team's de-duplicated matched-member roster (reusing the sector drilldown's 3-way match), the summed demand target/recruited/gap of its sectors, and any team sector not currently present in the Skills Taxonomy (`missingSectors`). Nothing is stored; recomputes live from the model on every call, so it updates as the Directory changes. Same read gate as the other reports, so member names are only ever returned to a signed-in member — never published publicly.

- `GET /api/workforce/one-percent` — the caller's own "What's Your 1%" card and the arithmetic beside it. Self-scoped: the Directory listing is read by the caller's own user id through `lib/shared/directory-interface`, and no member id is accepted from the client, so there is no way to ask for anybody else's. Returns `card` (initials, first/last name, occupation, skills, and an `isUnclaimed` flag when there is no claimed listing), `reach` (population, the one percent of it, the activity those people generate between them at the benchmark, and the even-split contribution/earnings averages carried over for comparison), `tradeLoad` (the per-trade weight — how many practitioners of the caller's occupation the model needs, how many people one of them serves, and how many times that 1% would be; `null` when there is no claimed occupation or the model carries no demand for it), `ladder` (the reference rate rows), and `routes` (the fixed five). The population comes from `getWorkforceConfig()` rather than the module default, so the figures move with the admin-set population model instead of drifting from it. Computed live on every call from `lib/workforce/one-percent.ts`; nothing is stored and no new table exists. 503 on persistence failure.
...
2. ~~Per-occupation demand is split evenly across a sector's job titles (no per-occupation weight exists in Skills Taxonomy). If finer weighting is wanted, it would need a new upstream signal.~~ Resolved (2026-09-14) — `skills_taxonomy_job_titles.workforce_share` now carries a per-occupation weight and `buildJobTitleDemand` splits by it (`lib/workforce/demand-split.ts`). Weights are relative within a sector, not percentages, and an unweighted occupation counts as 1, so a sector nobody has weighted still splits evenly and no live figure moved on the day the column landed. Remaining work is data rather than code: a weight is set one occupation at a time through the taxonomy change list (`setOccupationWorkforceShare`), each entry carrying a written rationale for where the number came from. Until an occupation is weighted, its figure is still the even split — accurate to say, and visible in the model rather than hidden.
...
4. The retained-but-unused `workforce_occupations` / `workforce_export_jobs` tables and the vestigial `workforce_profiles` / `workforce_recruited_events` / `workforce_recruited_sync_cursor` tables are dead weight in the schema. The SkillsHunt rare-skill snapshot stopped reading `workforce_profiles`/`workforce_occupations` on 2026-08-27 (it now reads the live gap model through the sanctioned interface); the remaining references are the demo seed (`workforce_occupations`) and the account deletion registry (`workforce_profiles`), so dropping the tables is a separate follow-up once those are updated.
...
| Web What's Your 1% | ✅ Delivered | `workforce-one-percent.tsx`, reached from the **What's your 1%?** tab at the end of the tab row in `workforce-shell.tsx`. Renders the member's own card, the per-trade weight, the two member-entered inputs (browser-only, nothing stored), the reference rate ladder, and the five routes. Bound to `GET /api/workforce/one-percent`. Read-only; recomputes live. Web-only per rule 105. |
...
- 2026-09-14: **Added "What's Your 1%", a tab that answers the question the rest of Workforce does not.** Every other tab describes the population. A member can read all of it and still conclude that none of it is about them — what was taken from them was taken inside another economy, and it is an easy step from there to deciding the skill went with it. This tab runs the overview's arithmetic backwards for the member reading it: instead of splitting a thriving economy's output evenly across everybody (`WORKFORCE_BENCHMARK_GDP_PER_PERSON_USD`, the Singapore PPP benchmark already on the overview), it starts from one percent of the population estimate — 50,000 of 5,000,000 — and works back. A rate ladder shows what $5, $20, $50, $100 and $250 per person per year comes to across that many people; $5 is $250,000, which is 3.5× the even-split average earnings. The even split is an average rather than a ceiling, because most people do not work full time, for every reason a person has. Because no trade reaches 50,000 people one job at a time, the tab names five routes for the same skill and links each to the part of the app that carries it — Foundation in person, Foundation by call, PeerProgramming live with a group, SkillUp teaching, the Knowledge Library written once. That list is fixed rather than generated per trade: generating it would invent plausible-sounding routes the app does not have. The card is the member's own claimed Directory listing and only ever theirs — the profile is read by the caller's own user id through `lib/shared/directory-interface` (a new `getOwnProfile` re-export, dated in that file), and no id is accepted from the client; an unclaimed listing shows an ask to claim one rather than a guessed name. One new route (`GET /api/workforce/one-percent`), one new command contract and access policy entry, `lib/workforce/one-percent.ts` with ten tests, `workforce-one-percent.tsx`, and the tab wired into `workforce-shell.tsx`. **No schema change and nothing stored** — every figure is computed on read. The `reach.activityTheyGenerateUsd` field is named that way on purpose: it is what those 50,000 people contribute between them, not what the member serving them earns, and presenting it as personal income would be a thousandfold overstatement. A test asserts it. The screen closes by saying the figures are speculative and not a forecast of anybody's earnings. **Revised the same day, on owner review:** the first cut showed every member identical figures, which contradicted the tab's own premise — it claims to be about the person reading it, and two members with different trades listed were handed the same answer. Two weights were added, neither of them invented. The first is derived from the demand model the Overview already computes: `WorkforceOccupation.target` says how many practitioners of an occupation a population this size needs, and inverting it gives how many people one practitioner serves, plus how many times that natural load reaching 1% would be. Authoring a frequency figure for each of ~650 occupations was rejected as exactly the plausible-but-wrong data the fixed route list exists to avoid. That weight separates trades in different sectors; it does not yet separate two occupations inside one sector, because Skills Taxonomy carries `workforce_share` per sector and no weight per job title — already recorded as Gaps item 2, and a per-job-title weight upstream would sharpen this with no change here. The second weight is how often one person needs a given trade, which exists nowhere in the app: the member enters it alongside what they charge for one job, held in the browser only, nothing saved or sent. It is a rate rather than a count so a trade engaged less than once a year stays expressible. The route now reads the population from `getWorkforceConfig()` instead of the module default. Six further tests; the `GET` handler was split into `buildCard` and `loadTradeWeight` to stay inside the complexity limit.
- 2026-09-14: **Per-occupation demand is weighted instead of split evenly (Gaps item 2, resolved).** Sectors have carried a `workforce_share` since the model was built; occupations never had one, so `buildJobTitleDemand` divided a sector's demand by its job-title count and gave every occupation the same number. That asserted a settlement needs as many Surveyors as Electricians because both sit under Housing & Construction. The flatness reached everything downstream: the per-occupation training-gap report that tells SkillUp which cohorts to stand up, the occupations browse ordering, and every per-trade figure on the Workforce screens. `skills_taxonomy_job_titles` gains a nullable `workforce_share`, and the split moved to a new pure module `lib/workforce/demand-split.ts` (no database, ten tests) so the weighting can be exercised directly. Weights are **relative within a sector, not percentages** — 3 against 1 means three times as many people — so one occupation can be weighted without restating its siblings, which is what makes the append-only change list a workable way to set them. **An unweighted occupation counts as 1**, so a sector nobody has weighted produces the even split it already produced and no live number moved when the column landed; a part-weighted sector also behaves, because the unset rows sit at the neutral value rather than at zero. An explicit `0` is honored and means the model expects nobody in that occupation, which is a different statement from never having set one; a sector weighted entirely to zero falls back to the even split rather than dividing by zero. Setting a weight goes through the taxonomy change list only (new op `setOccupationWorkforceShare`, validated by the taxonomy-change-gate, applied by the owner-run workflow) — deliberately not an admin form, because the admin write routes were removed in 2026-08-28 precisely to stop a second ungoverned write path. Each entry requires a written `rationale`: a demand weight is an assertion about how many people a settlement needs in a trade, and an unsourced one is worse than the even split it replaces because it looks researched. **No weights are set in this change.** The apply handler records its audit row with `action: 'update'` rather than a new verb, because `skills_taxonomy_change_events.action` is check-constrained and a value outside that list is rejected at apply time and rolls the entire run back.
- 2026-08-29: **Community Planning now plans for a community that runs without outside services (owner direction).** The ten teams transcribed from issue #1465 describe a gated community that still buys most of what it needs from outside — municipal water, the grid, schools, courts, shops, factories. That is the easy case, and planning for it does not plan for the hard one; planning for the hard one covers both. Scoping the teams to a settlement that can stand on its own makes a simply gated community a subset of the same plan rather than a separate one. **Nine of the twenty live taxonomy sectors had no team at all**: Water & Sanitation, Transport & Logistics, Telecommunications & IT, Education, Public Safety & Justice, Finance & Public Administration, Manufacturing & Industry, Mining / Extractive, and Tourism & Hospitality. A member whose only skills sat in one of those was invisible to every roster and their sector's demand gap was never counted in any team total. Two gaps contradicted the teams' own descriptions: **Build & Infrastructure** claimed responsibility for "water" and "connectivity" while drawing from neither Water & Sanitation nor Telecommunications & IT, and **Food & Agriculture** claimed "kitchen/commons planning" while every Chef / Cook lives in Tourism & Hospitality. Three teams added for capacities with no home at all — **Water & Sanitation** (running the supply after Build installs it: sources, treatment, testing, distribution), **Education & Childcare** (school-age lessons, tutoring, childcare, and training adults into the skills other teams are short of — the sector the taxonomy just gained Childcare Workers and Tutors in), and **Making & Repair** (fabrication, machining, spare parts, materials processing). Six widened: Legal & Governance +Public Safety & Justice, Finance +Finance & Public Administration, Land & Site +Mining / Extractive, Build & Infrastructure +Water & Sanitation +Telecommunications & IT, Food & Agriculture +Tourism & Hospitality, Safety & Security +Public Safety & Justice, Technology +Telecommunications & IT, Operations & Maintenance +Transport & Logistics. All 20 sectors are now drawn from by at least one team, and every sector name was checked against the live taxonomy so none renders as `missingSectors`. Sectors deliberately appear in several teams — as Housing & Construction always has across Land & Site, Build and Operations — because building a thing and running it afterwards are different jobs from the same trade; rosters de-duplicate members, so overlap costs nothing. Definitions only (`lib/workforce/community-planning.ts`) plus the report blurb and the command description. No schema, no route, no response-shape change: the report is computed from these definitions on every call, so the new teams appear without a migration or a backfill. Android needs no change — its Community tab renders whatever the API returns and hardcodes no team.
...
- 2026-08-16: **Added the Skills Coverage tile to the dashboard overview (fourth hero card).** Shows
  the integer percentage of the skills taxonomy with at least one active Directory member
...
- 2026-08-24: **The Workforce screen now scrolls the page itself, so Safari's "Full Page"
  screenshot captures the entire screen (owner report).** The shell used to be pinned to exactly one
