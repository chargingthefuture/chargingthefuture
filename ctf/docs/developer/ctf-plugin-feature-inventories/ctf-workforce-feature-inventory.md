...
- 2026-06-29: Dropped "Not Recruited" from the public Workforce snapshot (UI + endpoint). Against the 5M goal the unfilled-headcount-target figure is a multi-million number that dwarfed the Recruited bar and read as off-putting marketing on the signed-out landing. `GET /api/workforce/public-snapshot` now returns only `{recruited, sectorGaps, generatedAtIso}` and the landing card shows just Recruited + Sector Gaps. No schema change.
