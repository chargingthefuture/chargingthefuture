...
- 2026-06-26: Fixed the dashboard returning **503** ("Dashboard request failed (503)." on the Workforce screen). `GET /api/workforce/dashboard` was running the full `computeWorkforceModel()` — including the V2 per-bucket supply match (the per-member expansion across every occupation in a member's sector and the `DISTINCT directory_profile_skills → skills_taxonomy_skills` join) — even though the dashboard returns only top-line totals. At production data scale that extra work could exceed the DB statement timeout and throw, which the route turns into a 503 that blanks the whole page (the shell hard-fails when the core dashboard fetch is non-OK). `getDashboard()` now uses a dedicated lightweight read: config + sector demand + two `COUNT`s (active job titles, active profiles). The sector-demand math is shared with the full model via a new `buildSectorDemand()` helper, so `totalHeadcountTarget` can never diverge from the sector reports; `recruitedTotal` stays the count of all active profiles (V2 parity), identical to before. The report routes still use the full model. No schema, route surface, or contract-field change; the dashboard response shape is unchanged.
