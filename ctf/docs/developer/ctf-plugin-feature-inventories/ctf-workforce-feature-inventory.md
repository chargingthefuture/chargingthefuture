...
- `POST /api/workforce/export/jobs` — body `{ exportType }` is validated against the allowed whitelist (`summary`, `skill-level`, `sector`); an unknown value is rejected with `WORKFORCE_INVALID_PAYLOAD` / 400 before any job row is written. Records a `deferred` job (`completed_at` left NULL) and returns `501 WORKFORCE_EXPORT_DEFERRED`. Emits both the queryable admin-mutation audit row and the structured `workforce.export.job.create` audit event, each carrying the full evidence object the audit contract requires (`roleCheck`, `exportScopeCheck`, `piiGuardCheck`, `defermentCheck`).
...
4. Export job execution/result is intentionally deferred (per phase-1 product decision): `POST /api/workforce/export/jobs` records the request and `GET /api/workforce/export/jobs/[jobId]/result` return `501 exportDeferred`. The deferred job row is written with `completed_at` NULL (it has not completed), the `exportType` is validated against the allowed whitelist (`summary`, `skill-level`, `sector`) first, and both the admin-mutation audit row and the structured audit event are written with the contract-required evidence (`roleCheck`, `exportScopeCheck`, `piiGuardCheck`, `defermentCheck`); actual artifact generation is post-MVP.
