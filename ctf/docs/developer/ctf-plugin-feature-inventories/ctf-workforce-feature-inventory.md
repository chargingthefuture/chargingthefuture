...
| Android Community member → profile link | ⏳ Parity ticket [#1615](https://github.com/chargingthefuture/chargingthefuture/issues/1615) | The web Community tab links each member name to their Directory profile; Android `MemberCard` names are still plain text. The mobile app has no in-app navigation path from Workforce to a Directory profile screen yet, so wiring it is deferred to its own PR (see #1615). |
