...
4. A rate ladder: $5, $20, $50, $100 and $250 per person per year, each shown across those 50,000 people and as a multiple of the even-split average earnings. $5 a person comes to $250,000, which is 3.5× the average. A member reads across the row nearest what they already charge rather than being handed a forecast.
5. A fixed list of five routes for reaching that many people, each naming the part of the app that carries it and linking to it: Foundation (do the work in person, or travel to it), Foundation again (the same question taken by call instead), PeerProgramming (work a problem live with a group watching), SkillUp (take apprentices, teach the trade), Knowledge Library (write it down once). The list is fixed rather than generated per trade, so it can only ever name surfaces that exist.
6. The screen says plainly that the figures are speculative and are not a forecast of what anybody will earn — they are arithmetic on a stated share of a population estimate, and talent, work and luck decide the rest.
...
| Web What's Your 1% | ✅ Delivered | `workforce-one-percent.tsx`, reached from the **What's your 1%?** tab at the end of the tab row in `workforce-shell.tsx`. Renders the member's own card, the rate ladder, and the five routes. Bound to `GET /api/workforce/one-percent`. Read-only; recomputes live. Web-only per rule 105. |
