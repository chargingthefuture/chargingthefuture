...
The canonical entities live in `skills_taxonomy_sectors`, `skills_taxonomy_job_titles`, and `skills_taxonomy_skills`. Both `skills_taxonomy_sectors` and (since 2026-09-14) `skills_taxonomy_job_titles` carry a nullable `workforce_share`: the sector's is its share of the working population, the job title's is its weight **relative to the other occupations in the same sector** rather than a percentage. NULL means nobody has set one and Workforce counts it as 1, so an unweighted sector splits evenly exactly as it did before the column existed. A job title's weight is set only through the change list (`setOccupationWorkforceShare`), never by an HTTP write. Three further tables back the read model, the dependency tracker, and the audit log:
...
- 2026-09-20: **Medical Assistants joins Health, and Social Workers gains domestic violence advocacy (changes 80–92).** A conversation with a member whose working life is a medical assistant's named thirteen things they can do, and a read of the live Health sector settled where they belong. Health holds thirteen occupations — Community-Health Workers, EMS / Paramedics, General Practitioners, Laboratory Technicians, Mental Health Counselors, Midwives, Nurses, Pharmacists, Psychologists, Radiographers, Social Workers, Specialists (e.g., cardiology, pediatrics), Therapists — and not one is a medical assistant. Nurses is the nearest row and the wrong one; the member drew that distinction themselves, and a medical assistant working a psychiatry clinic is neither a nurse nor a counselor. So the role gets its own occupation, as Childcare Workers and Tutors did under Education in changes 58–67, rather than being hung off a role it is not. Op 80 creates **Medical Assistants**; ops 81–91 seed it with **Phlebotomy (blood draws)**, **Injections (intramuscular and subcutaneous)**, **Vital signs measurement**, **Patient intake and registration**, **Patient assessment and monitoring**, **EKG (electrocardiogram) testing**, **Splinting, casting and orthopedic wraps**, **Urgent care clinic support**, **Psychiatric clinic support**, **Patient advocacy** and **Patient communication**, in the same apply run so the occupation is not inert — Workforce matches holders by skill name, and a skill-less occupation matches nobody and shows empty in the browser. This is the first change at or past id 80, so it is the first the plural-twin guard applies to: `Medical Assistants` was checked against all thirteen live Health occupations and shares a role token with none. Two of the eleven reuse a live label **exactly** rather than inventing a synonym — `Patient assessment and monitoring` (live under Nurses) and `Patient communication` (live under General Practitioners). Duplicating a label across occupations is this sector's established shape, not an oversight: `Crisis intervention` already has four rows, `Trauma-informed care` two, `First Aid & CPR` two. One name keeps keyword search and Workforce's name matching joined, and the separate row is what makes the skill show when somebody browses that occupation. The second row does **not** fix attribution, and being exact about that matters: Workforce ignores the stored row's parent entirely — its skill arm re-expands by name across every same-named active row before counting — and the Directory picker collapses same-named rows into one chip and stores an arbitrary representative id, so the parent a member ends up attached to was never a choice they made. Within one sector that costs nothing, which is the case here (both reused labels stay inside Health); across sectors it would not, because the Directory sector filter does read the stored row's parent. Writing `Patient assessment` beside the live `Patient assessment and monitoring` is the failure to avoid, because a second **name** for one claim splits its holders (the harm changes 26–34 and 79 had to undo). Op 90, **Patient advocacy**, is the set's one judgment call and is flagged in the change comment rather than buried: Social Workers already carries `Client advocacy`, the same act in social-work vocabulary, so the label is proposed under Medical Assistants precisely to keep the two out of one occupation as the near-duplicate pair change 79 had to thin — dropping op 90 and having members pick `Client advocacy` is the alternative if one label is preferred. Op 92 adds **Domestic violence advocacy** to the pre-existing **Social Workers** occupation (`occupationExisting: true`); no live label says it, `Client advocacy`, `Crisis intervention`, `Family assessment and intervention` and `Trauma-informed care` each being adjacent and none of them naming it, and it sits there rather than under Medical Assistants because it is advocacy work in its own right. Two items from the same conversation are deliberately not proposed: **single mom**, a life circumstance already recorded as a title and not something a settlement's demand can be matched against, and **interacts with people easily**, too general to match on and carried concretely by op 91. No `proposalNormalizedSkills` is set — the request came from the owner directly, not from the `skill-proposal` intake queue, so there is no member proposal row for the apply run to mark promoted. Appended to `ctf/scripts/lib/taxonomyChange.mjs`; additive, applies on the next owner run of the seed-skills-taxonomy apply workflow. Data only — no schema, route, or contract change.

- 2026-09-14: **Occupations can carry a demand weight (`setOccupationWorkforceShare`).** Workforce split a sector's headcount evenly across the occupations under it because the taxonomy offered no weight to read — recorded as Gaps item 2 in the Workforce inventory and inherited by the SkillUp cohort signal, the occupations browse ordering, and every per-trade figure. `skills_taxonomy_job_titles` gains a nullable `workforce_share` alongside the one sectors have always had. The new change type takes `{ sector, occupation, share, rationale }`: `share` is relative within the sector rather than a percentage (3 against 1 is three times as many people), so one occupation can be weighted without restating its siblings. NULL stays the default and counts as 1 downstream, so the column changed no figure anywhere on the day it landed. `rationale` is mandatory and mirrors `acknowledgedImpact` on deactivations — a demand weight asserts how many people a settlement needs in a trade, and an unsourced number is worse than an even split because it looks researched; requiring the note is what keeps a weight list from being filled in wholesale by anybody who cannot say where the numbers came from. Re-weighting appends a new entry and the last one wins; a re-set to the current value no-ops. Deliberately **not** an admin form: the admin write routes were removed on 2026-08-28 to stop exactly that second ungoverned write path, and this change adds none back. The apply handler writes its audit row with `action: 'update'` and puts the detail in `reason`/`metadata`, because `skills_taxonomy_change_events.action` is check-constrained and an unlisted verb is rejected at apply time and rolls the run back. **No weights are set in this change** — the mechanism ships empty, and values arrive one reviewed entry at a time.
