...
- `ctf/scripts/seedSkillsTaxonomy.mjs` (the `seed:skills-taxonomy` script) now applies **only** the curated promotions against the live DB; it no longer backfills from any legacy dataset.
...
- Curated, owner-approved skill promotions are the only seed path that writes the taxonomy. The list lives in `ctf/scripts/lib/seedSkillsTaxonomyPromotions.mjs` and is applied by `ctf/scripts/seedSkillsTaxonomy.mjs` (the `seed:skills-taxonomy` script) against the live DB, so every reseed keeps promoted occupations and skills. To add an owner-approved skill, append an entry and run the seed. Each promotion looks the sector up by name in the live DB (never creates a sector), upserts the occupation under it, then upserts each skill under that occupation; every write is `ON CONFLICT` no-op so re-runs are safe. It also marks the matching `skills_hunt_proposed_skill_promotions` row `status = 'promoted'` (and auto-attaches the now-official skill to any Directory member who proposed it) when present. Data/script only — no schema change.
