...
- `GET /api/click-log` — List incidents for authenticated user
- `POST /api/click-log` — Create incident
- `DELETE /api/click-log/[id]` — Delete incident by id
...
- Every allowed operation emits an audit event (`click-log.incident.create`/`.list`/`.delete`)
