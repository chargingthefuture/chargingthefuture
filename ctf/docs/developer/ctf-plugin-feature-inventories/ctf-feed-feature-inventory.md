...
>   `/apps/[pluginSlug]` route branch, the `feed` / `announcements` aliases, and the entire
...
- `GET /api/feed/public/community` — read-only Commons for signed-out visitors. Returns `{ isPublic, posts }` where `posts` are community (peer) posts only — no announcements, AI answers, replies, per-user state, or author user ids. Returns `isPublic: false` (empty) unless `feed_render_config.is_public` is on and the community channel is enabled. Backs the signed-out home panel (community posts are public the way Quora posts are; visitors read but cannot post without signing in). Each post carries `image` (`{ url, alt, width, height }` or null): the picture an admin attached, served signed out by `GET /api/commons/images/:postId` under the same public-viewing rule (2026-09-25, see the Commons inventory). The on/off check is `isPublicCommunityReadOn`, shared by both routes. Rate-limited per IP (2026-07-16): 30 requests/minute via the shared in-memory limiter `lib/security/rate-limit.ts`; over-limit callers get `429` with a `Retry-After` header.
...
- 2026-08-18: **Added 💯 to the Commons reaction quick set.** The fixed reaction set gains the hundred-points mark, so it is now 👍 ❤️ 😂 🎉 🙏 😢 👋 💯. Appended at the end of `FEED_REACTION_EMOJIS` (`lib/feed/constants.ts`) so existing reactions keep their order/rank; the set is shared by server and client, so the server accepts 💯 and the picker offers it on both community posts and announcements. Enums + descriptions in `FEED_PLUGIN_COMMAND_CONTRACTS.yaml` (`feed.community.post.reaction.toggle` and `feed.announcement.reaction.toggle`) updated to match, along with the picker step in `feed-test-script.md`. No schema change. Web + mobile-responsive (Commons is web-only). Verified: `@ctf/web` typecheck + eslint clean.
...
- 2026-07-30 (later): **Three standing Commons notices, three cadences.** The single notice became a
  ...
  `(notice_key, milestone_count)` — that composite UNIQUE is still the entire concurrency story.
...
- 2026-07-30: **The Commons states its own purpose every 50 posts (owner decision).** A newcomer now
  ...
  - **Every paragraph of the copy is load-bearing and was corrected by the owner (2026-07-30).** The
    ...
    - **Why it is open**: the design reason is that the owner takes no direct messages — her inbox was
      ...
      entire community an account of what was done to her.
    ...
    - **The public rule is TOPIC, not character**: content is removed for repeatedly going nowhere,
      never for who somebody is suspected of being. An accusation posted to an entire community cannot be
...
- 2026-07-29 (later): **Moderation reason + moderating by member (owner: the real problem is volume of
  ...
  `off_topic` in the UI because that is the actual day-to-day judgment — one picker for the entire
  ...
  someone should not require reading everything they wrote. `?author=` then shows that member's entire
