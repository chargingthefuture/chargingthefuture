...
- 2026-07-05: **Listing price/currency/type display polish (web + mobile-responsive).** No schema, route, or contract change:
  - Browse card and listing detail render the rent with the amount large and a long currency label (e.g. "ServiceCredits") small, instead of the entire string at the price font size — a ServiceCredits price no longer overflows or breaks mid-word at phone width (`formatRentParts` in `shared.ts`).
...
- [x] Implement blocks UX parity. **Done 2026-08-03.** The listing detail carries the shared "Block
      ...
      entire product at `/account/blocks` rather than being rebuilt per plugin.
...
- 2026-09-18: **Stream failures say what failed and why (cross-plugin pass).** The match chat route's
