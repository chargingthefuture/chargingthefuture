...
5. `service_credits_disputes` has no resolution/closure state (no `status`/`resolved_at` column, and nothing marks a dispute resolved), unlike the level-up and trust-transport dispute tables. So the admin `openDisputes` metric counts every dispute ever opened. A dispute-closure model (mark resolved when an adjustment is applied or an operator closes it) is the follow-up that would let `openDisputes` reflect only unresolved cases (GitHub #1479).
