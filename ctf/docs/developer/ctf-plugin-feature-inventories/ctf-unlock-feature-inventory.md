...
6. Duplicate-identity guard is web + backend only; the React Native admin (`AdminUnlock.tsx`) now has Pending/Approved/All status tabs (so approved-but-uncredited rows are visible) and surfaces `withheld`/`errors` in the reconcile summary, but it still does not surface the per-row withheld/revoked badges or the grant/revoke determination actions (Android parity follow-up).
