...
4. A `spam` decision blocks the member from the entire app, not just the Unlock tier: in addition to
...
- 2026-08-24: **The spam denylist panel opens on demand instead of always being open (owner report).**
  ...
  title, how many URLs are on the list, and a chevron (`aria-expanded` / `aria-controls`, entire row is
