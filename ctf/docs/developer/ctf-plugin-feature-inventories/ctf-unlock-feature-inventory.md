...
- 2026-09-18: **One Quora profile could sign up three times without anything noticing (owner
  ...
  now matches what the function produces. Not changed: the Directory and SkillsHunt normalizers
  (`lib/directory/quora-url.ts`, `lib/skills-hunt/repository.ts`) have the same weakness on their own
  keys and are deliberately a separate decision — they accept any `quora.com` path, so their
  canonical form is not this one.
