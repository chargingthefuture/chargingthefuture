...
- **Android:** delivered (pixel pass, 2026-05-31) for the search/quote/Direct Line surface. The instant 1:1 call ring/answer lifecycle, its per-block billing, and its Web Push ring delivery (issue #808 tasks 3, 4 and 5) are **web + mobile-responsive web only**; React Native parity (including Expo native push) is deferred to the issue #808 Android follow-up. The mobile feature binds the real Foundation API — see below.
...
8. **Android/Expo native push for the instant-call ring is deferred to the #808 Android parity ticket.** Web Push (web + mobile-responsive web) is delivered (task 5); the React Native app has no instant-call surface yet, so native push there is premature. The `push_subscriptions` table is intentionally device-kind-agnostic (`kind` defaults to `'web'`, with room for `'expo'`) so an Expo push token can be added later without a schema change.
