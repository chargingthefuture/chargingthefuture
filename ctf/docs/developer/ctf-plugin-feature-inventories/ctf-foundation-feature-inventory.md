...
- `POST /api/foundation/connections/threads/:threadId/instant-call` — place an instant 1:1 audio call ring on the thread (Foundation "Connect now", issue #808 task 3). Rings the other thread participant; rate limited; returns the created call in `ringing` state. In-app only (push is task 5); no billing (task 4).
- `GET /api/foundation/connections/instant-calls/:callId` — poll a call's state for either participant; when answered, also returns the participant-only Stream audio-join credentials (same token path as the Direct Line). Realises the ~60s ring timeout lazily.
- `POST /api/foundation/connections/instant-calls/:callId/answer` — callee answers a ringing call (the seam where task 4 takes the first-block charge; no money moves in v1).
- `POST /api/foundation/connections/instant-calls/:callId/decline` — callee declines a ringing call (terminal).
- `POST /api/foundation/connections/instant-calls/:callId/end` — either participant ends/cancels the call (terminal; idempotent).
...
- **Android:** delivered (pixel pass, 2026-05-31) for the search/quote/Direct Line surface. The instant 1:1 call ring/answer lifecycle (issue #808 task 3) and its Web Push ring delivery (issue #808 task 5) are **web + mobile-responsive web only**; React Native parity (including Expo native push) is deferred to the issue #808 Android follow-up. The mobile feature binds the real Foundation API — see below.
...
The instant 1:1 call ring/answer lifecycle (issue #808 task 3) added ring columns to `foundation_call_sessions` but seeds **no** call rows — a call is an ephemeral, member-initiated runtime event, not seed content, so there is no deterministic ring to seed. `seedFoundation.mjs` is unchanged.
...
5. **Android/Expo native push for the instant-call ring is deferred to the #808 Android parity ticket.** Web Push (web + mobile-responsive web) is delivered (task 5); the React Native app has no instant-call surface yet, so native push there is premature. The `push_subscriptions` table is intentionally device-kind-agnostic (`kind` defaults to `'web'`, with room for `'expo'`) so an Expo push token can be added later without a schema change.
6. **Web Push requires the owner to provision VAPID keys.** Until `VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` / `VAPID_SUBJECT` are set (the owner generates them once with `npx web-push generate-vapid-keys` and stores them in Infisical `production`), the ring is delivered in-app only — every push is a graceful no-op. No keys are generated or committed in the repo (open-source secrets policy). See `.claude/rules/123-environment-configuration-rules.mdc`.
