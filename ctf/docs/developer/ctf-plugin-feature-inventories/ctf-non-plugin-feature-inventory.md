...
7. **`GET /api/admin/daily-exchange`** — admin read, read-only, no parameters, no writes. Returns `{ reading, weavers }`. `reading` carries `readAt`, `target` (384), `today`, `bestDay` (`{ day, members }` or null, read across every day on record rather than the window on screen), `daysAtTarget`, `days` (30 entries of `{ day, members }`, quiet days filled in as zero so a gap is not mistaken for missing data), and `todayRoster` (member id and that day's score, at most 500). `weavers` carries `holders` and the eight most recent earners as `{ username, firstEarnedAt }`. No per-event breakdown is returned for a named member: Foundation answered charged calls are among the thirteen and rule 132 keeps that participation internal.

8. **Screen `/admin/daily-exchange`**, linked from the admin directory at `/admin`. Four stat blocks (today, target, best day, days at or above the target), one bar per day drawn against the target rather than against the tallest day so a quiet run does not rescale into looking busy, then two widgets the owner reviews together: Weavers of the Commons (holders, and the most recent earners) and Delivering today (the roster, ordered by that day's score). A control copies the reading as plain text for pasting from a phone. A closing note names the shared definition, the attribution, and the two proxy dates.
