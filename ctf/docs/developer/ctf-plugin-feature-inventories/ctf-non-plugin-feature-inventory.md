...
1. **One definition, two readings — `ctf/packages/web/lib/contributor-access/value-events.ts`.** Owner directive: the weights and the event set must be identical to Weavers of the Commons and must stay in sync as features are added or change, because they are the same measure asked over different dates. So the fifteen value events now live in one registry — for each, the tables it touches, a row-level SELECT carrying the member it is attributed to and when it happened, and how it aggregates (a count, a sum of dollars for Contributions, distinct weeks for PeerProgramming, distinct broadcasts for Beacon). `lib/contributor-access/member-value-counts.ts` derives its all-time per-member queries from that registry instead of carrying its own copies, and the daily reading derives a per-day one. Adding a value event is one entry and both readings pick it up.
