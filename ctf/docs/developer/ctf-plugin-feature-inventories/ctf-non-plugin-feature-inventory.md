...
### 1.15 Invitations Strip (public, replaces the reviews popup)
...
1. **What shows instead — `ctf/packages/web/components/invites/invites-strip.tsx`** (client). A row of the blog's published invite posts, one card each, newest first, rendered first on every public (signed-out) plugin page from the single public branch of `/apps/[pluginSlug]/page.tsx`. Each card carries the post's title, date, opening words and a link to the post on the blog.
2. **Data source.** `https://chargingthefuture.github.io/chargingthefuture/invites.json`, the blog's own build output (written by `wiki:invites` in `chargingthefuture/wiki-site` from every listed post titled "An invitation to <name>"), served by GitHub Pages with an open CORS header. Nothing in this repository decides what an invite is; the blog does. No route, no table, no config. The strip renders nothing when the file cannot be read or is empty.
3. **Why a row and not a popup.** The person an invite names is told on Quora, and Quora erases the account before most of them tap the notification; the blog is the address that survives, and a row of the invites they would have missed is what a newcomer needs first. Nothing in the row moves on its own: it scrolls by touch, by two buttons, or by keyboard, every card is in the page for a screen reader, and `prefers-reduced-motion` turns smooth scrolling off.
