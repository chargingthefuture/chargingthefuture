...
3. The policy copy is written to match actual product behavior and should be kept in sync when that behavior changes: the third parties that process member data (Clerk, GetStream, Formance, Sentry; hosting/push transports; Supabase document-storage-only), the non-cash/non-withdrawable nature of ServiceCredits (vs. the separate real-money TrustTransport payouts), the transaction-bound messaging model, what account deletion removes vs. retains (own content and own messages removed; money/ledger/audit/deletion-accountability records retained), the invite-only + verification access model, adults-only (18+) eligibility, and the data-request/rights process. Operator named as "Charging the Future" (unincorporated), governing law Delaware, contact `ctf.connected070@slmails.com`.
