...
7. **`GET /api/admin/daily-exchange`** — admin read, read-only, no parameters, no writes. Returns `{ reading, weavers }`. `reading` carries `readAt`, `target` (384), `today`, `bestDay` (`{ day, members }` or null, read across every day on record rather than the window on screen), `typicalDay` (median of the last 30), `yearMedian` (median of the last 365), `longestRun` (longest stretch of consecutive days at or above the target within the year), `daysAtTarget`, `days` (30 entries of `{ day, members }`, quiet days filled in as zero so a gap is not mistaken for missing data), and `todayRoster` (member id, the member's handle where they have set one, and that day's score, at most 500). `weavers` carries `holders` and the eight most recent earners as `{ username, firstEarnedAt }`. `reading.events` carries the definition itself — every weighted event with its label, plugin, the weight in force right now, and whether it counts toward the day or only toward the badge — so an admin can audit what is weighted without taking it on trust (owner directive, 2026-09-20). No per-event breakdown is returned for a named member: Foundation answered charged calls are among the thirteen and rule 132 keeps that participation internal.

8. **Screen `/admin/daily-exchange`**, linked from the admin directory at `/admin`. Four stat blocks (today, target, typical day, longest run), one bar per day drawn against the target rather than against the tallest day so a quiet run does not rescale into looking busy, then two widgets the owner reviews together: Weavers of the Commons (holders, and the most recent earners) and Delivering today (the roster, ordered by that day's score, each row showing the member's handle with their member id under it — the id alone was unreadable beside a badge list that has named its members by handle all along, and it stays because a member can use the app without ever setting a handle, owner directive 2026-09-20). The goal is stated under the chart: the median day over a year at or above the target. A median rather than a mean, because a mean is satisfied by a dozen enormous days and ten quiet months, which is not an economy anybody could rely on, and quiet days count as the zeros they are. Typical day is the same measure over thirty days, so it moves while the year figure is still filling; longest run counts consecutive days at or above the target and one quiet day ends it. The count of days that ever reached the target moved into the line above the chart, because on its own it says nothing about whether an economy lasts (owner directive, 2026-09-20).

A third widget, What is weighted, lists every weighted event read-only: one row per event, delivering events first, each with its weight and a mark where it reaches the badge only. A weight of zero is shown rather than hidden, because an event nobody has tuned is still part of the definition. A control copies the reading as plain text for pasting from a phone, and the copied text carries that list too. A closing note names the shared definition, the attribution, and the two proxy dates.
