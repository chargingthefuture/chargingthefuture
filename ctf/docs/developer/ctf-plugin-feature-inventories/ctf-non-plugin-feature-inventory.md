...
2. **Areas with no dot.** Areas that are read-only dashboards, config editors, authoring, or browse views never get a dot: directory, beacon, lighthouse, foundation, socket-relay, weekly-performance, workforce, feed-announcements, contributor-access, service-credits, level-up, peer-programming. (level-up disputes/validations, service-credits disputes, and peer-programming feedback are latent candidate signals but their admin page does not currently surface a queue, so a dot there would lead nowhere useful — left as a follow-up.)
