...
- API:
  ...
  - `GET /api/account/services/:slug/export` and `GET /api/account/full-account/export` (issue
    #1264) — download the member's own data as JSON, per service or entire account. Both gated by
...
### 1.15 Invitations Popup (public, replaces the reviews popup)

Owner decision, 2026-09-19. The corner reviews popup, the `/reviews` wall page, the `GET /api/reviews` route and the hand-maintained `lib/reviews/reviews-data.ts` list are removed. The list held one entry, whose source link died when Quora erased the account it pointed at, so every signed-out page showed one card linking nowhere. Nothing hidden and nothing kept inactive: the code and the entry are gone, and stay in git history.

1. **What shows instead — `ctf/packages/web/components/invites/invites-popup.tsx`** (client). One of the blog's published invite posts at a time, in a small card fixed to the bottom-left corner of every public (signed-out) plugin page, rendered from the single public branch of `/apps/[pluginSlug]/page.tsx`. The card carries the post's date, title, opening words (three lines, then clipped) and a link to the post on the blog; a counter and two buttons move through the rest, newest first.
2. **Data source.** `https://chargingthefuture.github.io/chargingthefuture/invites.json`, the blog's own build output (written by `wiki:invites` in `chargingthefuture/wiki-site` from every listed post titled "An invitation to <name>"), served by GitHub Pages with an open CORS header. Nothing in this repository decides what an invite is; the blog does. No route, no table, no config. The card renders nothing when the file cannot be read or is empty.
3. **Why the invites and not reviews.** The person an invite names is told on Quora, and Quora erases the account before most of them tap the notification; the blog is the address that survives, and the invites they would have missed are what a newcomer needs to see.
...
1. **One definition, two readings — `ctf/packages/web/lib/contributor-access/value-events.ts`.** Owner directive: the weights and the event set must be identical to Weavers of the Commons and must stay in sync as features are added or change, because they are the same measure asked over different dates. So the value events (fourteen as of 2026-09-25) now live in one registry — for each, the tables it touches, a row-level SELECT carrying the member it is attributed to and when it happened, and how it aggregates (a count, a sum of dollars for Contributions, distinct weeks for PeerProgramming). `lib/contributor-access/member-value-counts.ts` derives its all-time per-member queries from that registry instead of carrying its own copies, and the daily reading derives a per-day one. Adding a value event is one entry and both readings pick it up.

2. **The one difference between the two readings, and it is stated on the screen.** Each event carries a `delivers` flag: whether somebody on the other side received something material. The badge scores every event, because earning it already requires five distinct counterparties, so a member who only talks can never clear it however long they keep talking — and narrowing what the badge scores would move who qualifies. The daily count has no such gate, since one event would put a member on the day's roster, so it draws only on the ones marked as delivering — twelve of fourteen as of 2026-09-25. The two left out are finishing a SkillUp course (the learner received the teaching; the trainer who gave it is credited by their own payout event) and posting in a PeerProgramming cohort. Contributions confirmed in real money counts as delivering even though no member is on the other side of the row, because something material arrived (owner directive, 2026-09-20).

3. **Attribution is the badge's attribution.** Value is credited to the member who delivered it — provider, host, fulfiller, sender, scout, author — which is what the badge has always counted. A ride credits its driver, so 384 here means 384 members delivering, not 384 people involved in something. Reaching it on a day therefore needs a good deal more than 384 approved members on the books, because nobody delivers every day: the target is a day's attendance, not a register (owner note, 2026-09-20).

4. **What a day's score is.** The same weights the badge uses, from `contributor_access_config.weights` through `effectiveWeight`, applied to that day's events. The count of members is what the target measures; the score orders the roster of who is delivering today.

5. **Every event has a real completion time (migration `0031_exchange_completion_timestamps.sql`).** Two tables recorded the outcome and not the moment. SocketRelay's close button already asked whether it went well and wrote `close_reason`; LightHouse moved a match to `completed`. Neither wrote a time, so a day-by-day reading had to fall back to `updated_at`, which any later edit moves — a stay from March could land on today because somebody touched the row, and in a reading that counts members per day a moving date moves a member between days. The migration adds `socket_relay_fulfillments.closed_at` and `lighthouse_matches.completed_at` and backfills finished rows from `updated_at` so the figures do not jump; both write paths now stamp the column once and leave it alone (`COALESCE(closed_at, NOW())` on close, a `CASE` on the move to `completed`). The registry reads `COALESCE(closed_at, updated_at)` so a row the backfill could not reach still dates.

6. **Not a second definition of the badge (§ contributor access).** It reads the same registry. The badge sums a member's entire time here, gates on account age and distinct plugins, and is permanent once earned, so it says who has ever contributed and can never say what happened today. This says who delivered today and how many.

7. **`GET /api/admin/daily-exchange`** — admin read, read-only, no parameters, no writes. Returns `{ reading }`. `reading` carries `readAt`, `target` (384), `today`, `bestDay` (`{ day, members }` or null, read across every day on record rather than the window on screen), `typicalDay` (median of the last 30), `yearMedian` (median of the last 365), `longestRun` (longest stretch of consecutive days at or above the target within the year), `daysAtTarget`, `days` (30 entries of `{ day, members }`, quiet days filled in as zero so a gap is not mistaken for missing data), and `todayRoster` (member id, the member's handle where they have set one, and that day's score, at most 500). `reading.events` carries the definition itself — every weighted event with its label, plugin, the weight in force right now, and whether it counts toward the day or only toward the badge — so an admin can audit what is weighted without taking it on trust (owner directive, 2026-09-20). No per-event breakdown is returned for a named member: Foundation answered charged calls are among the thirteen and rule 132 keeps that participation internal.

8. **Screen `/admin/daily-exchange`**, linked from the admin directory at `/admin`. Four stat blocks (today, target, typical day, longest run), one bar per day drawn against the target rather than against the tallest day so a quiet run does not rescale into looking busy, then Delivering today (the roster, ordered by that day's score, each row showing the member's handle with their member id under it — the id alone was unreadable beside a badge list that has named its members by handle all along, and it stays because a member can use the app without ever setting a handle, owner directive 2026-09-20). The goal is stated under the chart: the median day over a year at or above the target. A median rather than a mean, because a mean is satisfied by a dozen enormous days and ten quiet months, which is not an economy anybody could rely on, and quiet days count as the zeros they are. Typical day is the same measure over thirty days, so it moves while the year figure is still filling; longest run counts consecutive days at or above the target and one quiet day ends it. The count of days that ever reached the target moved into the line above the chart, because on its own it says nothing about whether an economy lasts (owner directive, 2026-09-20). Below the reading sits the Weavers of the Commons section — the badge's eligible members, its eligibility settings and per-event weights, and the gated channel status — because the badge and the day score the same events with the same weights (owner decision, 2026-09-25; the Contributor Access inventory describes that section). It replaces the read-only What is weighted panel and the Weavers holder widget this screen used to carry. The admin directory row reads Daily Exchange and Badge.

The weights themselves are edited in the Weavers of the Commons section below the reading, where each event shows the weight in force and the ones that reach the badge only are marked (the read-only What is weighted panel that listed them here was removed on 2026-09-25 as a duplicate). A control copies the reading as plain text for pasting from a phone, and the copied text carries that list too. A closing note names the shared definition, the attribution, and the two proxy dates.

### 1.17 Expenses (admin, what the app costs to run)

Owner decision, 2026-09-26. The owner pays every running cost of Skills Economy personally, no member contributes toward them, and cut decisions are being made now (a feature was turned down that week over $60 a month). Before this there was no screen that said what anything costs. These are real US dollar amounts a person pays and are written about as money; the rule against money language covers ServiceCredits and in-app credits only.

1. **Table `admin_expenses`** — one row per cost line, entered by hand. `provider`, `purpose` (what it pays for), `kind` (`recurring` = the amount is per month, `one_off` = a single payment), `billing` (`fixed` or `usage`), `amount_cents` (NULL = not known yet, kept apart from 0, which is a checked, free line; for a range this is the low end), `amount_max_cents` (the high end of a range such as Railway's $18–25; NULL for a single figure; recurring only), `paid_on` (one-off only), `last_checked_on` (the day somebody last read the provider's bill for this line), `stopped_on` (a canceled recurring cost; the row stays, marked, and leaves the total, so a cut is on the record), `notes`, `created_at`, `updated_at`. Amounts are whole cents so sums are exact. No member data: nothing here names a member, so it has no account-deletion entry.
2. **Table `admin_expenses_audit_trail`** — every add, edit and removal (rule 131), with the row before and after in `metadata`. Same columns as the other admin audit trails; index on `(created_at DESC, actor_id, command)`.
3. **Starting list — migration `post/0041_admin_expenses_starting_lines.sql`.** The five amounts the owner gave on 2026-09-26 (Render $25 fixed; Railway $18–25 usage-based; Claude Code $100 fixed; the phone, their only machine, $60 fixed; RunPod usage-based with the amount not stated and expected to rise, left blank rather than guessed), marked checked that day. Infisical, Unleash and the Formance ledger at $0 on their own lines, because they are self-hosted on Railway and their cost is inside the Railway line. Every other provider the app uses — Neon, GetStream, Sentry, Clerk, Supabase, Expo, GitHub, ntfy and the domain name — with no amount and no check date, shown as not priced yet until somebody reads the bill. It writes only while the audit trail is empty, so once the list has been edited a re-run never brings back a removed line.
4. **`GET /api/admin/expenses`** — admin only (`requiredRoles: ['admin']`). Returns `{ ok, expenses, approvedMembers }`: every row, stopped and one-off included, and the count of distinct members on the Unlock `approved_full` tier (`unlock_verification_submissions.access_tier`). 503 with the reason on a database failure.
5. **`POST /api/admin/expenses`** — admin only, CSRF-guarded (`x-ctf-csrf: 1` + same-origin). Adds a line (`lib/admin-expenses/parse.ts` validates: provider required, amounts whole cents from 0 to $1,000,000, the upper amount not below the lower, dates `YYYY-MM-DD`; fields that do not apply to the kind are dropped). Writes an `admin.expenses.create` audit row.
6. **`PATCH /api/admin/expenses/[id]`** — admin only, CSRF-guarded. Replaces the editable fields of one line; writes `admin.expenses.update` with before and after. **`DELETE /api/admin/expenses/[id]`** — same guards; removes a line entered by mistake and writes `admin.expenses.delete` with the row as it was. A canceled cost is meant to be marked stopped instead, and the screen says so before removing a live recurring line.
7. **`GET /api/admin/expenses/audit-events`** — admin only, read-only. `?limit=` (default 100, at most 200). Returns `{ ok, events }`, newest first.
8. **Screen `/admin/expenses`**, linked from the admin directory at `/admin` as Expenses. Two tabs, Costs and Audit log. Costs opens with three figures: the monthly total (a range when any line is a range, with a count of lines not priced yet and therefore not in it), the cost per approved member per month (the monthly total divided by the approved count), and one-off payments in the last 12 months (never part of the monthly figure). Below: every live recurring line, largest first, with its amount, its share of the monthly total as a percentage and a bar (shares use the middle of any range), fixed or usage-based, and when it was last checked (Never checked in amber); then Not priced yet, Stopped, and One-off payments, each in its own group. Each line has Edit, Checked today (stamps today's date without opening the form) and Remove. Add a cost opens the same form. Copy as text puts the entire screen on the clipboard as plain text, totals first, one line per cost, for pasting into a message from a phone. The totals and the copied text both come from `lib/admin-expenses/summary.ts`, so the two cannot disagree. Audit log lists each change in plain words ("Railway: $18.00–$25.00 → $25.00").

**Billing by API, for later (not built).** Every line is entered by hand. As far as is known today, and to be confirmed against each provider's current documentation before building anything: Railway's public API reports usage and an estimated bill for the current period; RunPod's API reports the account balance and current spend per hour; Neon's API reports consumption per project on paid plans; GitHub's billing API reports Actions and storage usage (the repository already holds a `GH_ACTIONS_BILLING_TOKEN`); Sentry's stats API reports event volumes, not dollars. Render, GetStream, Clerk, Supabase, Expo, ntfy, the domain registrar, the Claude Code subscription and the phone have no billing API anybody here has found, so those stay manual.
...
- 2026-09-26: **Expenses, what the app costs to run (§1.17).** Owner decision: the owner pays every bill personally and is deciding what to cut with no screen that shows what anything costs. New admin screen `/admin/expenses` with a Costs tab (monthly total, each line's share of it, cost per approved member, one-off payments kept apart, lines not priced yet named and kept out of the total, Copy as text) and an Audit log tab. New tables `admin_expenses` and `admin_expenses_audit_trail` in `schema.sql`; `schema.demo.sql` regenerated. Starting list from migration `post/0041_admin_expenses_starting_lines.sql`. Routes `GET`/`POST /api/admin/expenses`, `PATCH`/`DELETE /api/admin/expenses/[id]`, `GET /api/admin/expenses/audit-events`; every change writes an audit row. Manual entry only; which providers report billing through an API is noted in §1.17 for later. Android: out of scope (admin, web-only per rule 105).
- 2026-09-25: **A fourteenth value event, PeerProgramming goal cards that helped.** Owner decision: a goal-board card one member did for another, which the goal's owner marked "It helped", counts 3 toward the Weavers of the Commons badge and counts toward the day's 384 (`delivers: true`), credited to the member who did it. One entry in `value-events.ts`, so the badge, the daily count and the Weekly Performance Value section all pick it up. At most three per owner-and-helper pair per calendar week count, because only the owner confirms. The screen's closing note now reads twelve of the fourteen.

- 2026-09-25: **Daily Exchange and the badge on one admin page.** The Weavers of the Commons admin (eligible members, eligibility settings and weights, channel status) moved from `/admin/contributor-access` onto `/admin/daily-exchange`, below the reading, because the two readings score the same events with the same weights (owner decision). The screen's read-only What is weighted panel and Weavers holder widget are removed as duplicates of that section, and `GET /api/admin/daily-exchange` no longer returns `weavers`, which nothing read any more. The header reads Daily exchange and badge; the old address redirects to `/admin/daily-exchange#weavers`.

- 2026-09-20: **Daily Exchange, the 384 reading (§1.16).** The 384 goal had no counter and a wrong definition circulating: it is a day's throughput — 384 members trading with each other in one day, any 384 — and 384 is the sample size at which a reading generalizes to five million people. New read-only module `lib/engagement/exchange-activity.ts` counts distinct members on either side of a completed two-sided exchange per UTC day across seven sources (TrustTransport trips, ServiceCredits transfers including Chyme tips, Foundation calls, Foundation quotes, SocketRelay fulfillments, LightHouse matches, SkillUp trainer payouts), new route `GET /api/admin/daily-exchange`, new screen `/admin/daily-exchange` with a row in the admin directory. One-sided events are excluded because nobody is on the other side of the row; Recurring Activity is excluded because its row is a tie confirmed once rather than something done on a day; `login_events` is excluded because turning up is not trading. Counts only — no member data leaves the route, and no per-event breakdown is shown against a named member, because Foundation answered charged calls are among the thirteen and rule 132 keeps that participation internal. Owner directive in the same piece of work: the daily reading and the badge must use identical events and identical weights and stay in sync as features change, since they are the same measure over different dates. So the event definitions moved into `lib/contributor-access/value-events.ts` and both readings derive their queries from it — the badge's all-time per-member counts and the daily per-day counts — rather than each carrying its own SQL. Attribution follows the badge (the member who delivered the value), which means 384 counts deliverers rather than everybody involved. The screen carries a widget for each reading. Owner directive in the same work: SocketRelay and LightHouse recorded the outcome of a finished exchange but never the moment, so `0031_exchange_completion_timestamps.sql` adds `closed_at` and `completed_at`, backfills finished rows from `updated_at`, and both write paths stamp the new column once — a day a stay happened no longer moves because the row was edited later. Also recorded: reaching 384 on a day needs more than 384 approved members on the books, since nobody delivers every day. And, later the same day: Beacon broadcast engagement is removed from the value events and PeerProgramming posting drops to a weight of 1, because the measure is meant to separate members who deliver from members who only speak — the Contributor Access inventory carries the reasoning and what it does not change. And: each event now carries a `delivers` flag, so the daily count draws only on the eleven where somebody received something material while the badge keeps scoring all thirteen behind its counterparty gate — one definition, one set of weights, the difference stated on the screen rather than hidden. And last: the single-hit count is replaced on the face of the screen by a median — the goal is the median day over a year at or above 384, because reaching it once is a spike rather than an economy, and an average would hide a collapse. Later again: WhatWorks endorsements are removed from the event set altogether, because in WhatWorks the recommendation is the thing of value — somebody lists a tool and others mark whether it worked — so listing one stays weighted and agreeing with one does not.
...
- 2026-08-28: **A deletion event now records who asked for it.** `account_deletion_events.summary`
  ...
  Until now every full-account deletion wrote the same row whether the member chose to go or an
...
- 2026-08-18: **The Bug Reports dot no longer depends on a background job leaving the report
  ...
  so has already seen. This is the first signal where an automated job, not an admin, was moving
  rows out of the counted set; other areas' queues are drained only by an admin, so their
  status filters still hold. Registry change only (`lib/admin/area-attention.ts`) — no schema,
