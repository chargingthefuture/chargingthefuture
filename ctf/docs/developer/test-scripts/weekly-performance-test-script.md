...
2. **Current week auto-populates live.** Open the dashboard on the current (open) week. It shows
   live numbers computed from this week's activity (active members, questions, answers, community
   posts, enrollments) — not the old "metrics appear when the week closes" placeholder and not a
   blank page. With no activity yet the cards read zero, which is still a real value. → web ☐ mobile ☐ android ☐
...
3. On the current (open) week, leave the dashboard open: it silently re-fetches about every 60s
   and on tab focus, so the numbers refresh without a manual reload. Closed/past weeks do not poll.
**Expected:** Metric cards render humanized labels from `metric_key` and real values. A week with a
stored snapshot shows the recorded values; a week with no snapshot (the normal case for the open
week) shows numbers computed live for that week window from upstream tables. The comparison shows
per-metric deltas (this week vs last week); a declining metric shows a downward-trend indicator.
Reads audit `weekly-performance.metrics.get` or `…comparison.get` per branch. Loading, empty,
missing-metrics, and error states are all distinct.
**Result:** web ☐ mobile ☐ android ☐ — notes:
