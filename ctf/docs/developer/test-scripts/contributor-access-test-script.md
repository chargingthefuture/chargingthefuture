...
**Expected:** The editor shows: score threshold, minimum account age (days), minimum distinct plugins, minimum distinct counterparties, minimum eligible members to open channel, and per-event weight overrides. There are fourteen events to override. The fourteenth, PeerProgramming goal card that helped, was added on 2026-09-25 with a default of 3: a goal-board card one member did for another that the goal's owner marked "It helped" (PeerProgramming test script, PP-21). Two were removed on 2026-09-20: Beacon broadcast engagement, because reacting under a broadcast is talk rather than something given, and WhatWorks endorsements, because an endorsement is an upvote and the thing of value in WhatWorks is the recommendation — listing a tool, which stays. PeerProgramming posting reads a default of 1 rather than 2. No row for either removed event appears anywhere in the editor. It also shows the channel open/closed toggle and the channel status card (eligible count vs minimum, OPEN/CLOSED badge, synced Stream member count or "unavailable"). All values match the system defaults (threshold 100, age 90 days, 3 plugins, 5 counterparties, 10 min eligible) if the config has never been written.
