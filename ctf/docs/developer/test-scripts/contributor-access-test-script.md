...
**Expected:** The editor shows: score threshold, minimum account age (days), minimum distinct plugins, minimum distinct counterparties, minimum eligible members to open channel, and per-event weight overrides. There are fourteen events to override, not fifteen: Beacon broadcast engagement was removed on 2026-09-20 because reacting under a broadcast is talk rather than something delivered, and PeerProgramming posting reads a default of 1 rather than 2. No row for Beacon appears anywhere in the editor. It also shows the channel open/closed toggle and the channel status card (eligible count vs minimum, OPEN/CLOSED badge, synced Stream member count or "unavailable"). All values match the system defaults (threshold 100, age 90 days, 3 plugins, 5 counterparties, 10 min eligible) if the config has never been written.
