...
while account deletion stamped the timestamp, and each query tested whichever one its author picked. Both columns were dropped on 2026-09-21 (`db/migrations/post/0033`): every delete is a hard delete, so no tombstone is left for anything to disagree about, and the admin list has no “include deleted” scope.
