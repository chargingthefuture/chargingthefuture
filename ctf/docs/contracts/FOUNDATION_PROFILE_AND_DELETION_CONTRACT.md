...
- Table/entity: `foundation_call_sessions`
  - Contains personal data? yes (participant linkage, call modality, duration; instant-call ring lifecycle columns — `caller_user_id`, `callee_user_id`, `ring_status`, `answered_at`, `ended_at`, `ended_by_user_id` — added for issue #808 task 3; plus per-block billing columns — `rate_credits_locked`, `interval_minutes_locked`, `authorized_blocks`, `blocks_charged`, `paid_through_at`, `last_transfer_id`, `ended_reason` — added for issue #808 task 4)
  - Retention period: short-lived operational + medium-lived summary window for continuity/history
  - Legal/compliance note: trauma-informed handling and region-pinning required for real-time media. The call row holds no call media. The per-block billing columns describe the call's billing STATE only (the locked rate, how many blocks were paid, when the paid window ends, and the most recent transfer id for trace); they hold no wallet balance. **The money itself is NOT stored here — every block charge is a row in the canonical `service_credits_*` tables** (`service_credits_transfers`, `service_credits_ledger_entries`, `service_credits_escrow_holds`, `service_credits_command_idempotency`, `service_credits_adapter_outbox`). Those financial records are retained for financial-record / accountability purposes and are governed by the ServiceCredits deletion/reclaim policy (`account.deletion.reclaim.execute`) — they are NOT deleted with a Foundation call session. `last_transfer_id` is a reference into that ledger, not a value. Caller/callee linkage on the call row is deleted/pseudonymized on the same scopes as the existing call-session linkage; the linked ledger rows follow the ServiceCredits retention policy independently. Disputes/refunds for an instant-call block are a deferred follow-up handled through the existing ServiceCredits dispute path (`service_credits_disputes` + admin dispute APIs), not through this table.
