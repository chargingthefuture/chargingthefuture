# Design Audit: Replit Mockup vs Shell Code
    ...
    | Severity | Components |
    |---|---|
    | 🔴 STUB (< 15% implemented) | LevelUp, Trust |
    | 🟡 PARTIAL (< 65% implemented) | Mood, Community/Desktop |
    | 🟢 MINOR DRIFT (colors / labels) | Foundation, GDP, GentlePulse, LightHouse, Workforce |
    ...
    - Shell is a stub. Design shows cohort cards, skill tracks (React, Finance, Budgeting), enrollment CTAs, and progress bars.
    - **Blocker**: No `/api/levelup` route exists. Cannot implement without backend first.
    - **Verdict**: ⚠️ Backend API missing. Surfacing to user.

    ### Trust (shell = 1% of design)
    - **Shell file**: `ctf/packages/web/components/trust/trust-shell.tsx`
    - Shell is near-empty. Design shows a trust score dashboard with community endorsements and history.
    - **Status**: Awaiting user approval + API verification.
    ...
    ### Foundation, GDP, GentlePulse, LightHouse, Workforce
