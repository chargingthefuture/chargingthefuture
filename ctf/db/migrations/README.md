...
| `post/0001_directory_v2_to_v3_backfill.sql` | post | Copy the carried-over v2 Directory data into the v3 columns it landed empty in: `display_name <- first_name`, `bio <- description`, `profile_url <- quora_url`. Only fills columns that are still blank (never clobbers a later edit) and each copy is guarded on the v2 source column existing (so it is a no-op on a fresh v3 database). Profiles stay unclaimed. |
