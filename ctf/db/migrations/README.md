...
| `post/0003_socketrelay_drop_display_name.sql` | post | Drop the unused `display_name` column from `socketrelay_user_extension`; nothing rendered it (SocketRelay identifies people by their Clerk `@username`). Guarded on `display_name` still existing, so re-runs are no-ops. |
