...
| `post/0041_admin_expenses_starting_lines.sql` | post | Fill the new `admin_expenses` table with every provider in the stack: the five amounts the owner gave on 2026-09-26, Infisical/Unleash/Formance at $0 (inside the Railway line), and the rest with no amount yet. Writes only while `admin_expenses_audit_trail` is empty, so once anybody edits the list a re-run never brings back a removed line. |
