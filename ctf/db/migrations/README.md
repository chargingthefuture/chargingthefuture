...
| `post/0009_drop_skill_up_auto_cohort_tables.sql` | post | Drop the three SkillUp auto-cohort generation tables (`skill_up_cohort_proposals`, `skill_up_auto_cohort_config`, `skill_up_auto_cohort_term_overrides`, plus their pre-rename `level_up_` names) after the proposal-queue generation was removed on 2026-08-29. The cohorts that run already opened are untouched. Guarded with `IF EXISTS`, so re-runs are no-ops. |
