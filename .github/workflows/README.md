...
| `stream-guest-listener-setup.yml` | Stream — Guest Listener Setup | Weekly Tue 05:52 UTC (check); manual | Owns the Chyme guest-listener Stream setup in code: the `CHYME_GUEST_STREAM_ROLE` role exists, and on the `default` audio call it can join and read but not publish. Modes: plan (print only), check (the schedule — red on drift), apply (write and read back). Never prints a secret. |
