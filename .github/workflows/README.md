...
| `code-review-sweep.yml` | Code Review — Sweep and File Issues | Manual (schedule optional) | Reviews one slice of the codebase per run and files `code-review` issues for the findings, rotating through the codebase via a ledger. Files issues only — never writes code. |
