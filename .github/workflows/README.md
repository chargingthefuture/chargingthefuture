...
| `code-review-implement.yml` | Code Review — Open PR for Actionable Finding | Manual | Uses Claude to implement one `code-review:actionable` finding, runs typecheck, and opens a PR. The "actionable finding → PR" half of the incremental review pipeline. |
...
| `security-findings-triage.yml` | Security — Findings Triage | Mondays 13:00 UTC; manual | Reads the repo's open Dependabot, code scanning (CodeQL), and secret-scanning alerts and writes one always-current triage issue in the private triage repo (never prints secret values). |
