...
| `stream-guest-listener-setup.yml` | Stream — Guest Listener Setup | Weekly Tue 05:52 UTC (check); manual | Owns the Chyme guest-listener Stream setup in code: the `CHYME_GUEST_STREAM_ROLE` role exists, and on the `default` audio call it can join and read but not publish. Modes: plan (print only), check (the schedule — red on drift), apply (write and read back). Reads Stream secrets from Infisical, falling back to GitHub Actions secrets of the same name when Infisical is unreachable; the role can be given as an input. Never prints a secret. |
...
| `skills-proposal-issues.yml` | Skills Hunt — Propose Skill Promotions | Every 6 hours at :17; manual (optional `skip_classification`, `proposal_limit`) | Turns free-text "proposed" skills from accepted Skills Hunt nominations into GitHub issues proposing they join the canonical taxonomy, with an AI-suggested sector + occupation. Only files issues — never writes the taxonomy. A manual run with `skip_classification` files the issues with the placement left open (for an unfunded Anthropic account); the agent session that started the run then adds the placement to each issue. |
