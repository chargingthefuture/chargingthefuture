...
| `stream-grant-guest-listener.yml` | Stream — Grant Guest Listener Join | Manual | Repair: gives the Chyme guest-listener role (`CHYME_GUEST_STREAM_ROLE`) permission to join and read the `default` audio call, and removes its publish capabilities, through Stream's Video API — the one-time dashboard step, doable from a phone. Dry run unless **apply** is ticked; never prints a key or secret. |
